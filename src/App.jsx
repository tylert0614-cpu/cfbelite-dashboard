...
function NetworkMessageBody({body="",customEmojis=[]}){
  return String(body).split(/(:[a-z0-9_]{2,32}:)/gi).map((part,index)=><React.Fragment key={`${part}-${index}`}><NetworkEmojiToken value={part} customEmojis={customEmojis}/></React.Fragment>);
}
...
function LeagueHub({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],weeklyMatchups=[],currentYear,setActiveTab,setError}) {
  ...
  useEffect(()=>{loadChannelMessages();setReplyingTo(null);},[selectedChannel]);
  ...
  const renderChannelButton=(channel)=>{const matchup=matchupForChannel(channel);const team1=matchup?.team_1||teams.find((team)=>String(team.id)===String(matchup?.team_1_id));const team2=matchup?.team_2||teams.find((team)=>String(team.id)===String(matchup?.team_2_id));return <button key={channel.id} className={`${String(selectedChannel)===String(channel.id)?"active":""} ${channel.is_auto_matchup?"matchup":""}`} onClick={()=>openMobileChannel(channel)}>{channel.is_auto_matchup?<span className="network-matchup-logos"><TeamLogoMark team={team1} size={27}/><b>VS</b><TeamLogoMark team={team2} size={27}/></span>:<NetworkChannelMark channel={channel} size={38}/>}<span>{channel.is_auto_matchup?<><strong>{channel.week||"Matchup Room"}</strong><small>Private scheduling room</small></>:<><strong>{channel.name}</strong><small>{channel.description}</small></>}</span>{channel.is_auto_matchup?<em>YOUR GAME</em>:(channel.channel_type==="announcements"||channel.is_locked)&&<em>READ ONLY</em>}</button>;};
...
function HomeNewsroomRail({setActiveTab}) {
  ...
  useEffect(()=>{let active=true;async function load(){const {data}=await supabase.from("league_news_articles").select("id,season_year,week_label,category,headline,dek,body,published_at").eq("status","published").order("published_at",{ascending:false}).limit(4);if(active)setArticles(data||[]);}load();const channel=supabase.channel?.("home-newsroom-v35").on("postgres_changes",{event:"*",schema:"public",table:"league_news_articles"},load).subscribe();return()=>{active=false;if(channel)supabase.removeChannel?.(channel);};},[]);
...
function CoachProfile({ user, users = [], teams, assignments, results, weeklyMatchups = [], currentYear, currentWeek, rankingSnapshots = [], rankingHistory = [], allAmericans, awards, heismans, nationalChampions, recruiting, seasonPlayerStats = [], teamSeasonStats = [], sportsbook = {} }) {
  ...
  const nextMatchup = teamSchedule.find((row)=>!matchupHasResult(row));
...
function TabBar({ tabs, activeTab, setActiveTab, draggedTab, setDraggedTab, reorderTabs, adminUnlocked, adminCodeInput, setAdminCodeInput, unlockAdmin, teams = [], assignments = [], currentYear, users: navUsers = [], discordSession, linkedDiscordUser, signInWithDiscord, signOutDiscord,soundPreferences={}}) {
  ...
  const groups = [
    { title: "League", keys: ["dashboard", "leagueHub", "newsroom", "schedule", "redZone", "myTeam"] },
    { title: "Elite Books", keys: ["eliteBooks", "sportsbookHistory"] },
    { title: "Rankings", keys: ["eloRankings", "powerIndex", "rankingHistory", "conferencePower", "recruitingRankings"] },
  ...
  const navMeta={dashboard:["⌂","League overview"],leagueHub:["#","Chat, roles and channels"],newsroom:["N","Verified league coverage"],schedule:["G","Games and final scores"],redZone:["●","Live league streams"],myTeam:["M","Your personal headquarters"],eliteBooks:["$","Weekly picks and futures"],sportsbookHistory:["↗","Career betting ledger"],eloRankings:["E","User skill ratings"],powerIndex:["A","All-time coach rankings"],rankingHistory:["Y","Final rankings by year"],conferencePower:["C","Conference strength"],recruitingRankings:["R","Recruiting leaderboard"],allTeamsRatings:["T","Team ratings directory"],dynastyTimeline:["◷","Dynasty chronology"],dynastyRecords:["★","League record book"],rivalries:["⚔","Recurring matchups"],h2h:["VS","Head-to-head records"],coachHOF:["H","Legendary coaches"],playerHOF:["P","Legendary players"],allAmericans:["AA","All-American archive"],awards:["★","Award winners"],heismans:["H","Heisman history"],nationalChampions:["🏆","Championship history"],commissionerCenter:["⚙","Commissioner operations"],sportsbookManager:["$","Lines, locks and grading"],weeklyMatchups:["G","Schedule operations"],userManager:["U","Members and access"],assignments:["T","Coach-team assignments"],leagueDataCenter:["D","League data entry"],resultsManager:["✓","Results operations"],logoManager:["◇","Team asset manager"]};
          ...
          <aside style={drawerPanel} onClick={(event) => event.stopPropagation()}>
            ...
            <div style={drawerContent}>
              {visibleGroups.filter((group)=>group.keys.length > 0).map((group) => (
                <div key={group.title} style={drawerGroup} className="drawer-menu-group">
