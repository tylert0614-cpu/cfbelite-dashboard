...
function NetworkChannelMark({channel,size=38}) {
  ...
  return channel?.image_url?<span className="network-channel-mark image" style={style}><img src={channel.image_url} alt=""/></span>:<span className="network-channel-mark" style={style}>{channel?.icon||"#"}</span>;
}
...
function NetworkEmojiLibrary({customEmojis=[],isCommissioner,emojiUpload,setEmojiUpload,emojiBusy,uploadCustomEmoji,deleteCustomEmoji}){
  ...
  return <div className="network-emoji-library"><header><div><span>LEAGUE EMOJI VAULT</span><h2>Custom Reactions</h2><p>Upload the image, confirm its name, then add it to every channel’s emoji picker.</p></div><b>{customEmojis.length} EMOJIS</b></header>{isCommissioner&&<div className="network-emoji-uploader"><label><span>1. Choose Image • 512 KB max</span><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={chooseFile}/></label><label><span>2. Emoji Name</span><input value={emojiUpload.name} onChange={(event)=>setEmojiUpload({...emojiUpload,name:event.target.value.toLowerCase().replace(/[^a-z0-9_]/g,"")})} placeholder="panther_party"/><small>{emojiUpload.file?`${emojiUpload.file.name} • ${Math.ceil(emojiUpload.file.size/1024)} KB`:"PNG, JPG, WebP or GIF"}</small></label><button disabled={emojiBusy} onClick={uploadCustomEmoji}>{emojiBusy?"Uploading…":"3. Add League Emoji"}</button></div>}<div className="network-emoji-gallery">{customEmojis.map((emoji)=><article key={emoji.id}><img src={emoji.image_url} alt={emoji.name}/><strong>:{emoji.name}:</strong>{isCommissioner&&<button onClick={()=>deleteCustomEmoji(emoji)}>Delete</button>}</article>)}{!customEmojis.length&&<div className="network-empty"><b>No custom emojis yet.</b><span>Commissioners can upload the league's first reaction.</span></div>}</div></div>;
}
...
function LeagueHub({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],currentYear,setActiveTab,setError}) {
  ...
  const [channelForm,setChannelForm]=useState({id:null,name:"",slug:"",description:"",icon:"#",image_url:"",channel_type:"public",sort_order:100,is_locked:false});
  ...
  async function loadNetworkShell() {
    if(!discordSession?.user)return;
    await supabase.rpc("ensure_league_network_profile");
    const [channelRes,conversationRes,memberRes,notificationRes,prefRes,presenceRes,permissionRes,emojiRes,roleRes,roleMemberRes]=await Promise.all([
      supabase.from("league_channels").select("*").eq("is_archived",false).order("sort_order"),
      supabase.from("direct_conversations").select("*").order("last_message_at",{ascending:false}),
      supabase.from("direct_conversation_members").select("*"),
      supabase.from("app_notifications").select("*").order("created_at",{ascending:false}).limit(80),
    ...
    setConversations(conversationRes.data||[]);setConversationMembers(memberRes.data||[]);setPresence(presenceRes.data||[]);setPermissions(permissionRes.data||[]);setCustomEmojis(emojiRes.data||[]);setRoles(roleRes.data||[]);setRoleMembers(roleMemberRes.data||[]);
    const nextNotifications=notificationRes.data||[];
    if(lastNotificationRef.current&&nextNotifications[0]?.id&&nextNotifications[0].id!==lastNotificationRef.current&&prefRes.data?.sound_enabled!==false)playEliteSound("notification",true);
    lastNotificationRef.current=nextNotifications[0]?.id||lastNotificationRef.current;
    const nextPreferences=prefRes.data||null;setNotifications(nextNotifications);setPreferences(nextPreferences);
  ...
  async function enablePush(){try{if(!("serviceWorker" in navigator)||!("PushManager" in window))throw new Error("Push notifications are not supported in this browser.");const permission=await Notification.requestPermission();if(permission!=="granted")throw new Error("Notification permission was not granted.");const vapid=import.meta.env.VITE_VAPID_PUBLIC_KEY;if(!vapid)throw new Error("Push delivery will activate after the commissioner installs the VAPID public key.");const registration=await navigator.serviceWorker.register("/sw.js");const bytes=Uint8Array.from(atob(vapid.replace(/-/g,"+").replace(/_/g,"/")),(char)=>char.charCodeAt(0));const subscription=await registration.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:bytes});const json=subscription.toJSON();const {error}=await supabase.from("push_subscriptions").upsert({auth_user_id:discordSession.user.id,endpoint:json.endpoint,p256dh:json.keys.p256dh,auth_secret:json.keys.auth,user_agent:navigator.userAgent,last_used_at:new Date().toISOString()},{onConflict:"endpoint"});if(error)throw error;await savePreference("push_enabled",true);setError?.("Push notifications enabled on this device.");}catch(error){setError?.(error.message);}}
  async function markNotificationsRead(){await supabase.rpc("mark_app_notification_read",{});await loadNetworkShell();}
  ...
  function openChannelManager(channel=null){const row=channel||{id:null,name:"",slug:"",description:"",icon:"#",image_url:"",channel_type:"public",sort_order:(channels.at(-1)?.sort_order||0)+10,is_locked:false};setChannelForm({...row,image_url:row.image_url||""});setPermissionUserId("");setPermissionDraft({view:"inherit",post:"inherit",manage:"inherit",muted_until:""});setManageOpen(true);}
  async function saveChannel(){setBusy(true);const {data,error}=await supabase.rpc("manage_league_channel",{p_channel_id:channelForm.id||null,p_name:channelForm.name,p_slug:channelForm.slug,p_description:channelForm.description||"",p_icon:channelForm.icon||"#",p_image_url:channelForm.image_url||null,p_channel_type:channelForm.channel_type,p_sort_order:Number(channelForm.sort_order)||100,p_is_locked:Boolean(channelForm.is_locked)});setBusy(false);if(error){setError?.(`Channel not saved: ${error.message}`);return;}const saved=Array.isArray(data)?data[0]:data;await loadNetworkShell();setSelectedChannel(saved?.id||selectedChannel);setManageOpen(false);setError?.("Channel settings saved.");}
  ...
  async function uploadCustomEmoji(){
    ...
    if(!file||!file.type.startsWith("image/")||file.size>524288){setError?.("Choose a PNG, JPG, WebP or GIF no larger than 512 KB.");return;}
    setEmojiBusy(true);const extension=(file.name.split(".").pop()||"png").toLowerCase().replace(/[^a-z0-9]/g,"");const path=`${Date.now()}-${name}.${extension}`;
    const upload=await supabase.storage.from("league-emojis").upload(path,file,{contentType:file.type,upsert:false});
  ...
  const renderChannelButton=(channel)=><button key={channel.id} className={`${String(selectedChannel)===String(channel.id)?"active":""} ${channel.is_auto_matchup?"matchup":""}`} onClick={()=>openMobileChannel(channel)}><NetworkChannelMark channel={channel} size={38}/><span><strong>{channel.name}</strong><small>{channel.description}</small></span>{channel.is_auto_matchup?<em>YOUR GAME</em>:channel.is_locked&&<em>LOCKED</em>}</button>;
  const mobilePanelTitle=mode==="channels"?(selectedChannelRow?.name||"League Channel"):mode==="direct"?(conversationOther(selectedConversation)?.discord_username||"Direct Messages"):({notifications:"Alerts",roles:"Roles & Permissions",emojis:"League Emojis",settings:"Settings"}[mode]||"League Hub");
  ...
  return <main className={`cfb-v2-page network-page network-mobile-${mobileView}`}>
    ...
    <div className="network-layout"><aside className="network-sidebar">
      ...
      {mode==="channels"&&<><div className="network-sidebar-label"><span>CHANNELS</span>{canCreateChannels&&<button onClick={()=>{openChannelManager();setMobileView("panel");}}>＋ New</button>}</div><div className="network-channel-list"><div className="network-channel-category">LEAGUE CENTRAL</div>{publicChannels.map(renderChannelButton)}{matchupChannels.length>0&&<div className="network-channel-category">USER SCHEDULING • {matchupChannels.length}</div>}{matchupChannels.map(renderChannelButton)}</div><div className="network-online-roster"><header><i/><span>ONLINE NOW</span><b>{onlineUsers.length}</b></header>{onlineUsers.slice(0,12).map((user)=><div key={user.id}><span className="network-presence-dot"/><NetworkIdentity userId={user.id} users={users} teams={teams} assignments={assignments} currentYear={currentYear} compact/></div>)}{!onlineUsers.length&&<small>No other members are active right now.</small>}</div></>}
    ...
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      @media (max-width:760px) {
        ...
        .network-role-manager,.network-emoji-library,.network-notifications,.network-settings,.network-channel-manager { height:100%;overflow-y:auto!important;background:#111218!important; }
      }
