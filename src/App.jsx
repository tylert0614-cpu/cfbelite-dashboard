...
const CFBELITE_SOCIAL_BUILD = "v54-network-mobile-experience-overhaul";
...
function playEliteSound(kind="notification",enabled=true) {
  ...
  const peak=kind==="notification"?.19:kind==="team"?.105:.075;
  const tail=kind==="notification"?.92:kind==="team"?.58:.26;
  master.gain.exponentialRampToValueAtTime(peak,now+.018);
  master.gain.exponentialRampToValueAtTime(.0001,now+tail);
  master.connect(context.destination);
  const notes=kind==="notification"?[587.33,739.99,880,1174.66]:kind==="team"?[196,246.94,293.66,392]:[659.25,783.99];
  notes.forEach((frequency,index)=>{
    const oscillator=context.createOscillator();
    const gain=context.createGain();
    const start=now+index*(kind==="notification"?.12:kind==="team"?.085:.055);
    ...
    const noteLength=kind==="notification"?.44:kind==="team"?.3:.16;
    gain.gain.exponentialRampToValueAtTime(.0001,start+noteLength);
    oscillator.connect(gain); gain.connect(master); oscillator.start(start); oscillator.stop(start+noteLength+.02);
  });
...
class FeatureErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error("CFBElite feature recovered from an error:", error, info);
  }

  render() {
    if (this.state.error) {
      return (
        <section className="cfb-feature-fallback">
          <span>FEATURE TEMPORARILY UNAVAILABLE</span>
          <h2>{this.props.title || "This section"} did not load</h2>
          <p>The rest of CFBElite is still available. Refresh this section or return later.</p>
...
export default function App() {
  ...
  async function loadData() {
    ...
    const [teamsRes, settingsRes, tabOrderRes, usersRes, assignmentsRes, standingsRes, rankingsRes, resultsRes, weeklyMatchupsRes, draftPicks27Res, draftSettings27Res, aaRes, awardsRes, heismanRes, championsRes, draftRes, playoffRes, recruitingRes, seasonStatsRes, teamStatsRes, historyRes, conferenceAssetsRes] = await Promise.all([
      ...
      supabase.from("discord_users").select("id,discord_username,is_active,is_commissioner,discord_avatar_url,sportsbook_seed,sportsbook_notes,is_banned,banned_at,banned_by,banned_reason").order("discord_username"),
    ...
    const rankingSnapshotsRes = await supabase
      ...
      .order("rank");
    const rankingHistoryRes = await supabase
      .from("automatic_ranking_history")
      .select("*")
      .order("season_year", { ascending: false })
      .order("final_rank");
    const loadResponses = [teamsRes, settingsRes, tabOrderRes, usersRes, assignmentsRes, standingsRes, rankingsRes, resultsRes, weeklyMatchupsRes, draftPicks27Res, draftSettings27Res, aaRes, awardsRes, heismanRes, championsRes, draftRes, playoffRes, recruitingRes, seasonStatsRes, teamStatsRes, historyRes, conferenceAssetsRes, rankingSnapshotsRes, rankingHistoryRes];
    const loadErrors = loadResponses.filter((response)=>response?.error).map((response)=>response.error);
    if (loadErrors.length) {
      console.warn("CFBElite optional data sources unavailable:", loadErrors.map((item)=>item.message));
      const criticalError = [teamsRes, usersRes, assignmentsRes].find((response)=>response?.error)?.error;
      setError(criticalError ? `Core league data could not load: ${criticalError.message}` : "");
    } else {
      setError("");
    }
    {
      ...
      setRankingHistory(rankingHistoryRes.error ? [] : (rankingHistoryRes.data || []));
    }
    setLoading(false);
  }
  useEffect(() => {
  ...
  useEffect(()=>{
    ...
    const touch=async()=>{
      ...
      const {error:presenceError}=await supabase.rpc("touch_league_presence",{p_active_tab:activeTab,p_status:status});
      if(active&&presenceError&&/suspended|banned|inactive/i.test(String(presenceError.message||""))){
        setError(presenceError.message);setLinkedDiscordUser(null);await supabase.auth?.signOut?.();
      }
    };
    touch();const timer=window.setInterval(touch,25000);const onVisibility=()=>touch();document.addEventListener("visibilitychange",onVisibility);
    return()=>{active=false;window.clearInterval(timer);document.removeEventListener("visibilitychange",onVisibility);};
  },[discordSession?.user?.id,linkedDiscordUser?.id,activeTab]);
  ...
  async function saveLeagueSettings() {
    const {data:previousSettings}=await supabase.from("league_settings").select("current_year,current_week").eq("id",1).maybeSingle();
    const payload = {
      id: 1,
    ...
    const priorYear=Number(previousSettings?.current_year);
    if(Number.isFinite(priorYear)&&priorYear!==payload.current_year){
      const priorResults=results.filter((row)=>String(row.season_year)===String(priorYear));
      const priorTeamIds=new Set(priorResults.flatMap((row)=>[String(row.team_1_id),String(row.team_2_id)]));
      const priorTeams=teamOptions.filter((team)=>priorTeamIds.has(String(team.id))||assignments.some((row)=>String(row.team_id)===String(team.id)&&assignmentActiveForYear(row,priorYear)));
      const finalRows=computerRankingRows(priorTeams,priorResults,assignments,users);
      if(finalRows.length){
        const historyPayload=finalRows.map((row)=>({
          season_year:priorYear,
          team_id:String(row.team.id),
          discord_user_id:coachForTeamYear(row.team.id,priorYear,assignments)?.discord_user_id||null,
          final_rank:row.rank,
          rating:Number(row.rating||0),
          wins:Number(row.wins||0),
          losses:Number(row.losses||0),
          sor:Number(row.sor||0),
          avg_pf:Number(row.avgPf||0),
          avg_pa:Number(row.avgPa||0),
          top_10_wins:Number(row.top10||0),
          archived_at:new Date().toISOString(),
        }));
        const {error:archiveError}=await supabase.from("automatic_ranking_history").upsert(historyPayload,{onConflict:"season_year,team_id"});
        if(archiveError){setError(`Season advancement stopped: final ${priorYear} rankings could not be archived (${archiveError.message}).`);return;}
        archivedSeason=priorYear;
      }
    }
    ...
    setError(archivedSeason?`${archivedSeason} final rankings archived. ${payload.current_year} ${payload.current_week} is now active.`:`League advanced to ${payload.current_year} ${payload.current_week}. Matchup channels synchronized.`);
    await loadData();
  }
  ...
  async function loadEliteBooksData() {
    const names = ["sportsbook_boards","sportsbook_lines","sportsbook_pick_directory","sportsbook_future_markets","sportsbook_future_options","sportsbook_future_pick_directory","sportsbook_badges","sportsbook_badge_awards","elite_books_standings","elite_books_all_time_standings","sportsbook_season_champions","elite_books_pick_consensus","elite_books_future_consensus"];
    try {
      ...
      setSportsbook({
        ...
        consensus: responses[11].error ? [] : (responses[11].data || []),
        futureConsensus: responses[12].error ? [] : (responses[12].data || []),
      });
    } catch (_) {
  ...
  async function setDiscordUserBan(userId,enabled,reason="") {
    const {data,error:banError}=await supabase.rpc("set_league_member_ban",{p_discord_user_id:String(userId),p_banned:Boolean(enabled),p_reason:reason||null});
    if(banError){setError(`League access update failed: ${banError.message}`);return false;}
    const updated=Array.isArray(data)?data[0]:data;
    setUsers((rows)=>rows.map((user)=>String(user.id)===String(userId)?{...user,...updated}:user));
    setError(`${updated?.discord_username||"League member"} ${enabled?"has been banned from the app":"can access the app again"}.`);
  ...
  return <><GlobalStyle/><div style={page} className={`cfb-app cfb-page-${activeTab}`}><div style={container} className="cfb-app-container"><Header loading={loading} reload={loadData}/><LeagueIdentityBar currentYear={currentYear} currentWeek={currentWeek} advanceAt={advanceAt} weeklyMatchups={weeklyMatchups} results={results} teams={teamOptions} setActiveTab={setActiveTab}/><AppSkeleton visible={loading && !teams.length}/>{error && <div style={isErrorMessage(error) ? errorBox : successBox}>{error}</div>}<TabBar tabs={tabs} activeTab={activeTab} setActiveTab={setActiveTab} draggedTab={draggedTab} setDraggedTab={setDraggedTab} reorderTabs={reorderTabs} adminUnlocked={adminUnlocked} adminCodeInput={adminCodeInput} setAdminCodeInput={setAdminCodeInput} unlockAdmin={unlockAdmin} teams={teamOptions} assignments={assignments} currentYear={currentYear} users={userOptions} discordSession={discordSession} linkedDiscordUser={linkedDiscordUser} signInWithDiscord={signInWithDiscord} signOutDiscord={signOutDiscord} soundPreferences={soundPreferences} draftIsLive={Boolean(draftSettings27?.is_live)}/><UniversalSearchBar tabs={tabs} teams={teamOptions} users={userOptions} assignments={assignments} weeklyMatchups={weeklyMatchups} results={results} currentYear={currentYear} setActiveTab={setActiveTab}/>
    ...
    {activeTab === "rankingsCenter" && <RankingsCenterV38 setActiveTab={setActiveTab}/>} 
...
{activeTab === "recruitingRankings" && <RecruitingRankings rows={recruiting} teams={teamOptions} users={userOptions} assignments={assignments} currentYear={currentYear} loadData={loadData} deleteRow={deleteRow} updateRow={updateRow}/>}    
    ...
    {selectedTeam && <TeamPage team={selectedTeam} standings={standings.find((row)=>row.team_id===selectedTeam.id)} results={currentYearResults} allResults={results} teams={teamOptions} assignments={assignments} allAmericans={allAmericans} awards={awards} heismans={heismans} recruiting={recruiting} historyRows={historyRows} currentYear={currentYear} addRecruiting={addRecruiting} addHistory={addHistory} updateRow={updateRow} deleteRow={deleteRow} newRecruiting={newRecruiting} setNewRecruiting={setNewRecruiting} newHistory={newHistory} setNewHistory={setNewHistory}/>}    
    {selectedCoach && <CoachProfile user={selectedCoach} users={userOptions} teams={teamOptions} assignments={assignments} results={results} weeklyMatchups={weeklyMatchups} currentYear={currentYear} currentWeek={currentWeek} rankingSnapshots={rankingSnapshots} rankingHistory={rankingHistory} allAmericans={allAmericans} awards={awards} heismans={heismans} nationalChampions={nationalChampions} recruiting={recruiting} seasonPlayerStats={seasonPlayerStats} teamSeasonStats={teamSeasonStats} sportsbook={sportsbook}/>}    
  </div></div></>;
}
...
function RankingMovement({ currentRank, previousRank }) {
  if (!previousRank) return <span style={v2MovementEven}>NEW</span>;
  if (previousRank===currentRank) return <span style={v2MovementEven}>EVEN</span>;
...
function gameCenterResultInsights(row) {
  ...
  const score1 = Number(scoreForScheduledTeam(row.result, row.team_1_id));
  const score2 = Number(scoreForScheduledTeam(row.result, row.team_2_id));
  const team1Won = score1 > score2;
  const winner = team1Won ? row.team1 : row.team2;
  const winnerRank = Number(team1Won ? row.rank1 : row.rank2);
  const loserRank = Number(team1Won ? row.rank2 : row.rank1);
  const tags = Array.isArray(row.result.tags) ? row.result.tags.join(" ") : String(row.result.tags || "");
  const insights = [{ label:`${getTeamAbbreviation(winner)} WIN`, kind:"winner" }];

  if (winnerRank && loserRank && winnerRank > loserRank) insights.push({ label:"RANKED UPSET", kind:"upset" });
  if (/overtime|\bot\b/i.test(tags) || row.result.overtime) insights.push({ label:"OVERTIME", kind:"overtime" });
  if (row.result.player_of_game || row.result.player_of_the_game) {
    insights.push({ label:`POTG: ${row.result.player_of_game || row.result.player_of_the_game}`, kind:"player" });
  }
...
function GameCenterV2({ teams = [], users = [], assignments = [], weeklyMatchups = [], results = [], currentYear = "2026", currentWeek = "Week 1", conferenceAssets = [], adminUnlocked = false, loadData = async()=>{}, setActiveTab }) {
  ...
  return <section className="cfb-v2-page" style={v2Page}>
    ...
    <div className="cfb-v2-game-grid" style={v2GameGrid}>{rows.length?rows.map((row)=><article key={row.id||`${row.week}-${row.team_1_id}-${row.team_2_id}`} style={{...v2GameCard,borderColor:`${getTeamSecondary(row.team1)}55`}}>
      ...
      {row.result && <div className="cfb-game-result-insights">{gameCenterResultInsights(row).map((item)=><span key={item.label} data-kind={item.kind}>{item.label}</span>)}</div>}
...
function cleanNetworkChannelName(value=""){
  return String(value).replace(/^#+\s*/,"").replace(/\s*#\s*$/,"").replace(/\s{2,}/g," ").trim();
}

function NetworkChannelMark({channel,size=38}) {
  const style={width:size,height:size};
  return channel?.image_url?<span className="network-channel-mark image" style={style}><img src={channel.image_url} alt=""/></span>:<span className="network-channel-mark glyph" style={style}>#</span>;
}

const NETWORK_EMOJIS=["👍","🔥","😂","🏆","😮","💀","👏","💯","❤️","👀","🎯","🚨","🫡","😤","🤝","✅","❌","🐐"];

function NetworkEmojiToken({value,customEmojis=[]}){
  const name=String(value||"").match(/^:([a-z0-9_]{2,32}):$/i)?.[1]?.toLowerCase();
...
function NetworkEmojiLibrary({customEmojis=[],isCommissioner,emojiUpload,setEmojiUpload,emojiBusy,uploadCustomEmoji,deleteCustomEmoji}){
  const chooseFile=(event)=>{const file=event.target.files?.[0]||null;const suggested=file?file.name.replace(/\.[^.]+$/,"" ).toLowerCase().replace(/[^a-z0-9]+/g,"_").replace(/^_|_$/g,"").slice(0,32):"";setEmojiUpload((current)=>({...current,file,name:current.name||suggested}));};
  return <div className="network-emoji-library"><header><div><span>LEAGUE EMOJI VAULT</span><h2>Custom Reactions</h2><p>Upload the image, confirm its name, then add it to every channel’s emoji picker.</p></div><b>{customEmojis.length} EMOJIS</b></header>{isCommissioner&&<div className="network-emoji-uploader"><label><span>1. Choose Image • 5 MB max</span><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={chooseFile}/></label><label><span>2. Emoji Name</span><input value={emojiUpload.name} onChange={(event)=>setEmojiUpload({...emojiUpload,name:event.target.value.toLowerCase().replace(/[^a-z0-9_]/g,"")})} placeholder="panther_party"/><small>{emojiUpload.file?`${emojiUpload.file.name} • ${Math.ceil(emojiUpload.file.size/1024)} KB`:"Oversized static images are optimized automatically"}</small></label><button disabled={emojiBusy} onClick={uploadCustomEmoji}>{emojiBusy?"Optimizing & Uploading…":"3. Add League Emoji"}</button></div>}<div className="network-emoji-gallery">{customEmojis.map((emoji)=><article key={emoji.id}><img src={emoji.image_url} alt={emoji.name}/><strong>:{emoji.name}:</strong>{isCommissioner&&<button onClick={()=>deleteCustomEmoji(emoji)}>Delete</button>}</article>)}{!customEmojis.length&&<div className="network-empty"><b>No custom emojis yet.</b><span>Commissioners can upload the league's first reaction.</span></div>}</div></div>;
}
...
function NetworkOrganizer({
  ...
  const sortedCategories=[...categories].sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
  const visibleChannels=channels.filter((channel)=>!channel.is_auto_matchup);
  const uncategorized=visibleChannels.filter((channel)=>!channel.category_id);
  const channelsFor=(categoryId)=>visibleChannels
    .filter((channel)=>String(channel.category_id||"")===String(categoryId||""))
    .sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));

  const renderChannel=(channel,index,list)=>(
    <article className="network-organizer-channel-v53" key={channel.id}>
      <span className="network-organizer-grip-v53">⋮⋮</span>
      <span className="network-organizer-channel-copy-v53">
        <strong>#{cleanNetworkChannelName(channel.name)}</strong>
        <small>{channel.description||"No description"}</small>
      </span>
      <select
        value={channel.category_id||""}
        disabled={busy}
        onChange={(event)=>onMoveChannelToCategory(channel,event.target.value||null)}
        aria-label={`Move ${channel.name} to category`}
      >
        <option value="">No Category</option>
        {sortedCategories.map((category)=><option key={category.id} value={category.id}>{category.name}</option>)}
      </select>
      <div className="network-organizer-actions-v53">
        <button type="button" disabled={busy||index===0} onClick={()=>onMoveChannel(channel,-1)}>↑</button>
        <button type="button" disabled={busy||index===list.length-1} onClick={()=>onMoveChannel(channel,1)}>↓</button>
      </div>
    </article>
  );

  return <section className="network-organizer-v53">
    <header>
      <div>
        <span>COMMISSIONER TOOLS</span>
        <h2>Organize Network</h2>
        <p>Reorder categories and channels or move a channel into a different category. Changes save immediately.</p>
      </div>
      <button type="button" onClick={onClose}>Done</button>
    </header>

    <div className="network-organizer-list-v53">
      {sortedCategories.map((category,categoryIndex)=>{
        const categoryChannels=channelsFor(category.id);
        return <section className="network-organizer-category-v53" key={category.id}>
          <header>
            <span className="network-organizer-grip-v53">⋮⋮</span>
            <div><strong>{category.name}</strong><small>{categoryChannels.length} channel{categoryChannels.length===1?"":"s"}</small></div>
            <div className="network-organizer-actions-v53">
              <button type="button" disabled={busy||categoryIndex===0} onClick={()=>onMoveCategory(category,-1)}>↑</button>
              <button type="button" disabled={busy||categoryIndex===sortedCategories.length-1} onClick={()=>onMoveCategory(category,1)}>↓</button>
            </div>
          </header>
          <div>{categoryChannels.map((channel,index)=>renderChannel(channel,index,categoryChannels))}</div>
        </section>;
      })}

      {uncategorized.length>0&&<section className="network-organizer-category-v53 muted">
        <header><span className="network-organizer-grip-v53">⋮⋮</span><div><strong>No Category</strong><small>{uncategorized.length} channel{uncategorized.length===1?"":"s"}</small></div></header>
        <div>{uncategorized.map((channel,index)=>renderChannel(channel,index,uncategorized))}</div>
      </section>}
    </div>
  </section>;
}
...
function LeagueHub({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],weeklyMatchups=[],results=[],currentYear,setActiveTab,setError}) {
  ...
  const [networkSearchAuthor,setNetworkSearchAuthor]=useState("");
  const [networkSearchHas,setNetworkSearchHas]=useState("any");
  ...
  const [emojiUpload,setEmojiUpload]=useState({name:"",file:null});
  ...
  async function loadNetworkShell() {
    ...
    const [channelRes,categoryRes,conversationRes,memberRes,notificationRes,prefRes,presenceRes,permissionRes,emojiRes,roleRes,roleMemberRes]=await Promise.all([
      ...
      supabase.from("league_presence").select("*"),
      supabase.from("league_channel_permissions").select("*"),
  ...
  async function loadChannelMessages(){
    ...
    const [messageRes,reactionRes]=await Promise.all([
      supabase.from("league_channel_messages").select("*").eq("channel_id",selectedChannel).is("deleted_at",null).order("created_at",{ascending:true}).limit(300),
      supabase.from("league_message_reactions").select("*"),
    ]);
    if(messageRes.error)setError?.(messageRes.error.message);else setMessages(messageRes.data||[]);
    setReactions(reactionRes.data||[]);
  }
  async function loadDirectMessages(){
    if(!selectedConversation)return;
    const {data,error}=await supabase.from("direct_messages").select("*").eq("conversation_id",selectedConversation).is("deleted_at",null).order("created_at",{ascending:true}).limit(300);
  ...
  async function loadSocialExtras(){
    if(!selectedChannel)return;
    const [pollRes,optionRes,voteRes,settingsRes]=await Promise.all([
      supabase.from("league_polls").select("*").eq("channel_id",selectedChannel).order("created_at",{ascending:false}),
      supabase.from("league_poll_options").select("*").order("sort_order"),
      supabase.from("league_poll_votes").select("*"),
      supabase.from("league_social_settings").select("*").eq("id",1).maybeSingle(),
    ]);
    if(!pollRes.error)setPolls(pollRes.data||[]);
    if(!optionRes.error)setPollOptions(optionRes.data||[]);
    if(!voteRes.error)setPollVotes(voteRes.data||[]);
    const url=settingsRes.data?.player_reporting_url||"";setReportingUrl(url);setReportingDraft(url);
  }
  useEffect(()=>{
    loadNetworkShell();const timer=window.setInterval(loadNetworkShell,25000);
    const channel=supabase.channel?.("league-network-v25")
      .on("postgres_changes",{event:"*",schema:"public",table:"league_channel_messages"},()=>loadChannelMessages())
      .on("postgres_changes",{event:"*",schema:"public",table:"league_message_reactions"},()=>loadChannelMessages())
      ...
      .on("postgres_changes",{event:"*",schema:"public",table:"league_poll_votes"},()=>loadSocialExtras())
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"app_notifications"},()=>loadNetworkShell()).subscribe();
    return()=>{window.clearInterval(timer);if(channel)supabase.removeChannel?.(channel);};
  },[discordSession?.user?.id,selectedChannel,selectedConversation]);
  useEffect(()=>{loadChannelMessages();loadSocialExtras();setReplyingTo(null);setSocialMessageSearch("");setPinnedOnly(false);},[selectedChannel]);
  useEffect(()=>{loadDirectMessages();setReplyingTo(null);},[selectedConversation]);

  async function sendMessage(overrideBody=null){
    const messageBody=String(overrideBody??draft).trim();
    if(!messageBody||busy)return;setBusy(true);
    const rpc=mode==="direct"?supabase.rpc("send_direct_message",{p_conversation_id:selectedConversation,p_body:messageBody,p_reply_to_id:replyingTo?.id||null}):supabase.rpc("send_league_channel_message",{p_channel_id:selectedChannel,p_body:messageBody,p_reply_to_id:replyingTo?.id||null});
    const {error}=await rpc;setBusy(false);
    if(error){setError?.(`Message not sent: ${error.message}`);return;}
    setDraft("");setReplyingTo(null);setEmojiOpen(false);setGiphyOpen(false);if(mode==="direct")await loadDirectMessages();else await loadChannelMessages();
  }
  async function uploadNetworkAttachment(file){
    if(!file||attachmentBusy)return;
    if(file.size>12*1024*1024){setError?.("Attachments must be 12 MB or smaller.");return;}
    setAttachmentBusy(true);
    try{
      const extension=(file.name.split(".").pop()||"bin").toLowerCase().replace(/[^a-z0-9]/g,"");
      const path=`${discordSession.user.id}/${Date.now()}-${Math.random().toString(36).slice(2)}.${extension}`;
      const {error:uploadError}=await supabase.storage.from("league-chat-media").upload(path,file,{contentType:file.type||"application/octet-stream",upsert:false});
      if(uploadError)throw uploadError;
      const {data}=supabase.storage.from("league-chat-media").getPublicUrl(path);
      await sendMessage(data.publicUrl);
    }catch(error){setError?.(`Attachment failed: ${error.message||error}`);}
  ...
  async function enablePush(){
    try{
      ...
      const functionUrl=`${String(import.meta.env.VITE_SUPABASE_URL||"").replace(/\/$/,"")}/functions/v1/send-push-notifications`;
      let vapid=import.meta.env.VITE_VAPID_PUBLIC_KEY||"";
      if(!vapid){
        const keyResponse=await fetch(`${functionUrl}?mode=public-key`);
        const keyPayload=await keyResponse.json().catch(()=>({}));
        if(!keyResponse.ok||!keyPayload.publicKey)throw new Error(keyPayload.error||"Push delivery is not configured yet.");
        vapid=keyPayload.publicKey;
      }
      ...
      await supabase.rpc("queue_test_push_notification");
      await fetch(functionUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}"});
      setError?.("Push notifications are enabled. A test notification is on the way.");
    }catch(error){setError?.(error.message);}
  }
  async function markNotificationsRead(){await supabase.rpc("mark_app_notification_read",{});await loadNetworkShell();}
  async function toggleReaction(messageId,reaction){const {error}=await supabase.rpc("toggle_league_message_reaction",{p_message_id:messageId,p_reaction:reaction});if(error)setError?.(`Reaction not saved: ${error.message}`);else await loadChannelMessages();}
  async function editMessage(message){const body=window.prompt("Edit message:",message.body);if(body===null||!body.trim()||body.trim()===message.body)return;const {error}=await supabase.rpc("edit_league_channel_message",{p_message_id:message.id,p_body:body.trim()});if(error)setError?.(`Message not edited: ${error.message}`);else await loadChannelMessages();}
  ...
  async function saveReportingUrl(){if(!isCommissioner)return;const {error}=await supabase.from("league_social_settings").upsert({id:1,player_reporting_url:reportingDraft.trim(),updated_at:new Date().toISOString()},{onConflict:"id"});if(error)setError?.(`Reporting link not saved: ${error.message}`);else{setReportingUrl(reportingDraft.trim());setError?.("Player reporting link saved.");}}
  function pollCountdown(poll){if(poll.is_closed)return "Closed";if(!poll.ends_at)return "No deadline";const remaining=new Date(poll.ends_at)-Date.now();if(remaining<=0)return "Ended";const hours=Math.floor(remaining/3600000);const minutes=Math.floor((remaining%3600000)/60000);const days=Math.floor(hours/24);return days?`${days}d ${hours%24}h remaining`:`${hours}h ${minutes}m remaining`;}
  ...
  async function persistChannelOrder(channel,sortOrder){
    const direct=await supabase.from("league_channels").update({sort_order:sortOrder,updated_at:new Date().toISOString()}).eq("id",channel.id);
    if(!direct.error)return null;
    const fallback=await supabase.rpc("manage_league_channel",{
      p_channel_id:channel.id,
      p_name:channel.name,
      p_slug:channel.slug,
      p_description:channel.description||"",
      p_icon:channel.icon||"#",
      p_image_url:channel.image_url||null,
      p_channel_type:channel.channel_type||"public",
      p_sort_order:sortOrder,
      p_is_locked:Boolean(channel.is_locked),
    });
    return fallback.error||null;
  }

  async function moveCategory(category,direction){
    if(!isCommissioner||busy)return;
    const ordered=[...categories].sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
    const index=ordered.findIndex((row)=>String(row.id)===String(category.id));
    const target=ordered[index+direction];
    if(index<0||!target)return;
    const currentOrder=Number(category.sort_order||((index+1)*10));
    const targetOrder=Number(target.sort_order||((index+direction+1)*10));
    setBusy(true);
    setCategories((current)=>current.map((row)=>String(row.id)===String(category.id)?{...row,sort_order:targetOrder}:String(row.id)===String(target.id)?{...row,sort_order:currentOrder}:row).sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0)));
    const [firstError,secondError]=await Promise.all([
      persistCategoryOrder(category,targetOrder),
      persistCategoryOrder(target,currentOrder),
    ]);
    setBusy(false);
    if(firstError||secondError){
      setError?.(`Category order was not saved: ${(firstError||secondError).message}`);
      await loadNetworkShell();
      return;
    }
    setError?.("Category order updated.");
    await loadNetworkShell();
  }
  ...
  async function moveChannel(channel,direction){
    ...
    const siblings=channels
      .filter((row)=>!row.is_auto_matchup&&String(row.category_id||"")===String(channel.category_id||""))
      .sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
    const index=siblings.findIndex((row)=>String(row.id)===String(channel.id));
    const target=siblings[index+direction];
    if(index<0||!target)return;
    const currentOrder=Number(channel.sort_order||((index+1)*10));
    const targetOrder=Number(target.sort_order||((index+direction+1)*10));
    setBusy(true);
    setChannels((current)=>current.map((row)=>String(row.id)===String(channel.id)?{...row,sort_order:targetOrder}:String(row.id)===String(target.id)?{...row,sort_order:currentOrder}:row));
    const [firstError,secondError]=await Promise.all([
      persistChannelOrder(channel,targetOrder),
      persistChannelOrder(target,currentOrder),
    ]);
    setBusy(false);
    if(firstError||secondError){
      setError?.(`Channel order was not saved: ${(firstError||secondError).message}`);
      await loadNetworkShell();
      return;
    }
    setError?.("Channel order updated.");
    await loadNetworkShell();
  }
  ...
  async function moveChannelToCategory(channel,categoryId){
    ...
    const destination=channels
      .filter((row)=>!row.is_auto_matchup&&String(row.category_id||"")===String(categoryId||""))
      .sort((a,b)=>Number(a.sort_order||0)-Number(b.sort_order||0));
    const nextOrder=destination.length?Number(destination.at(-1)?.sort_order||0)+10:10;
    const categoryResult=await supabase.rpc("set_league_channel_category",{p_channel_id:channel.id,p_category_id:categoryId||null});
    let orderError=null;
    if(!categoryResult.error)orderError=await persistChannelOrder(channel,nextOrder);
    setBusy(false);
    if(categoryResult.error||orderError){
      setError?.(`Channel was not moved: ${(categoryResult.error||orderError).message}`);
      return;
    }
    setError?.("Channel moved.");
    await loadNetworkShell();
  }
  ...
  async function optimizeEmojiFile(file){
    ...
    try{
      const image=await new Promise((resolve,reject)=>{const next=new Image();next.onload=()=>resolve(next);next.onerror=()=>reject(new Error("The image could not be read."));next.src=objectUrl;});
      const limit=320;const scale=Math.min(1,limit/Math.max(image.naturalWidth,image.naturalHeight));const width=Math.max(1,Math.round(image.naturalWidth*scale));const height=Math.max(1,Math.round(image.naturalHeight*scale));
      ...
      const body=await new Promise((resolve,reject)=>canvas.toBlob((blob)=>blob?resolve(blob):reject(new Error("The image could not be optimized.")),"image/png"));
      return {body,extension:"png",contentType:"image/png"};
    }finally{URL.revokeObjectURL(objectUrl);}
  }
  async function uploadCustomEmoji(){
    const name=emojiUpload.name.trim().toLowerCase();const file=emojiUpload.file;
    if(!isCommissioner)return;
    if(!/^[a-z0-9_]{2,32}$/.test(name)){setError?.("Emoji names must be 2–32 lowercase letters, numbers or underscores.");return;}
    if(!file||!file.type.startsWith("image/")||file.size>5242880){setError?.("Choose a PNG, JPG, WebP or GIF no larger than 5 MB.");return;}
    setEmojiBusy(true);let prepared;try{prepared=await optimizeEmojiFile(file);}catch(error){setEmojiBusy(false);setError?.(error.message);return;}const extension=prepared.extension.replace(/[^a-z0-9]/g,"");const path=`${Date.now()}-${name}.${extension}`;
    const upload=await supabase.storage.from("league-emojis").upload(path,prepared.body,{contentType:prepared.contentType,upsert:false});
    if(upload.error){setEmojiBusy(false);setError?.(`Emoji not uploaded: ${upload.error.message}`);return;}
    const {data:urlData}=supabase.storage.from("league-emojis").getPublicUrl(path);
    const {error}=await supabase.from("league_custom_emojis").insert({name,image_url:urlData.publicUrl,storage_path:path,created_by:String(linkedDiscordUser.id)});
    if(error){await supabase.storage.from("league-emojis").remove([path]);setError?.(`Emoji not saved: ${error.message}`);}else{setEmojiUpload({name:"",file:null});setError?.(`:${name}: is ready for the league.`);await loadNetworkShell();}
    setEmojiBusy(false);
  }
  async function deleteCustomEmoji(emoji){
    if(!isCommissioner||!window.confirm(`Delete :${emoji.name}: from the league?`))return;
    setEmojiBusy(true);const {error}=await supabase.from("league_custom_emojis").delete().eq("id",emoji.id);
    if(!error)await supabase.storage.from("league-emojis").remove([emoji.storage_path]);
    setEmojiBusy(false);if(error)setError?.(`Emoji not deleted: ${error.message}`);else await loadNetworkShell();
  }
  async function saveRole(){
    if(!isCommissioner)return;setBusy(true);
  ...
  const filteredMembers=users.filter((user)=>user.is_active!==false&&!user.is_banned&&String(user.id)!==String(linkedDiscordUser.id)&&String(user.discord_username).toLowerCase().includes(memberSearch.toLowerCase())).slice(0,12);
  const rawDisplayedMessages=mode==="direct"?directMessages:messages;
  const displayedMessages=rawDisplayedMessages.filter((message)=>{
    const authorName=String(users.find((user)=>String(user.id)===String(message.author_discord_user_id))?.discord_username||"");
    const body=String(message.body||"");
    const searchMatch=!socialMessageSearch||body.toLowerCase().includes(socialMessageSearch.toLowerCase())||authorName.toLowerCase().includes(socialMessageSearch.toLowerCase());
    const authorMatch=!networkSearchAuthor||authorName.toLowerCase().includes(networkSearchAuthor.toLowerCase());
    const hasLink=/https?:\/\//i.test(body);
    const hasImage=/\.(gif|png|jpe?g|webp)(?:\?.*)?$/im.test(body)||/tenor\.com|giphy\.com/i.test(body);
    const typeMatch=networkSearchHas==="any"||(networkSearchHas==="link"&&hasLink)||(networkSearchHas==="image"&&hasImage);
    const pinMatch=!pinnedOnly||Boolean(message.is_pinned);
    return searchMatch&&authorMatch&&typeMatch&&pinMatch;
  });
  const activePresence=presence.filter((row)=>Date.now()-new Date(row.last_seen_at).getTime()<90000&&row.status!=="offline");
  const presenceUserIds=new Set(activePresence.map((row)=>String(row.discord_user_id)));
  const onlineUsers=users.filter((user)=>presenceUserIds.has(String(user.id))&&!user.is_banned);
  const myRoleIds=new Set(roleMembers.filter((row)=>String(row.discord_user_id)===String(linkedDiscordUser.id)).map((row)=>String(row.role_id)));
  const myRoles=roles.filter((role)=>myRoleIds.has(String(role.id)));
  const canCreateChannels=isCommissioner||myRoles.some((role)=>role.can_manage_channels);
  const canManageMessages=isCommissioner||myRoles.some((role)=>role.can_manage_channels||role.can_manage_messages);
  const myPermission=permissions.find((item)=>String(item.channel_id)===String(selectedChannel)&&String(item.discord_user_id)===String(linkedDiscordUser.id));
  const canModerate=canManageMessages||myPermission?.can_manage===true;
  ...
  const renderChannelButton=(channel)=>{
    ...
    const matchupLabel=team1&&team2?`${team1.name} vs ${team2.name}`:"Scheduling Room";

    return <button
      type="button"
      key={channel.id}
      title={isMatchup?`${matchupLabel} • ${channel.week||"Matchup Room"}`:channelLabel}
      aria-label={isMatchup?`${matchupLabel}, ${channel.week||"Matchup Room"}, ${status}`:channelLabel}
      className={`cfbn-row-v53 ${String(selectedChannel)===String(channel.id)?"active":""} ${isMatchup?"logo-only-matchup":""}`}
      onClick={()=>openMobileChannel(channel)}
    >
      {isMatchup
        ? <span className="cfbn-logo-matchup-v53">
            <span className="cfbn-team-logo-v53">{team1?.logo_url?<img src={team1.logo_url} alt={team1.name||""}/>:<i/>}</span>
            <b>VS</b>
            <span className="cfbn-team-logo-v53">{team2?.logo_url?<img src={team2.logo_url} alt={team2.name||""}/>:<i/>}</span>
          </span>
        : <>
            <span className="cfbn-leading-v53"><span className="cfbn-hash-v53">#</span></span>
            <span className="cfbn-copy-v53"><strong>{channelLabel}</strong></span>
          </>}
      {status&&<span className="cfbn-status-v53">{status}</span>}
    </button>;
  };
  const renderCategory=(category)=>{const categoryChannels=channels.filter((channel)=>String(channel.category_id)===String(category.id));if(!categoryChannels.length)return null;return <React.Fragment key={category.id}><div className="cfbn-category-v53"><span>{category.name.toUpperCase()}</span><b>{categoryChannels.length}</b></div>{categoryChannels.map(renderChannelButton)}</React.Fragment>;};
  ...
  {mode==="channels"&&selectedChannelRow?.slug==="streams"&&<button onClick={()=>setActiveTab?.("redZone")}>Open RedZone</button>}
  {mode==="channels"&&canCreateChannels&&!selectedChannelRow?.is_auto_matchup&&<button onClick={()=>openChannelManager(selectedChannelRow)}>Manage Channel</button>}
</div></header>{mode==="channels"&&<div className="network-channel-utilities">
  <div className="network-search-wrap">
    <label><span>⌕</span><input value={socialMessageSearch} onFocus={()=>setNetworkSearchOpen(true)} onChange={(event)=>setSocialMessageSearch(event.target.value)} placeholder={`Search #${cleanNetworkChannelName(selectedChannelRow?.name)||"channel"}`}/></label>
    <button className="network-search-filter-toggle" onClick={()=>setNetworkSearchOpen((value)=>!value)}>Filters</button>
    {networkSearchOpen&&<div className="network-search-popover">
      <header><b>Search Filters</b><button onClick={()=>setNetworkSearchOpen(false)}>×</button></header>
      <label><span>From a specific user</span><input value={networkSearchAuthor} onChange={(event)=>setNetworkSearchAuthor(event.target.value)} placeholder="Coach username"/></label>
      <label><span>Includes a specific type</span><select value={networkSearchHas} onChange={(event)=>setNetworkSearchHas(event.target.value)}><option value="any">Any message</option><option value="link">Has a link</option><option value="image">Has an image or GIF</option></select></label>
      <button className={pinnedOnly?"active":""} onClick={()=>setPinnedOnly((value)=>!value)}>📌 {pinnedOnly?"Pinned only":"Include pinned only"}</button>
      <footer><button onClick={()=>{setSocialMessageSearch("");setNetworkSearchAuthor("");setNetworkSearchHas("any");setPinnedOnly(false);}}>Clear filters</button><button onClick={()=>setNetworkSearchOpen(false)}>Done</button></footer>
    </div>}
  </div>
...
function NewsroomPlatform({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],currentYear,setActiveTab,setError}) {
  ...
  async function loadNewsroom(){
    const [articleRes,jobRes,commentRes,reactionRes,emojiRes]=await Promise.all([
      supabase.from("league_news_articles").select("*").order("published_at",{ascending:false}).order("generated_at",{ascending:false}).limit(120),
      isCommissioner?supabase.from("league_news_jobs").select("*").order("created_at",{ascending:false}).limit(40):Promise.resolve({data:[]}),
      supabase.from("league_news_comments").select("*").is("deleted_at",null).order("created_at",{ascending:true}).limit(500),
      supabase.from("league_news_reactions").select("*"),
      supabase.from("league_custom_emojis").select("*").eq("is_active",true).order("name"),
    ]);
    if(articleRes.error){setError?.(`Newsroom could not load: ${articleRes.error.message}`);return;}
    setArticles(articleRes.data||[]);setJobs(jobRes.data||[]);setComments(commentRes.data||[]);setReactions(reactionRes.data||[]);setCustomEmojis(emojiRes.data||[]);
    const first=(articleRes.data||[]).find((row)=>row.status==="published");setExpanded((value)=>value||first?.id||null);
  }
  useEffect(()=>{loadNewsroom();const channel=supabase.channel?.("newsroom-platform-v35").on("postgres_changes",{event:"*",schema:"public",table:"league_news_articles"},loadNewsroom).on("postgres_changes",{event:"*",schema:"public",table:"league_news_comments"},loadNewsroom).on("postgres_changes",{event:"*",schema:"public",table:"league_news_reactions"},loadNewsroom).subscribe();return()=>{if(channel)supabase.removeChannel?.(channel);};},[discordSession?.user?.id,isCommissioner]);

  async function generate(){if(!isCommissioner||busy)return;setBusy(true);const queued=await supabase.rpc("queue_league_news_digest");if(queued.error){setBusy(false);setError?.(`Newsroom job not queued: ${queued.error.message}`);return;}const generated=await supabase.functions.invoke("generate-league-news",{body:{source:"commissioner"}});setBusy(false);await loadNewsroom();setError?.(generated.error?"The digest is queued and the automatic worker will retry it shortly.":`Newsroom created ${generated.data?.articles||0} active-user story draft${generated.data?.articles===1?"":"s"}.`);}
  async function saveDraft(article,values){setBusy(true);const {error}=await supabase.rpc("update_league_news_draft",{p_article_id:article.id,p_headline:values.headline,p_dek:values.dek,p_body:values.body});setBusy(false);if(error){setError?.(`Draft not saved: ${error.message}`);return;}setEditing(null);await loadNewsroom();setError?.("Newsroom draft saved.");}
  async function review(article,action){if(!window.confirm(action==="publish"?"Publish this story to the Newsroom and notify league members?":"Reject this draft?"))return;setBusy(true);const {error}=await supabase.rpc("review_league_news_article",{p_article_id:article.id,p_action:action});setBusy(false);if(error){setError?.(`Newsroom review failed: ${error.message}`);return;}await loadNewsroom();setError?.(action==="publish"?"Story published to the Newsroom. Push delivery is queued.":"Draft rejected.");}
  async function postComment(articleId){const body=String(commentDrafts[articleId]||"").trim();if(!body||busy)return;setBusy(true);const {error}=await supabase.rpc("post_league_news_comment",{p_article_id:articleId,p_body:body,p_reply_to_id:replying[articleId]?.id||null});setBusy(false);if(error){setError?.(`Comment not posted: ${error.message}`);return;}setCommentDrafts((value)=>({...value,[articleId]:""}));setReplying((value)=>({...value,[articleId]:null}));await loadNewsroom();}
  async function deleteComment(comment){if(!window.confirm("Remove this comment?"))return;const {error}=await supabase.rpc("delete_league_news_comment",{p_comment_id:comment.id});if(error)setError?.(`Comment not removed: ${error.message}`);else await loadNewsroom();}
  async function toggleReaction(articleId,reaction){const {error}=await supabase.rpc("toggle_league_news_reaction",{p_article_id:articleId,p_reaction:reaction});if(error)setError?.(`Reaction not saved: ${error.message}`);else await loadNewsroom();}
  const sorted=[...articles].sort((a,b)=>new Date(b.published_at||b.generated_at)-new Date(a.published_at||a.generated_at));
  const published=sorted.filter((row)=>row.status==="published");const drafts=sorted.filter((row)=>row.status==="draft");
  const authorName=(id)=>users.find((user)=>String(user.id)===String(id))?.discord_username||"League Member";
  const reactionGroups=(articleId)=>{const rows=reactions.filter((row)=>String(row.article_id)===String(articleId));const map=new Map();rows.forEach((row)=>{const item=map.get(row.reaction)||{reaction:row.reaction,count:0,mine:false};item.count+=1;if(String(row.discord_user_id)===String(linkedDiscordUser?.id))item.mine=true;map.set(row.reaction,item);});return [...map.values()];};
  const pending=jobs.filter((row)=>row.status==="pending"||row.status==="processing").length;
  return <main className="cfb-v2-page newsroom-platform-page">
    <header className="newsroom-platform-hero"><div><span>CFB ELITE NEWSWIRE</span><h1>The Newsroom</h1><p>Verified stories about the coaches and user-controlled programs shaping the dynasty.</p></div><div className="newsroom-platform-score"><b>{published.length}</b><span>PUBLISHED STORIES</span><small>Commissioner reviewed • Active users only</small></div>{isCommissioner&&<button disabled={busy} onClick={generate}>{busy?"Building the wire…":"Generate News Digest"}</button>}</header>
    {isCommissioner&&<section className="newsroom-platform-desk"><div><span>COMMISSIONER DESK</span><strong>{pending?`${pending} automation job${pending===1?"":"s"} queued`:drafts.length?`${drafts.length} draft${drafts.length===1?"":"s"} ready for review`:"The desk is clear"}</strong></div><small>CPU-only games are blocked. Every story must feature an active league member before it can reach this desk.</small></section>}
    {isCommissioner&&drafts.length>0&&<section className="league-newsroom-section newsroom-review"><header><span>REVIEW QUEUE</span><h3>Unpublished drafts</h3></header><div className="league-newsroom-grid">{drafts.map((article)=>{const active=editing?.id===article.id;return <article key={article.id} className="newsroom-story draft"><div className="newsroom-story-meta"><b>{article.category}</b><span>{article.season_year} • {article.week_label}</span><em>AI DRAFT</em></div>{active?<><input value={editing.headline} onChange={(event)=>setEditing({...editing,headline:event.target.value})}/><input value={editing.dek} onChange={(event)=>setEditing({...editing,dek:event.target.value})}/><textarea value={editing.body} onChange={(event)=>setEditing({...editing,body:event.target.value})}/></>:<><h4>{article.headline}</h4><strong>{article.dek}</strong><p>{article.body}</p></>}<footer>{active?<><button disabled={busy} onClick={()=>saveDraft(article,editing)}>Save Draft</button><button className="secondary" onClick={()=>setEditing(null)}>Cancel</button></>:<button onClick={()=>setEditing({id:article.id,headline:article.headline,dek:article.dek||"",body:article.body})}>Edit</button>}<button disabled={busy} className="publish" onClick={()=>review(article,"publish")}>Publish</button><button disabled={busy} className="reject" onClick={()=>review(article,"reject")}>Reject</button></footer></article>;})}</div></section>}
    <section className="newsroom-platform-feed"><header><div><span>LATEST COVERAGE</span><h2>Inside CFB Elite 27</h2></div><button onClick={()=>setActiveTab?.("dashboard")}>Back to Home</button></header>{published.length?<div className="newsroom-platform-list">{published.map((article,index)=>{const open=String(expanded)===String(article.id);const articleComments=comments.filter((row)=>String(row.article_id)===String(article.id));const groups=reactionGroups(article.id);return <article key={article.id} className={`newsroom-platform-story ${open?"open":""}`}><button className="newsroom-platform-story-head" onClick={()=>setExpanded(open?null:article.id)}><div className="newsroom-story-meta"><b>{article.category}</b><span>{article.season_year} • {article.week_label}</span><em>{index===0?"LATEST":"VERIFIED"}</em></div><h3>{article.headline}</h3><strong>{article.dek}</strong><small>{new Date(article.published_at||article.generated_at).toLocaleString()} • {articleComments.length} comment{articleComments.length===1?"":"s"}</small><i>{open?"−":"＋"}</i></button>{open&&<div className="newsroom-platform-story-body"><p>{article.body}</p><div className="newsroom-article-reactions">{groups.map((item)=><button key={item.reaction} className={item.mine?"mine":""} onClick={()=>toggleReaction(article.id,item.reaction)}><NetworkEmojiToken value={item.reaction} customEmojis={customEmojis}/> <b>{item.count}</b></button>)}<span>{NETWORK_EMOJIS.slice(0,6).map((emoji)=><button key={emoji} onClick={()=>toggleReaction(article.id,emoji)}>{emoji}</button>)}{customEmojis.slice(0,4).map((emoji)=><button key={emoji.id} onClick={()=>toggleReaction(article.id,`:${emoji.name}:`)}><img src={emoji.image_url} alt={emoji.name}/></button>)}</span></div><section className="newsroom-comments"><header><b>League Conversation</b><span>{articleComments.length} comment{articleComments.length===1?"":"s"}</span></header>{articleComments.map((comment)=>{const parent=articleComments.find((row)=>String(row.id)===String(comment.reply_to_id));const mine=String(comment.author_discord_user_id)===String(linkedDiscordUser?.id);return <div key={comment.id} className="newsroom-comment">{parent&&<small>↳ Reply to {authorName(parent.author_discord_user_id)}: {parent.body}</small>}<div><NetworkIdentity userId={comment.author_discord_user_id} users={users} teams={teams} assignments={assignments} currentYear={currentYear} compact/><time>{new Date(comment.created_at).toLocaleString()}</time></div><p>{comment.body}</p><footer><button onClick={()=>setReplying((value)=>({...value,[article.id]:comment}))}>Reply</button>{(mine||isCommissioner)&&<button onClick={()=>deleteComment(comment)}>Remove</button>}</footer></div>;})}<div className="newsroom-comment-composer">{replying[article.id]&&<div><span>Replying to {authorName(replying[article.id].author_discord_user_id)}</span><button onClick={()=>setReplying((value)=>({...value,[article.id]:null}))}>×</button></div>}<textarea value={commentDrafts[article.id]||""} onChange={(event)=>setCommentDrafts((value)=>({...value,[article.id]:event.target.value}))} placeholder="Join the conversation…"/><button disabled={busy||!String(commentDrafts[article.id]||"").trim()} onClick={()=>postComment(article.id)}>Post Comment</button></div></section></div>}</article>;})}</div>:<div className="league-newsroom-empty"><b>The wire is warming up.</b><span>The first commissioner-approved active-user story will appear here.</span></div>}</section>
  </main>;
}
...
function normalizeStreamProfile(platformValue,channelKeyValue,channelUrlValue) {
  const platform=String(platformValue||"").toLowerCase();
  let channelKey=String(channelKeyValue||"").trim();
  let channelUrl=String(channelUrlValue||"").trim();
  const withProtocol=(value)=>/^https?:\/\//i.test(value)?value:`https://${value.replace(/^\/+/,"")}`;
  if(platform==="youtube"){
    const source=channelUrl||channelKey;
    if(/^https?:\/\//i.test(source))channelUrl=source;
    else if(/^(?:www\.)?youtube\.com\//i.test(source))channelUrl=withProtocol(source);
    else if(/^UC[\w-]{20,}$/i.test(source))channelUrl=`https://www.youtube.com/channel/${source}`;
  ...
  }else if(channelUrl&&!/^https?:\/\//i.test(channelUrl))channelUrl=withProtocol(channelUrl);
...
function RedZoneCenter({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],currentYear,currentWeek,weeklyMatchups=[],sportsbook={},setActiveTab,setError}) {
  ...
  async function saveProfile(){if(!form.channel_key.trim()){setError?.("Enter the platform channel ID, handle or slug.");return;}const normalized=normalizeStreamProfile(form.platform,form.channel_key,form.channel_url);setBusy(true);const payload={discord_user_id:String(linkedDiscordUser.id),platform:form.platform,channel_key:normalized.channelKey,channel_url:normalized.channelUrl,display_name:form.display_name.trim()||linkedDiscordUser.discord_username,enabled:true,updated_at:new Date().toISOString()};const{error}=await supabase.from("stream_profiles").upsert(payload,{onConflict:"discord_user_id,platform"});setBusy(false);if(error)setError?.(`Stream profile not saved: ${error.message}`);else{setEditing(false);setForm({...form,channel_key:"",channel_url:"",display_name:""});await loadStreams();}}
  async function removeProfile(profile){if(!window.confirm(`Remove your ${String(profile.platform).toUpperCase()} stream from RedZone?`))return;setBusy(true);const{error}=await supabase.from("stream_profiles").delete().eq("id",profile.id).eq("discord_user_id",String(linkedDiscordUser.id));setBusy(false);if(error){setError?.(`Stream not removed: ${error.message}`);return;}setSelected((ids)=>ids.filter((id)=>String(id)!==String(profile.id)));setError?.(`${String(profile.platform).toUpperCase()} stream removed from RedZone.`);await loadStreams();}
...
function EliteBooks({sportsbook={},teams=[],users=[],assignments=[],results=[],weeklyMatchups=[],conferenceAssets=[],currentYear,currentWeek,advanceAt,discordSession,linkedDiscordUser,busy,signInWithDiscord,signOutDiscord,submitPick,submitFuture,setActiveTab}) {
  ...
  function futureLeaders(marketId,options){
    const optionMap=new Map(options.map((option)=>[String(option.id),option]));
    return (sportsbook.futureConsensus||[]).filter((row)=>String(row.market_id)===String(marketId)).sort((a,b)=>Number(b.selection_votes)-Number(a.selection_votes)||String(a.option_id).localeCompare(String(b.option_id))).slice(0,3).map((row)=>({...row,option:optionMap.get(String(row.option_id))})).filter((row)=>row.option);
  }
  ...
  function consensusPercentage(line,type,selection){
...
<div className="elite-books-hero"><div><span>CFBELITE 27 PRESENTS</span><h1>ELITE <i>BOOKS</i></h1><p>Beat the line. Build your card. Own the season.</p><div className="elite-books-rule-pills">{newTicketRules?<><b>✓ One ML or spread pick</b><b>✓ Favorite spreads earn cover bonuses</b><b>✓ Plus one over/under</b><b>✓ Editable until manual lock</b></>:<><b>✓ Week 2 legacy board</b><b>✓ Moneyline and spread remain separate</b><b>✓ New format begins Week 3</b></>}</div></div><div className="elite-auth-card">{discordSession?<><span>BETTING AS</span><strong>{linkedDiscordUser?.discord_username||discordSession.user?.user_metadata?.user_name||"Discord Member"}</strong><small>Scores stay tied to your Discord identity.</small><button type="button" onClick={signOutDiscord}>Sign out</button></>:<><span>YOUR CARD STARTS HERE</span><strong>Sign in with Discord</strong><small>Your picks, points, futures and badges follow your account.</small><button type="button" onClick={signInWithDiscord}>Continue with Discord</button></>}</div></div>
    ...
    <EliteTeamBettingRecords lines={sportsbook.lines||[]} teams={teams}/>
    <section className="elite-futures"><div className="elite-section-head"><div><span>SEASON FUTURES</span><h2>Call Your Shot</h2></div><small>Selections save instantly to your Discord profile • longer odds pay more bonus points</small></div>{markets.length?<div className="elite-futures-grid">{markets.map((market)=>{const options=(sportsbook.options||[]).filter((option)=>String(option.market_id)===String(market.id)).sort((a,b)=>Number(a.american_odds)-Number(b.american_odds)||String(a.selection_label).localeCompare(String(b.selection_label))||String(a.id).localeCompare(String(b.id))); const existing=(sportsbook.futurePicks||[]).find((pick)=>String(pick.discord_user_id)===myId&&String(pick.market_id)===String(market.id)); const voteLeaders=futureLeaders(market.id,options); return <article className="elite-future-card" key={market.id}><header><div>{market.market_type==="conference_champion"&&<ConferenceLogoMark conference={market.conference_name} conferenceAssets={conferenceAssets} size={38}/>}<span><small>{market.conference_name||"CFBELITE 27"}</small><strong>{market.title||marketLabel[market.market_type]}</strong></span></div><b>{market.status.toUpperCase()}</b></header><FutureVoteLeaders rows={voteLeaders} teamFor={teamFor} teamForCoach={teamForCoach}/><div className="elite-future-options">{options.map((option)=>{const team=option.team_id?teamFor(option.team_id):option.discord_user_id?teamForCoach(option.discord_user_id):null; const selected=String(existing?.option_id)===String(option.id); return <button className={selected?"selected":""} disabled={busy||market.status!=="open"||!discordSession} key={option.id} onClick={()=>submitFuture(option.id)}>{team?<TeamLogoMark team={team} size={38}/>:<span className="elite-coach-avatar">{option.selection_label.slice(0,1).toUpperCase()}</span>}<span><strong>{option.selection_label}</strong><small>{formatAmericanOdds(option.american_odds)}</small></span><em>{option.bonus_points} pts</em></button>})}</div></article>})}</div>:<div className="elite-empty"><b>FUTURES OPENING SOON</b><span>National Champion, Conference Champions and Heath Hurley COTY begin in Season 1. Most Improved Team automatically joins in Season 2.</span></div>}</section>
  </main>;
}

function FutureVoteLeaders({rows=[],teamFor,teamForCoach}){
  return <div className="elite-future-vote-leaders"><span>TOP 3 LEAGUE PICKS</span>{rows.length?<div>{rows.map((row,index)=>{const option=row.option;const team=option.team_id?teamFor(option.team_id):option.discord_user_id?teamForCoach(option.discord_user_id):null;return <article key={row.option_id}><b>#{index+1}</b>{team?<TeamLogoMark team={team} size={28}/>:<span className="elite-coach-avatar">{String(option.selection_label||"?").slice(0,1)}</span>}<strong>{option.selection_label}</strong><em>{Number(row.selection_percentage||0).toFixed(0)}%</em></article>})}</div>:<small>No league selections yet.</small>}</div>;
}

function eliteTeamBettingRows(lines=[],teams=[]){
  const rows=new Map();
  const rowFor=(teamId)=>{const key=String(teamId);if(!rows.has(key))rows.set(key,{team:teams.find((team)=>String(team.id)===key),teamId:key,atsWins:0,atsLosses:0,atsPushes:0,coverMargin:0,over:0,under:0,totalPushes:0,games:0});return rows.get(key);};
  lines.filter((line)=>line.settled_at&&!line.voided_at&&line.team_1_score!==null&&line.team_2_score!==null).forEach((line)=>{
    const score1=Number(line.team_1_score);const score2=Number(line.team_2_score);const total=score1+score2;const totalLine=Number(line.total_line||0);
    [[line.team_1_id,score1,score2,Number(line.team_1_spread||0)],[line.team_2_id,score2,score1,Number(line.team_2_spread||0)]].forEach(([teamId,pf,pa,spread])=>{
      const row=rowFor(teamId);const margin=pf+spread-pa;row.games++;row.coverMargin+=margin;if(margin>0)row.atsWins++;else if(margin<0)row.atsLosses++;else row.atsPushes++;if(total>totalLine)row.over++;else if(total<totalLine)row.under++;else row.totalPushes++;
    });
  });
  return [...rows.values()].map((row)=>({...row,coverPct:(row.atsWins+row.atsLosses)?row.atsWins/(row.atsWins+row.atsLosses)*100:0,avgCoverMargin:row.games?row.coverMargin/row.games:0})).sort((a,b)=>b.coverPct-a.coverPct||b.atsWins-a.atsWins||String(a.team?.name||"").localeCompare(String(b.team?.name||"")));
}

function EliteTeamBettingRecords({lines=[],teams=[]}){
  const rows=eliteTeamBettingRows(lines,teams);
  return <section className="elite-team-betting"><div className="elite-section-head"><div><span>BETTING INTELLIGENCE</span><h2>Against the Spread</h2></div><small>Every settled Elite Books line • pushes excluded from cover percentage</small></div>{rows.length?<div className="elite-team-betting-table"><div className="head"><span>#</span><span>Team</span><span>ATS</span><span>Cover %</span><span>Avg Cover Margin</span><span>O/U/P</span></div>{rows.map((row,index)=><div key={row.teamId}><b>#{index+1}</b><span><TeamLogoMark team={row.team} size={30}/><strong>{row.team?.name||"Team"}</strong></span><span>{row.atsWins}-{row.atsLosses}-{row.atsPushes}</span><strong>{row.coverPct.toFixed(1)}%</strong><span>{row.avgCoverMargin>0?"+":""}{row.avgCoverMargin.toFixed(1)}</span><span>{row.over}-{row.under}-{row.totalPushes}</span></div>)}</div>:<div className="elite-empty"><b>ATS TRACKING BEGINS AFTER FINALS</b><span>Team cover records populate automatically as Elite Books matchups settle.</span></div>}</section>;
}
...
function spreadPointPreview(spread){const n=Number(spread||0);if(n>=0)return 1;const cover=Math.abs(n);return cover<7?2:cover<14?3:cover<21?4:5;}
...
function CenterCollectionV38({eyebrow,title,description,groups=[],setActiveTab}) {
  return <main className="cfb-v2-page cfb-center-v38">
    <section className="cfb-center-hero-v38">
      <div><span>{eyebrow}</span><h1>{title}</h1><p>{description}</p></div>
      <button type="button" onClick={()=>setActiveTab?.("dashboard")}>Back to Home</button>
    </section>
    <div className="cfb-center-groups-v38">
      {groups.map((group)=><section key={group.title} className="cfb-center-group-v38">
        <header><span>{group.kicker}</span><h2>{group.title}</h2><p>{group.description}</p></header>
        <div>{group.items.map((item)=><button type="button" key={item.key} onClick={()=>setActiveTab?.(item.key)}>
...
function RankingsCenterV38({setActiveTab}) {
  const groups=[
    {kicker:"CURRENT SEASON",title:"Live Rankings",description:"The rankings that move as results are recorded.",items:[
      {key:"automaticRankings",title:"Automatic Rankings",description:"Rating, record, SOR and scoring profile"},
      {key:"gameTop25",title:"Dynasty Top 25",description:"Captured in-game national poll"},
      {key:"conferencePower",title:"Conference Power",description:"League-wide conference strength"},
    ]},
    {kicker:"PROGRAM STRENGTH",title:"Coach & Program Ratings",description:"Long-term performance beyond a single week.",items:[
      {key:"eloRankings",title:"User ELO",description:"Head-to-head coach skill ratings"},
      {key:"powerIndex",title:"All-Time Coach Rankings",description:"Career coaching performance"},
      {key:"recruitingRankings",title:"Recruiting Rankings",description:"Final class rankings by season"},
    ]},
    {kicker:"HISTORY",title:"Season Archives",description:"Return to any completed season's final order.",items:[
      {key:"rankingHistory",title:"Ranking History",description:"Final automatic rankings by year"},
    ]},
  ];
  return <CenterCollectionV38 eyebrow="COMPETITIVE INTELLIGENCE" title="Rankings Center" description="One organized home for live rankings, coach ratings, conference strength and historical finishes." groups={groups} setActiveTab={setActiveTab}/>;
}

function LeagueArchiveCenterV38({setActiveTab}) {
  const groups=[
    {kicker:"DYNASTY RECORD",title:"League Archive",description:"The verified year-by-year history of CFBElite.",items:[
      {key:"dynastyData",title:"Season Data Archive",description:"Captured polls, schedules and season data"},
      {key:"dynastyTimeline",title:"Dynasty Timeline",description:"The chronology of defining league events"},
    ...
    {kicker:"TROPHY ROOM",title:"Recognition",description:"Championships, award winners and national recognition.",items:[
      ...
      {key:"awards",title:"Awards",description:"Season award winners"},
      {key:"heismans",title:"Heisman Winners",description:"Heisman history"},
      {key:"nationalChampions",title:"National Champions",description:"Championship history"},
    ]},
    {kicker:"LEGENDS",title:"Hall of Fame",description:"The coaches and players who built the dynasty's legacy.",items:[
      {key:"coachHOF",title:"Coach Hall of Fame",description:"Legendary coaching careers"},
      {key:"playerHOF",title:"Player Hall of Fame",description:"Legendary player careers"},
    ]},
  ];
  return <CenterCollectionV38 eyebrow="PERMANENT RECORD" title="League Archive" description="Records, rivalries, trophies and Hall of Fame history—organized without crowding the main menu." groups={groups} setActiveTab={setActiveTab}/>;
}

function TeamsCoachesCenterV38({teams=[],users=[],assignments=[],currentYear,setActiveTab}) {
  const [coachId,setCoachId]=useState("");
  const coaches=users.filter((user)=>user.is_active!==false).map((user)=>{
    const assignment=assignments.find((row)=>String(row.discord_user_id)===String(user.id)&&assignmentActiveForYear(row,currentYear))
      || assignments.find((row)=>String(row.discord_user_id)===String(user.id)&&row.status==="Active");
    const team=teams.find((row)=>String(row.id)===String(assignment?.team_id));
    return {user,team};
  }).filter((row)=>row.team).sort((a,b)=>String(a.team.name).localeCompare(String(b.team.name)));
  return <main className="cfb-v2-page cfb-center-v38">
    <section className="cfb-center-hero-v38"><div><span>LEAGUE DIRECTORY</span><h1>Teams & Coaches</h1><p>Team ratings, captured schedules and coach profiles in one focused directory.</p></div><button type="button" onClick={()=>setActiveTab?.("dashboard")}>Back to Home</button></section>
    <div className="cfb-center-groups-v38 cfb-teams-center-v38">
      <section className="cfb-center-group-v38"><header><span>PROGRAMS</span><h2>Team Directory</h2><p>Browse every program or review the current season schedule archive.</p></header><div>
        <button type="button" onClick={()=>setActiveTab?.("allTeamsRatings")}><span><strong>Teams & Ratings</strong><small>Logos, conferences and team ratings</small></span><b>›</b></button>
        <button type="button" onClick={()=>setActiveTab?.("teamSchedules")}><span><strong>Team Schedules</strong><small>Captured full-season schedules</small></span><b>›</b></button>
      </div></section>
      <section className="cfb-center-group-v38 cfb-coach-finder-v38"><header><span>COACH PROFILES</span><h2>Find a Coach</h2><p>Select a user-controlled program to open its complete coach profile.</p></header><label><span>Active coach</span><select value={coachId} onChange={(event)=>setCoachId(event.target.value)}><option value="">Choose a team or coach…</option>{coaches.map(({user,team})=><option key={user.id} value={user.id}>{team.name} — {user.discord_username}</option>)}</select></label><button type="button" disabled={!coachId} onClick={()=>coachId&&setActiveTab?.(`coach-${coachId}`)}>Open Coach Profile →</button><small>{coaches.length} active coach profile{coaches.length===1?"":"s"}</small></section>
    </div>
  </main>;
}

function AutomaticRankingsPageV38({teams=[],users=[],assignments=[],results=[],currentYear,currentWeek,rankingSnapshots=[],goToTeam,setActiveTab}) {
  ...
  return <main className="cfb-v2-page cfb-rankings-page-v38">
    <section className="cfb-center-hero-v38"><div><span>{currentYear} • {currentWeek}</span><h1>Automatic Rankings</h1><p>Every active user-controlled team ranked by results, opponent strength and scoring performance.</p></div><button type="button" onClick={()=>setActiveTab?.("rankingsCenter")}>Back to Rankings</button></section>
    <section style={v2Panel}><div className="cfb-v2-table-panel-head" style={v2PanelHeader}><div><span style={v2Eyebrow}>LIVE LADDER</span><h2>Complete Rankings</h2></div><span className="cfb-v2-active-count" style={v2CountPill}>{rankings.length} Active Teams</span></div><div style={v2RankingTableScroll}><div className="cfb-v2-ranking-head" style={v2RankingTableHead}><span>Rank</span><span>Team / User</span><span>Move</span><span>Record</span><span>SOR</span><span>Rating</span><span>Avg PA</span><span>Avg PF</span><span>Top 10 Wins</span></div><div style={v2RankingList}>{rankings.map((row)=><button className="cfb-v2-ranking-row" key={row.team.id} style={{...v2RankingRow,borderColor:`${getTeamSecondary(row.team)}44`}} onClick={()=>goToTeam?.(row.team.id)}><b>#{row.rank}</b><span style={v2RankingIdentity}><span style={v2RankingLogo}><TeamLogoMark team={row.team} size={38}/></span><span style={v2RankingTeam}><strong>{row.teamName}</strong><small>{assignedCoachName(row.team.id,null,users,assignments,currentYear)}</small></span></span><RankingMovement currentRank={row.rank} previousRank={previousMap.get(String(row.team.id))}/><span>{row.wins}-{row.losses}</span><span>{row.sor.toFixed(1)}</span><strong>{row.rating.toFixed(1)}</strong><span>{row.avgPa.toFixed(1)}</span><span>{row.avgPf.toFixed(1)}</span><span>{row.top10}</span></button>)}</div></div></section>
  </main>;
}

function HomeLeaguePulseV38({sportsbook={},linkedDiscordUser,currentYear,currentWeek,setActiveTab}) {
  ...
  const board=currentSportsbookBoard(sportsbook,currentYear,currentWeek);
  const lineCount=(sportsbook.lines||[]).filter((line)=>String(line.board_id)===String(board?.id)).length;
  const myStanding=(sportsbook.seasonStandings||[]).find((row)=>String(row.season_year)===String(currentYear)&&String(row.discord_user_id)===String(linkedDiscordUser?.id));
  return <section className="home-pulse-v38"><header><div><span>LEAGUE PULSE</span><h2>Everything happening now</h2></div><small>{currentWeek}</small></header><div>
    <button type="button" onClick={()=>setActiveTab?.("leagueHub")}><span>CFBELITE NETWORK</span><strong>{pulse.unread}</strong><small>unread alert{pulse.unread===1?"":"s"}</small><p>{pulse.announcement?.body||"League chat, scheduling and announcements"}</p></button>
    <button type="button" onClick={()=>setActiveTab?.("eliteBooks")}><span>ELITE BOOKS</span><strong>{myStanding?.total_points||0} PTS</strong><small>{lineCount} game{lineCount===1?"":"s"} on the board</small><p>{board?.status?`${String(board.status).toUpperCase()} weekly board`:"Weekly card will appear after generation"}</p></button>
    <button type="button" className={pulse.live?"live":""} onClick={()=>setActiveTab?.("redZone")}><span>REDZONE</span><strong>{pulse.live}</strong><small>stream{pulse.live===1?"":"s"} live</small><p>{pulse.live?"Watch the league live now":"Stream control room is standing by"}</p></button>
  </div></section>;
}

function HomeUserMatchupV38({teams=[],assignments=[],weeklyMatchups=[],results=[],linkedDiscordUser,currentYear,currentWeek,setActiveTab}) {
  const assignment=assignments.find((row)=>String(row.discord_user_id)===String(linkedDiscordUser?.id)&&assignmentActiveForYear(row,currentYear))
    || assignments.find((row)=>String(row.discord_user_id)===String(linkedDiscordUser?.id)&&row.status==="Active");
  const team=teams.find((row)=>String(row.id)===String(assignment?.team_id));
  if(!team) return null;
  const next=(weeklyMatchups||[]).filter((row)=>String(row.season_year)===String(currentYear)&&(String(row.team_1_id)===String(team.id)||String(row.team_2_id)===String(team.id))&&weekIndex(row.week)>=weekIndex(currentWeek)&&!scheduledResultFor(row,results,currentYear)).sort((a,b)=>weekIndex(a.week)-weekIndex(b.week))[0];
  if(!next) return null;
  const away=next.team_1||teams.find((row)=>String(row.id)===String(next.team_1_id));
  const home=next.team_2||teams.find((row)=>String(row.id)===String(next.team_2_id));
  return <section className="home-matchup-v38"><div><span>{String(next.week)===String(currentWeek)?"YOUR MATCHUP":"NEXT MATCHUP"}</span><h2>{next.week}</h2><small>{next.scheduled_at?new Date(next.scheduled_at).toLocaleString():"Time TBD"}</small></div><div className="home-matchup-teams-v38"><span><TeamLogoMark team={away} size={58}/><strong>{away?.name}</strong></span><b>VS</b><span><TeamLogoMark team={home} size={58}/><strong>{home?.name}</strong></span></div><button type="button" onClick={()=>setActiveTab?.("schedule")}>Open GameCenter →</button></section>;
}

function HomeTopRankingsV38({rankings=[],users=[],assignments=[],currentYear,goToTeam,setActiveTab}) {
  return <section className="home-rankings-v38"><header><div><span>LIVE LADDER</span><h2>Automatic Top 5</h2><p>A quick read on the league. The complete table lives in Rankings Center.</p></div><button type="button" onClick={()=>setActiveTab?.("automaticRankings")}>View Full Rankings →</button></header><div>{rankings.slice(0,5).map((row)=><button type="button" key={row.team.id} onClick={()=>goToTeam?.(row.team.id)}><b>#{row.rank}</b><TeamLogoMark team={row.team} size={38}/><span><strong>{row.teamName}</strong><small>{assignedCoachName(row.team.id,null,users,assignments,currentYear)}</small></span><em>{row.wins}-{row.losses}</em><i>{row.rating.toFixed(1)}</i></button>)}</div></section>;
}
...
function DashboardV2({ teams = [], users = [], assignments = [], results = [], allResults = [], weeklyMatchups = [], conferenceAssets = [], allAmericans = [], awards = [], heismans = [], nationalChampions = [], recruiting = [], teamSeasonStats = [], currentYear, currentWeek, advanceAt, setCurrentYear, setCurrentWeek, saveSettings, goToTeam, setActiveTab, rankingSnapshots = [], adminUnlocked = false, sportsbook = {}, linkedDiscordUser = null }) {
  ...
  return <main className="cfb-v2-page" style={v2Page}>
    ...
    <HomeLeaguePulseV38 sportsbook={sportsbook} linkedDiscordUser={linkedDiscordUser} currentYear={currentYear} currentWeek={currentWeek} setActiveTab={setActiveTab}/>

    <HomeUserMatchupV38 teams={teams} assignments={assignments} weeklyMatchups={weeklyMatchups} results={allResults} linkedDiscordUser={linkedDiscordUser} currentYear={currentYear} currentWeek={currentWeek} setActiveTab={setActiveTab}/>

    <HomeNewsroomRail setActiveTab={setActiveTab}/>

    <HomeTopRankingsV38 rankings={rankings} users={users} assignments={assignments} currentYear={currentYear} goToTeam={goToTeam} setActiveTab={setActiveTab}/>
  </main>;
}
...
function UserManagerV2({users=[],assignments=[],teams=[],linkedDiscordUser,addDiscordUser,renameDiscordUser,setDiscordUserActive,setDiscordUserBan,setDiscordCommissionerStatus}) {
  ...
  return <main className="cfb-v2-page" style={v2Page}>
    ...
    <section style={v2Panel}><div style={v2PanelHeader}><div><span style={v2Eyebrow}>MEMBER DIRECTORY</span><h2>{users.length} Discord Users</h2></div></div><div className="cfb-v2-user-grid" style={v2UserGrid}>{users.map((user)=>{
      ...
      return <article key={user.id} style={{...v2UserCard,borderColor:user.is_banned?"rgba(239,68,68,.75)":user.is_commissioner?"rgba(167,139,250,.7)":`${getTeamSecondary(team)}44`}}><div style={v2UserIdentity}><TeamLogoMark team={team} size={48} plate/><div><b>{user.discord_username}</b><span>{team?.name||"No active team"}</span>{user.is_banned&&<small className="elite-ban-reason">{user.banned_reason||"Access suspended"}</small>}</div><div className="elite-user-statuses"><small style={user.is_active===false?v2InactiveBadge:v2ActiveBadge}>{user.is_banned?"BANNED":user.is_active===false?"INACTIVE":"ACTIVE"}</small>{user.is_commissioner&&<small className="elite-commissioner-badge">COMMISSIONER</small>}</div></div><div style={v2InlineForm}><input style={v2Input} value={draft} onChange={(e)=>setDrafts({...drafts,[user.id]:e.target.value})}/><button style={v2GhostButton} onClick={()=>renameDiscordUser(user.id,draft)}>Save Name</button><button style={user.is_active===false?v2PrimaryButton:v2DangerSoft} disabled={user.is_banned} onClick={()=>setDiscordUserActive(user.id,user.is_active===false)}>{user.is_active===false?"Reactivate":"Deactivate"}</button></div><div className="elite-member-control-row"><button className={`elite-commissioner-toggle ${user.is_commissioner?"revoke":"grant"}`} disabled={!linkedDiscordUser?.is_commissioner||user.is_banned} onClick={()=>setDiscordCommissionerStatus(user.id,!user.is_commissioner)}>{user.is_commissioner?"Revoke Commissioner Status":"Grant Commissioner Status"}</button><button className={`elite-ban-toggle ${user.is_banned?"restore":"ban"}`} disabled={!linkedDiscordUser?.is_commissioner||String(user.id)===String(linkedDiscordUser?.id)} onClick={()=>{if(user.is_banned){if(window.confirm(`Restore app access for ${user.discord_username}?`))setDiscordUserBan?.(user.id,false,"");}else{const reason=window.prompt(`Reason for banning ${user.discord_username}:`,"Removed from the CFB Elite league");if(reason!==null&&window.confirm(`Ban ${user.discord_username} from the entire app?`))setDiscordUserBan?.(user.id,true,reason);}}}>{user.is_banned?"Restore App Access":"Ban From App"}</button></div></article>;
    })}</div></section>
  </main>;
}
...
function AutomaticRankingHistory({rows=[],teams=[],users=[],setActiveTab}){
  ...
  useEffect(()=>{if(!years.includes(Number(selectedYear)))setSelectedYear(years[0]||"");},[rows.length]);
  const visible=rows.filter((row)=>String(row.season_year)===String(selectedYear)).sort((a,b)=>Number(a.final_rank)-Number(b.final_rank));
  const teamFor=(id)=>teams.find((team)=>String(team.id)===String(id));
  const userFor=(id)=>users.find((user)=>String(user.id)===String(id));
  return <main className="cfb-v2-page ranking-history-page"><section className="ranking-history-hero"><div><span>DYNASTY ARCHIVE</span><h1>Final Automatic Rankings</h1><p>Every completed season preserved exactly as the league finished it.</p></div><div><select value={selectedYear} onChange={(event)=>setSelectedYear(event.target.value)}>{years.map((year)=><option key={year} value={year}>{year} Season</option>)}</select><button onClick={()=>setActiveTab?.("dashboard")}>Current Rankings</button></div></section>{visible.length?<section className="ranking-history-table"><div className="head"><span>Rank</span><span>Team / Coach</span><span>Record</span><span>SOR</span><span>Rating</span><span>Avg PA</span><span>Avg PF</span><span>Top 10</span></div>{visible.map((row)=>{const team=teamFor(row.team_id);const user=userFor(row.discord_user_id);return <article key={row.id||`${row.season_year}-${row.team_id}`}><b>#{row.final_rank}</b><span><TeamLogoMark team={team} size={38}/><span><strong>{team?.name||"Team"}</strong><small>{user?.discord_username||"CPU / Unassigned"}</small></span></span><span>{row.wins}-{row.losses}</span><span>{Number(row.sor||0).toFixed(1)}</span><strong>{Number(row.rating||0).toFixed(1)}</strong><span>{Number(row.avg_pa||0).toFixed(1)}</span><span>{Number(row.avg_pf||0).toFixed(1)}</span><span>{row.top_10_wins||0}</span></article>})}</section>:<div className="ranking-history-empty"><b>No completed seasons archived yet.</b><span>The first table is saved automatically when the commissioner advances into the next dynasty year.</span></div>}</main>;
}

function CoachRankingHistoryChart({rows=[]}){
  const sorted=[...rows].sort((a,b)=>Number(a.season_year)-Number(b.season_year));
  if(!sorted.length)return <section className="coach-ranking-chart"><div className="elite-section-head"><div><span>AUTOMATIC RANKINGS</span><h2>Year-by-Year Finish</h2></div></div><div className="ranking-history-empty"><b>No completed seasons yet.</b><span>The chart begins when the first season is archived.</span></div></section>;
  const width=760,height=250,padX=58,padTop=34,padBottom=48,maxRank=Math.max(32,...sorted.map((row)=>Number(row.final_rank)||1));
...
function CoachProfile({ user, users = [], teams, assignments, results, weeklyMatchups = [], currentYear, currentWeek, rankingSnapshots = [], rankingHistory = [], allAmericans, awards, heismans, nationalChampions, recruiting, seasonPlayerStats = [], teamSeasonStats = [], sportsbook = {} }) {
  ...
  const coachPrestigeScore = Math.min(100, Math.round(Number(stats?.rawPrestige||0) / 2.2));
  ...
  const hofCriteria=coachHofCriteria(stats);
  const hofEstimatedSeasons=Math.floor(Number(stats?.games||0)/12);
  const hofAccolades=Number(stats?.heismans||0)+Number(stats?.awards||0)+Number(stats?.allAmericans||0);
  const hofAchievement=Math.max(Number(stats?.nattys||0),Number(stats?.confTitles||0)/4,Number(stats?.top10Wins||0)/18,hofAccolades/55);
  const hofPct=hofCriteria.qualifies?100:Math.min(99,Math.max(0,Math.round(
    Math.min(Number(stats?.rawPrestige||0)/220,1)*35+
    Math.min(Number(stats?.games||0)/100,1)*25+
    Math.min(Number(stats?.games||0)/100,1)*(Number(stats?.winPct||0)>=.7?15:8)+
    Math.min(hofAchievement,1)*25
  )));
  ...
  const capturedSchedule=useCapturedTeamSchedule(currentTeam?.id,currentYear);
  const teamSchedule=weeklyMatchups.filter((row)=>String(row.season_year)===String(currentYear)&&(String(row.team_1_id)===String(currentTeam?.id)||String(row.team_2_id)===String(currentTeam?.id))).sort((a,b)=>weekIndex(a.week)-weekIndex(b.week));
  const matchupHasResult=(matchup)=>currentSeasonResults.some((result)=>String(result.week)===String(matchup.week)&&((String(result.team_1_id)===String(matchup.team_1_id)&&String(result.team_2_id)===String(matchup.team_2_id))||(String(result.team_1_id)===String(matchup.team_2_id)&&String(result.team_2_id)===String(matchup.team_1_id))));
  const nextCaptured=capturedSchedule.find((row)=>row.status!=="final"&&row.status!=="bye"&&row.opponent_id);
  const nextMatchup = nextCaptured||teamSchedule.find((row)=>!matchupHasResult(row));
  const opponentId = nextCaptured?.opponent_id||(nextMatchup ? (String(nextMatchup.team_1_id)===String(currentTeam?.id)?nextMatchup.team_2_id:nextMatchup.team_1_id) : null);
  const nextOpponent = teams.find((team)=>String(team.id)===String(opponentId))||teams.find((team)=>String(team.name).toLowerCase()===String(nextCaptured?.opponent_name||"").toLowerCase());
  ...
  const coachRankingHistory=(rankingHistory||[]).filter((row)=>String(row.discord_user_id)===String(safeUser.id));
  ...
  return (
    <section style={coachPageV43} className="cfb-coach-page cfb-coach-mobile-fix">
      ...
      <section className="cfb-v2-coach-strip" style={v2CoachSeasonStrip}>
        ...
        <div><span>Next Opponent</span><b style={v2NextOpponent}>{nextOpponent?<><TeamLogoMark team={nextOpponent} size={30}/>{getTeamAbbreviation(nextOpponent)}</>:"TBD"}</b><small>{nextCaptured?.week_label||nextMatchup?.week||currentWeek}</small></div>
      </section>
      <CapturedTeamScheduleCard rows={capturedSchedule} teams={teams} title={`${currentYear} Team Schedule`} compact/>
      <CoachRankingHistoryChart rows={coachRankingHistory}/>
<section style={coachTrophyCaseV46}>
      ...
      <section style={coachProfileGridV43}>
        <div style={coachPanelV37}>
          ...
          <div style={miniRow}>HOF Readiness: <b>{hofPct}%</b></div>
          <div style={miniRow}>Longevity Gate: <b>{hofEstimatedSeasons} full seasons • {stats?.games||0}/100 games</b></div>
          <div style={miniRow}>Performance Gate: <b>{((stats?.winPct||0)*100).toFixed(1)}% / 70.0%</b></div>
          <div style={miniRow}>Résumé Gate: <b>{Math.round(stats?.rawPrestige||0)}/220</b></div>
...
function coachHofCriteria(row) {
  ...
  const reasons = [];
  if (row.games >= 100) reasons.push(`${estimatedSeasons}+ Season Résumé`);
...
const DATA_IMPORT_TYPES = [
  ["user_game","Completed user game"],["top_25","Top 25 poll"],["conference_standings","Conference standings"],
  ["playoff_picture","CFP / playoff picture"],["team_stats","Team statistics"],["player_stats","Player statistics"],
  ["stat_leaders","Stat leaders"],["recruiting_rankings","Recruiting rankings"],["transfer_portal","Transfer portal"],
  ["rosters","Rosters / roster changes"],["depth_charts","Depth charts and ratings"],["team_schedules","Complete team schedules"],["award_watch","Award watch lists"],
  ["award_results","Award results / All-Americans"],["season_summary","Final season summary"],
];
const DATA_CYCLES = [["preseason","Beginning of Year"],["weekly","Weekly"],["postseason","Postseason"],["offseason","Offseason"]];
function dataLabel(value, rows=DATA_IMPORT_TYPES){ return rows.find(([key])=>key===value)?.[1] || String(value||"").replaceAll("_"," "); }
function metricSummary(value){
  if(!value) return "No metrics";
  if(Array.isArray(value)) return value.slice(0,5).map((item)=>`${item.name}: ${item.value??"—"}${item.unit?` ${item.unit}`:""}`).join(" • ") || "No metrics";
  if(typeof value==="object") return Object.entries(value).slice(0,5).map(([key,item])=>`${key.replaceAll("_"," ")}: ${typeof item==="object"?JSON.stringify(item):item}`).join(" • ");
  return String(value);
}

function useCapturedTeamSchedule(teamId,seasonYear){
  const [rows,setRows]=useState([]);
  useEffect(()=>{
    let active=true;
    if(!teamId||!seasonYear){setRows([]);return()=>{active=false;};}
    supabase.from("dynasty_team_schedules").select("*").eq("season_year",Number(seasonYear)).eq("team_id",String(teamId)).order("week_index").order("week_label").then(({data,error})=>{if(active&&!error)setRows(data||[]);});
    return()=>{active=false;};
  },[teamId,seasonYear]);
  return rows;
}

function CapturedTeamScheduleCard({rows=[],teams=[],title="Captured Schedule",compact=false}){
  const teamFor=(id,name)=>teams.find((team)=>String(team.id)===String(id))||teams.find((team)=>String(team.name).toLowerCase()===String(name||"").toLowerCase());
  return <section className={`captured-schedule-card ${compact?"compact":""}`}><header><div><span>SCREENSHOT VERIFIED</span><h3>{title}</h3></div><b>{rows.length} GAMES</b></header><div className="captured-schedule-list">{rows.length?rows.map((row)=>{const opponent=teamFor(row.opponent_id,row.opponent_name);return <article key={row.id||`${row.week_label}-${row.opponent_name}`} className={row.status==="final"?"final":"upcoming"}><div className="captured-schedule-week"><small>{row.week_label}</small><strong>{String(row.home_away||"").toUpperCase()}</strong></div><TeamLogoMark team={opponent} size={compact?34:44}/><div className="captured-schedule-opponent"><strong>{row.opponent_name}</strong><small>{row.date_text||((row.status||"scheduled").replaceAll("_"," "))}</small></div><div className="captured-schedule-result">{row.status==="final"?<><b className={row.result==="W"?"win":row.result==="L"?"loss":""}>{row.result||"FINAL"}</b><strong>{row.team_score}–{row.opponent_score}</strong></>:<b>UPCOMING</b>}</div></article>}):<div className="data-empty">No approved schedule screenshot has been published for this team yet.</div>}</div></section>;
}

function CapturedTop25Page({currentYear,teams=[],setActiveTab}){
  const [rows,setRows]=useState([]);const [year,setYear]=useState(String(currentYear));const [week,setWeek]=useState("latest");const [poll,setPoll]=useState("all");const [message,setMessage]=useState("");
  async function load(){const {data,error}=await supabase.from("dynasty_top_25").select("*").order("season_year",{ascending:false}).order("approved_at",{ascending:false}).order("rank");if(error){setMessage(`Top 25 load failed: ${error.message}`);return;}setRows(data||[]);setMessage("");}
  useEffect(()=>{load();},[]);
  const years=[...new Set([String(currentYear),...rows.map((row)=>String(row.season_year))])].sort((a,b)=>Number(b)-Number(a));
  const yearRows=rows.filter((row)=>String(row.season_year)===year);
  const snapshots=[...new Map(yearRows.map((row)=>[`${row.source_import_id}|${row.poll_name}|${row.week_label}`,{key:`${row.source_import_id}|${row.poll_name}|${row.week_label}`,source_import_id:row.source_import_id,poll_name:row.poll_name,week_label:row.week_label,approved_at:row.approved_at}])).values()].sort((a,b)=>new Date(b.approved_at)-new Date(a.approved_at));
  const selectedSnapshot=week==="latest"?snapshots.find((item)=>poll==="all"||item.poll_name===poll):snapshots.find((item)=>item.key===week);
  const visible=selectedSnapshot?yearRows.filter((row)=>String(row.source_import_id)===String(selectedSnapshot.source_import_id)&&row.poll_name===selectedSnapshot.poll_name).sort((a,b)=>a.rank-b.rank):[];
  ...
  const polls=[...new Set(yearRows.map((row)=>row.poll_name))].sort();
  return <main className="captured-data-page cfb-v2-page"><section className="captured-data-hero top25"><div><span>IN-GAME NATIONAL POLL</span><h1>Dynasty Top 25</h1><p>Every commissioner-approved poll screenshot, preserved independently from CFBElite Automatic Rankings.</p></div><div><b>{visible.length}</b><span>RANKED TEAMS</span></div></section><section className="captured-data-controls"><label>Season<select value={year} onChange={(event)=>{setYear(event.target.value);setWeek("latest");}}>{years.map((item)=><option key={item}>{item}</option>)}</select></label><label>Poll<select value={poll} onChange={(event)=>{setPoll(event.target.value);setWeek("latest");}}><option value="all">Latest poll</option>{polls.map((item)=><option key={item}>{item}</option>)}</select></label><label>Snapshot<select value={week} onChange={(event)=>setWeek(event.target.value)}><option value="latest">Most recent</option>{snapshots.filter((item)=>poll==="all"||item.poll_name===poll).map((item)=><option key={item.key} value={item.key}>{item.poll_name} • {item.week_label||"Season"}</option>)}</select></label><button onClick={load}>Refresh</button><button onClick={()=>setActiveTab?.("dataIntake")}>Upload Poll</button></section>{message&&<div className="data-local-message">{message}</div>}<section className="captured-top25-board"><header><div><span>{selectedSnapshot?.poll_name||"TOP 25"}</span><h2>{selectedSnapshot?.week_label||`${year} Season`}</h2></div><small>{selectedSnapshot?.approved_at?`Verified ${new Date(selectedSnapshot.approved_at).toLocaleDateString()}`:"Awaiting first approved poll"}</small></header><div>{visible.length?visible.map((row)=>{const team=teamFor(row);return <article key={row.id} className={row.is_user_controlled?"user-team":""}><b>#{row.rank}</b><TeamLogoMark team={team} size={48}/><div><strong>{row.team_name}</strong><small>{row.team_record||"Record not shown"}{row.is_user_controlled?" • CFBElite user team":""}</small></div><span>{row.previous_rank?`Prev #${row.previous_rank}`:"—"}</span></article>}):<div className="data-empty">No approved Top 25 screenshot is available for this selection.</div>}</div></section></main>;
}

function CapturedSchedulesPage({currentYear,teams=[],assignments=[],setActiveTab}){
  const [rows,setRows]=useState([]);const [year,setYear]=useState(String(currentYear));const [teamId,setTeamId]=useState("all");const [message,setMessage]=useState("");
  async function load(){const {data,error}=await supabase.from("dynasty_team_schedules").select("*").order("season_year",{ascending:false}).order("week_index").order("week_label");if(error){setMessage(`Schedule load failed: ${error.message}`);return;}setRows(data||[]);setMessage("");}
  useEffect(()=>{load();},[]);
  const activeIds=new Set(assignments.filter((row)=>row.status==="Active").map((row)=>String(row.team_id)));
  const teamChoices=teams.filter((team)=>activeIds.has(String(team.id))).sort((a,b)=>String(a.name).localeCompare(String(b.name)));
  const years=[...new Set([String(currentYear),...rows.map((row)=>String(row.season_year))])].sort((a,b)=>Number(b)-Number(a));
...
function LeagueDataIntakeCenter({discordSession,linkedDiscordUser,currentYear,currentWeek,adminUnlocked,setError}) {
  ...
  const [importType,setImportType]=useState("user_game");
  const [seasonYear,setSeasonYear]=useState(String(currentYear));
  const [weekLabel,setWeekLabel]=useState(currentWeek||"Week 1");
  ...
  const selected=imports.find((row)=>row.id===selectedId)||null;
  const isCommissioner=adminUnlocked||linkedDiscordUser?.is_commissioner===true;

  async function loadIntake(){
    if(!discordSession?.user) return;
    await supabase.rpc("ensure_league_upload_checklist",{p_season_year:Number(seasonYear||currentYear),p_week_label:weekLabel||currentWeek||""});
    const [importRes,checkRes]=await Promise.all([
      supabase.from("league_data_imports").select("*, league_data_import_files(*)").order("created_at",{ascending:false}).limit(100),
      supabase.from("league_upload_checklist").select("*").eq("season_year",Number(seasonYear||currentYear)).order("cycle").order("sort_order"),
    ]);
    if(importRes.error){setError(`Data Intake load failed: ${importRes.error.message}`);return;}
    if(checkRes.error){setError(`Checklist load failed: ${checkRes.error.message}`);return;}
    setImports(importRes.data||[]);setChecklist(checkRes.data||[]);
  }
  useEffect(()=>{loadIntake();},[discordSession?.user?.id,seasonYear,weekLabel]);
  useEffect(()=>{if(selected?.extraction)setReviewText(JSON.stringify(selected.corrected_payload||selected.extraction,null,2));},[selectedId,selected?.updated_at]);

  function chooseFiles(event){
    const picked=[...(event.target.files||[])];
    const bad=picked.find((file)=>!["image/png","image/jpeg","image/webp"].includes(file.type)||file.size>8388608);
    if(bad){setLocalMessage(`${bad.name} must be a PNG, JPG or WebP no larger than 8 MB.`);event.target.value="";return;}
    if(picked.length>12){setLocalMessage("Use no more than 12 screenshots in one upload batch.");event.target.value="";return;}
    setFiles(picked);setLocalMessage("");
  }
  async function submitImport(event){
    event.preventDefault();
    if(!discordSession?.user?.id||!linkedDiscordUser?.id){setLocalMessage("Your Discord league identity is not linked.");return;}
    if(!files.length){setLocalMessage("Choose at least one screenshot.");return;}
    if(cycle==="weekly"&&!weekLabel){setLocalMessage("Choose the week represented by these screenshots.");return;}
    setBusy(true);setLocalMessage("Creating the secure upload batch…");
    const importId=crypto.randomUUID();
    try{
      const {error:createError}=await supabase.from("league_data_imports").insert({id:importId,season_year:Number(seasonYear),week_label:cycle==="weekly"?weekLabel:"",cycle,import_type:importType,status:"draft",submitted_by_auth_user_id:discordSession.user.id,submitted_by_discord_user_id:String(linkedDiscordUser.id),notes:notes.trim()});
      if(createError)throw createError;
      const metadata=[];
      for(let index=0;index<files.length;index+=1){
        const file=files[index];const extension=(file.name.split(".").pop()||"png").toLowerCase().replace(/[^a-z0-9]/g,"");
        const path=`${discordSession.user.id}/${importId}/${String(index+1).padStart(2,"0")}.${extension}`;
        const {error:uploadError}=await supabase.storage.from("league-data-intake").upload(path,file,{contentType:file.type,upsert:false});
        if(uploadError)throw uploadError;
        metadata.push({import_id:importId,storage_path:path,original_name:file.name,mime_type:file.type,file_size:file.size,page_order:index});
      }
      const {error:fileError}=await supabase.from("league_data_import_files").insert(metadata);if(fileError)throw fileError;
      const {error:readyError}=await supabase.from("league_data_imports").update({status:"uploaded",updated_at:new Date().toISOString()}).eq("id",importId);if(readyError)throw readyError;
      setLocalMessage("Screenshots secured. AI is extracting the visible data…");
      const {data:functionData,error:functionError}=await supabase.functions.invoke("extract-league-data",{body:{import_id:importId}});
      if(functionError)throw functionError;if(functionData?.ok===false)throw new Error(functionData.error||"Extraction failed");
      setLocalMessage(isCommissioner?"Extraction complete. Review the draft before approving it.":"Extraction complete and waiting for commissioner review.");
      setSelectedId(importId);setFiles([]);setNotes("");setFileKey((value)=>value+1);await loadIntake();
    }catch(error){const message=error.message||String(error);await supabase.from("league_data_imports").update({status:"failed",error_message:message,updated_at:new Date().toISOString()}).eq("id",importId);setLocalMessage(`Upload failed: ${message}`);await loadIntake();}
    finally{setBusy(false);}
  }
  async function retryExtraction(row){
    setBusy(true);setLocalMessage("Running extraction again…");
    const {data,error}=await supabase.functions.invoke("extract-league-data",{body:{import_id:row.id}});
    setLocalMessage(error||data?.ok===false?`Extraction failed: ${error?.message||data?.error}`:"Extraction finished and is ready for review.");
    await loadIntake();setBusy(false);
  }
  async function openFile(file){
    const {data,error}=await supabase.storage.from("league-data-intake").createSignedUrl(file.storage_path,300);
    if(error){setLocalMessage(`Screenshot could not be opened: ${error.message}`);return;}
    window.open(data.signedUrl,"_blank","noopener,noreferrer");
  }
  async function approveImport(){
    setBusy(true);
    try{
      const payload=JSON.parse(reviewText);const {data,error}=await supabase.rpc("approve_league_data_import",{p_import_id:selected.id,p_corrected_payload:payload});if(error)throw error;
      setLocalMessage(`Approved and published • ${data?.game_results_added||0} game result(s) • ${data?.top_25_rows||0} Top 25 row(s) • ${data?.schedule_rows||0} schedule row(s) • ${data?.stats_published||0} stat row(s) • ${data?.recruiting_rows||0} recruiting row(s) • ${data?.recognition_rows||0} recognition row(s) • ${data?.archive_records_added||0} archive record(s).`);await loadIntake();
    }catch(error){setLocalMessage(`Approval failed: ${error.message||error}`);}finally{setBusy(false);}
  }
  async function rejectImport(){
    const reason=window.prompt("Why is this upload being rejected?",selected?.error_message||"");if(reason===null)return;
    setBusy(true);const {error}=await supabase.rpc("reject_league_data_import",{p_import_id:selected.id,p_reason:reason});setLocalMessage(error?`Rejection failed: ${error.message}`:"Upload rejected without changing official records.");await loadIntake();setBusy(false);
  }
  async function setChecklistStatus(row,status){
    const {error}=await supabase.rpc("set_league_upload_checklist_status",{p_item_id:row.id,p_status:status});if(error)setLocalMessage(`Checklist update failed: ${error.message}`);await loadIntake();
  }
  const phases=DATA_CYCLES.map(([key,label])=>({key,label,rows:checklist.filter((row)=>row.cycle===key&&(key!=="weekly"||row.week_key===weekLabel))}));
  const approvedGamePacks=imports.filter((row)=>row.status==="approved"&&row.import_type==="user_game"&&Number(row.season_year)===Number(seasonYear)&&row.week_label===weekLabel).length;

  return <div className="data-intake-page cfb-v2-page">
    <section className="data-intake-hero"><div><span>AI DATA OPERATIONS</span><h1>Dynasty Data Intake</h1><p>Upload screenshots once. AI extracts the visible data, a commissioner verifies it, and only then does it become official.</p></div><div><b>{imports.filter((row)=>row.status==="review").length}</b><span>AWAITING REVIEW</span></div></section>
    <div className="data-intake-grid">
      <section className="data-upload-card"><div className="data-section-head"><span>01 • CAPTURE</span><h2>Upload Screenshots</h2></div>
        <div className="data-policy-strip"><b>User-v-CPU counts.</b><span>Every completed game involving an active coach belongs in season records and stats. CPU-only games do not.</span></div>
        <form onSubmit={submitImport}>
          <label>Season<select value={seasonYear} onChange={(event)=>setSeasonYear(event.target.value)}>{YEARS.map((year)=><option key={year}>{year}</option>)}</select></label>
          <label>League phase<select value={cycle} onChange={(event)=>setCycle(event.target.value)}>{DATA_CYCLES.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
          {cycle==="weekly"&&<label>Week<select value={weekLabel} onChange={(event)=>setWeekLabel(event.target.value)}>{WEEKS.map((week)=><option key={week}>{week}</option>)}</select></label>}
          <label>Screenshot set<select value={importType} onChange={(event)=>setImportType(event.target.value)}>{DATA_IMPORT_TYPES.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
          <label className="data-upload-wide">Notes for the extractor<textarea value={notes} onChange={(event)=>setNotes(event.target.value)} placeholder="Optional: team, screen order, anything unusual…"/></label>
          <label className="data-file-drop data-upload-wide"><input key={fileKey} type="file" accept="image/png,image/jpeg,image/webp" multiple onChange={chooseFiles}/><b>{files.length?`${files.length} screenshot${files.length===1?"":"s"} ready`:"Choose screenshots"}</b><span>PNG, JPG or WebP • up to 12 files • 8 MB each</span></label>
          <button className="data-primary-button data-upload-wide" disabled={busy}>{busy?"Working…":"Upload & Extract"}</button>
        </form>{localMessage&&<div className="data-local-message">{localMessage}</div>}
      </section>
      <section className="data-queue-card"><div className="data-section-head"><span>02 • VERIFY</span><h2>{isCommissioner?"Commissioner Review":"My Uploads"}</h2></div>
        <div className="data-import-list">{imports.length?imports.map((row)=><button key={row.id} onClick={()=>setSelectedId(row.id)} className={selectedId===row.id?"selected":""}><span className={`data-status data-status-${row.status}`}>{row.status}</span><strong>{dataLabel(row.import_type)}</strong><small>{row.season_year}{row.week_label?` • ${row.week_label}`:""} • {row.league_data_import_files?.length||0} image(s)</small>{row.error_message&&<em>{row.error_message}</em>}</button>):<div className="data-empty">No upload batches yet.</div>}</div>
      </section>
    </div>
    {selected&&<section className="data-review-card"><div className="data-review-head"><div><span>SELECTED BATCH</span><h2>{dataLabel(selected.import_type)} • {selected.status}</h2></div><div>{(selected.league_data_import_files||[]).map((file,index)=><button key={file.id} onClick={()=>openFile(file)}>View {index+1}</button>)}</div></div>
      {selected.warnings?.length>0&&<div className="data-warning-list"><b>Extraction flags</b>{selected.warnings.map((warning,index)=><span key={index}>{warning}</span>)}</div>}
      {isCommissioner&&selected.status==="review"?<><p className="data-review-note">Compare this draft with every screenshot. Correct any value below before approval. Approval is the only action that writes official results or archive records.</p><textarea className="data-json-review" value={reviewText} onChange={(event)=>setReviewText(event.target.value)}/><div className="data-review-actions"><button onClick={approveImport} disabled={busy}>Approve & Publish</button><button onClick={rejectImport} disabled={busy}>Reject</button></div></>:selected.status==="failed"?<button className="data-primary-button" onClick={()=>retryExtraction(selected)} disabled={busy}>Retry Extraction</button>:<pre className="data-json-preview">{JSON.stringify(selected.corrected_payload||selected.extraction||selected.applied_summary,null,2)}</pre>}
    </section>}
    <section className="data-checklist-card"><div className="data-section-head"><span>SEASON CONTROL</span><h2>Nothing-Missed Checklist</h2><p>Approved uploads complete their matching task automatically. “Completed user-game packs” remains a commissioner check because only you know when every game is in.</p></div>
      <div className="data-checklist-progress"><b>{checklist.filter((row)=>row.status!=="pending").length}/{checklist.length}</b><span>requirements cleared for {seasonYear}{weekLabel?` • ${weekLabel}`:""}</span></div>
      <div className="data-phase-grid">{phases.map((phase)=><article key={phase.key}><header><span>{phase.label}</span><b>{phase.rows.filter((row)=>row.status!=="pending").length}/{phase.rows.length}</b></header>{phase.rows.map((row)=><div key={row.id} className={`data-check-item ${row.status}`}><span>{row.status==="complete"?"✓":row.status==="not_applicable"?"—":"○"}</span><div><strong>{row.title}</strong><small>{row.description}{row.task_key==="user_games"?` • ${approvedGamePacks} approved game pack(s) this week`:""}</small></div>{isCommissioner&&<select value={row.status} onChange={(event)=>setChecklistStatus(row,event.target.value)}><option value="pending">Pending</option><option value="complete">Complete</option><option value="not_applicable">N/A</option></select>}</div>)}</article>)}</div>
    </section>
  </div>;
}

function DynastyDataArchive({currentYear,setActiveTab}){
  const [snapshots,setSnapshots]=useState([]);const [records,setRecords]=useState([]);const [year,setYear]=useState(String(currentYear));const [type,setType]=useState("all");const [expanded,setExpanded]=useState("");const [message,setMessage]=useState("");
  async function loadArchive(){
    const [snapshotRes,recordRes]=await Promise.all([supabase.from("league_data_snapshots").select("*").order("season_year",{ascending:false}).order("approved_at",{ascending:false}),supabase.from("league_data_records").select("*").order("season_year",{ascending:false}).order("created_at",{ascending:false}).limit(2000)]);
    if(snapshotRes.error||recordRes.error){setMessage(`Archive load failed: ${snapshotRes.error?.message||recordRes.error?.message}`);return;}setSnapshots(snapshotRes.data||[]);setRecords(recordRes.data||[]);
  }
  useEffect(()=>{loadArchive();},[]);
  const years=[...new Set([String(currentYear),...snapshots.map((row)=>String(row.season_year))])].sort((a,b)=>Number(b)-Number(a));
  const types=[...new Set(snapshots.map((row)=>row.snapshot_type))].sort();
  const filtered=snapshots.filter((row)=>String(row.season_year)===year&&(type==="all"||row.snapshot_type===type));
  return <div className="dynasty-archive-page cfb-v2-page"><section className="archive-hero"><div><span>PERMANENT LEAGUE MEMORY</span><h1>Dynasty Data Archive</h1><p>Verified rankings, schedules, rosters, statistics, recruiting, portal movement, awards and playoff pictures—preserved by season and week.</p></div><div className="archive-hero-links"><b>{snapshots.length}</b><span>APPROVED CAPTURES</span><button onClick={()=>setActiveTab?.("gameTop25")}>Top 25</button><button onClick={()=>setActiveTab?.("teamSchedules")}>Team Schedules</button></div></section>
    <section className="archive-filter"><label>Season<select value={year} onChange={(event)=>setYear(event.target.value)}>{years.map((item)=><option key={item}>{item}</option>)}</select></label><label>Data set<select value={type} onChange={(event)=>setType(event.target.value)}><option value="all">All verified data</option>{types.map((item)=><option key={item} value={item}>{dataLabel(item)}</option>)}</select></label><button onClick={loadArchive}>Refresh Archive</button></section>
    {message&&<div className="data-local-message">{message}</div>}
    <section className="archive-grid">{filtered.length?filtered.map((snapshot)=>{const snapshotRecords=records.filter((row)=>row.snapshot_id===snapshot.id);const open=expanded===snapshot.id;return <article key={snapshot.id} className={open?"open":""}><button className="archive-card-head" onClick={()=>setExpanded(open?"":snapshot.id)}><span><small>{dataLabel(snapshot.snapshot_type)}</small><strong>{snapshot.week_label||dataLabel(snapshot.cycle,DATA_CYCLES)}</strong></span><b>{snapshotRecords.length} RECORDS</b><em>{open?"−":"+"}</em></button>{open&&<div className="archive-card-body"><div className="archive-record-list">{snapshotRecords.slice(0,250).map((row)=><div key={row.id}><span>{row.rank?`#${row.rank}`:"•"}</span><strong>{row.entity_name||row.team_name||dataLabel(row.record_type)}</strong><small>{row.entity_secondary||row.team_name||"Verified league record"}</small><em>{metricSummary(row.metrics)}</em></div>)}</div><details><summary>View verified source JSON</summary><pre>{JSON.stringify(snapshot.payload,null,2)}</pre></details></div>}</article>}):<div className="data-empty">No approved captures match these filters yet.</div>}</section>
  </div>;
}
  ...
  return (
    <style>{`
      :root {
        ...
        --cfb-display: "Chakra Petch", "Inter", ui-sans-serif, system-ui, sans-serif;
        --cfb-red: #3e7fc1;
        --cfb-red-dark: #274d73;
        --cfb-gold: #c9d0d9;
        --cfb-green: #2fb673;
      ...
      * { box-sizing: border-box; }
      ...
      .elite-ticker { min-width:0; display:grid; grid-template-columns:116px minmax(0,1fr); overflow:hidden; border:1px solid rgba(201,208,217,.16); border-radius:12px; background:var(--cfb-panel); box-shadow:0 16px 35px rgba(0,0,0,.22); }
      .elite-ticker-label { display:flex; flex-direction:column; justify-content:center; align-items:flex-start; gap:1px; padding:12px 15px; border:0; color:white; background:linear-gradient(135deg,var(--cfb-danger),#7d1b2f); cursor:pointer; clip-path:polygon(0 0,100% 0,100% 100%,14% 100%,0 84%); }
      .elite-ticker-label span { font-family:var(--cfb-display); font-size:9px; letter-spacing:.18em; font-weight:700; display:flex; align-items:center; gap:5px; }
      .elite-ticker-label span::before { content:""; width:5px; height:5px; border-radius:50%; background:#fff; box-shadow:0 0 6px #fff; animation:networkPulse 1.5s infinite; }
      .elite-ticker-label b { font-family:var(--cfb-display); font-weight:600; font-size:17px; font-variant-numeric:tabular-nums; }
      .elite-ticker-label small { opacity:.74; font-family:var(--cfb-display); letter-spacing:.02em; }
      ...
      .elite-status-open { color:var(--cfb-green); } .elite-status-settled { color:var(--cfb-red); } .elite-status-locked { color:#fb923c; }
      ...
      .elite-discord-drawer > div { min-width:0;display:flex;flex-direction:column; } .elite-discord-drawer span { color:#a5b4fc;font-size:8px;font-weight:1000;letter-spacing:.12em; } .elite-discord-drawer strong { overflow:hidden;text-overflow:ellipsis; } .elite-discord-drawer button { flex:0 0 auto;border:0;border-radius:7px;padding:8px 10px;color:#fff;background:var(--cfb-red);font-size:9px;font-weight:1000;cursor:pointer; }
      .elite-commissioner-access { display:flex;justify-content:space-between;align-items:center;gap:18px;padding:20px;border:1px solid rgba(167,139,250,.32);border-radius:16px;background:linear-gradient(135deg,rgba(76,29,149,.22),rgba(15,23,42,.92)); } .elite-commissioner-access span { font-family:var(--cfb-display);color:var(--cfb-red);font-size:9px;font-weight:1000;letter-spacing:.15em; } .elite-commissioner-access h2 { margin:4px 0;font-size:22px; } .elite-commissioner-access p { max-width:780px;margin:0;color:var(--cfb-muted);line-height:1.45; } .elite-commissioner-access > strong { flex:0 0 auto;padding:10px 13px;border:1px solid rgba(167,139,250,.32);border-radius:999px;color:#ddd6fe;background:rgba(62,127,193,.12);font-family:var(--cfb-display);font-variant-numeric:tabular-nums;font-size:11px; }
      .elite-user-statuses { display:flex;flex-direction:column;align-items:flex-end;gap:5px; } .elite-commissioner-badge { padding:5px 7px;border:1px solid rgba(167,139,250,.38);border-radius:999px;color:#ddd6fe;background:rgba(62,127,193,.13);font-size:7px;font-weight:1000;letter-spacing:.08em; }
      .elite-commissioner-toggle { width:100%;margin-top:10px;padding:9px;border-radius:8px;font-size:9px;font-weight:1000;cursor:pointer; } .elite-commissioner-toggle.grant { border:1px solid rgba(167,139,250,.34);color:#ddd6fe;background:rgba(62,127,193,.12); } .elite-commissioner-toggle.revoke { border:1px solid rgba(248,113,113,.3);color:#fecaca;background:rgba(127,29,29,.16); } .elite-commissioner-toggle:disabled { opacity:.45;cursor:not-allowed; }
      ...
      @media (max-width:760px) {
        ...
        .league-login-card { grid-template-columns:1fr;text-align:center; }.league-login-mark { margin:auto; }.league-login-actions,.league-login-error { grid-column:1;justify-content:center; }.network-layout { grid-template-columns:1fr; }.network-sidebar { border-right:0;border-bottom:1px solid rgba(148,163,184,.15); }.network-sidebar > nav { grid-template-columns:repeat(6,minmax(82px,1fr));overflow-x:auto; }.network-channel-list,.network-conversation-list { grid-template-columns:repeat(2,1fr); }.network-stage { min-height:620px; }.redzone-multiview { grid-template-columns:1fr; }.redzone-stream-grid { grid-template-columns:1fr; }.redzone-profile-form { grid-template-columns:1fr; }.redzone-profile-form small { grid-column:1; }.network-setting-grid { grid-template-columns:1fr; }.network-online-roster { display:flex;align-items:center;gap:10px;overflow-x:auto;padding-bottom:9px; }.network-online-roster header,.network-online-roster > div { flex:none;margin:0; }.network-online-roster > div .network-identity > span { max-width:110px; }.network-channel-form { grid-template-columns:1fr; }.network-channel-form label.wide,.network-lock-toggle,.network-manager-actions { grid-column:1; }.network-permission-panel > div:last-child { grid-template-columns:1fr; }.network-stage-actions,.redzone-header-actions { width:100%; }.network-stage-actions button,.redzone-header-actions button { flex:1; }.elite-member-control-row { grid-template-columns:1fr; }.network-hero-badges { display:none; }.network-emoji-uploader { grid-template-columns:1fr; }.ranking-history-hero { align-items:stretch;flex-direction:column; }.ranking-history-hero > div:last-child { flex-wrap:wrap; }.redzone-stream-grid footer { grid-template-columns:1fr 1fr; }.redzone-stream-grid footer .redzone-remove-stream { grid-column:1/-1; }
      }
      @media (max-width:520px) {
        ...
        .elite-pick-button { padding:7px 5px;grid-template-columns:1fr; } .elite-pick-button b,.elite-pick-button em,.elite-pick-button small,.elite-pick-consensus { text-align:center; }
      ...
      /* v30 mobile coach + League Hub layout repair. */
      @media (max-width:760px) {
        .cfb-coach-hero-identity-v68 { overflow:visible!important; }
        .cfb-coach-hero-identity-v68 > span:first-child { width:84px!important;height:84px!important;min-width:84px!important;min-height:84px!important;overflow:hidden!important; }
        .cfb-coach-hero-identity-v68 > span:first-child img { width:82px!important;height:82px!important;max-width:82px!important;max-height:82px!important; }
        .cfb-coach-hero-identity-v68 > div:last-child { min-width:0!important;position:relative;z-index:2; }
        .cfb-coach-hero-identity-v68 > div:last-child > div,
        .cfb-coach-hero-identity-v68 > div:last-child > p { max-width:100%;overflow-wrap:anywhere; }
        .cfb-v2-coach-strip > div { overflow:hidden; }
        .cfb-v2-coach-strip > div > b { max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }
        .cfb-v2-coach-strip > div:nth-child(4) > b { display:grid!important;grid-template-columns:30px minmax(0,1fr);width:100%; }
        .cfb-v2-coach-strip > div:nth-child(4) > b > span { overflow:hidden!important; }

        .network-page { width:100%;max-width:100%;overflow:hidden; }
        .network-page * { box-sizing:border-box; }
        .network-hero { width:100%;display:grid!important;grid-template-columns:minmax(0,1fr) auto;align-items:center!important;gap:14px!important;padding:18px!important;border-radius:17px!important; }
        .network-hero > div:first-child { min-width:0; }
        .network-hero h1 { margin:4px 0!important;font-size:clamp(40px,13vw,56px)!important;line-height:.92!important; }
        .network-hero p { max-width:100%;margin-top:8px!important;font-size:13px!important;line-height:1.45; }
        .network-live-presence { min-width:90px!important;padding:11px 12px!important; }
        .network-live-presence strong { font-size:25px!important; }
        .network-layout { width:100%;min-height:0!important;display:block!important;overflow:hidden!important;border-radius:17px!important; }
        .network-sidebar { width:100%;min-width:0;overflow:hidden;border-right:0!important;border-bottom:1px solid rgba(148,163,184,.15); }
        .network-sidebar > nav { width:100%;grid-template-columns:repeat(3,minmax(0,1fr))!important;gap:5px!important;padding:8px!important;overflow:visible!important; }
        .network-sidebar > nav button { min-width:0!important;min-height:39px!important;padding:7px 3px!important;font-size:10px!important; }
        .network-sidebar-label { padding:9px 11px 5px!important; }
        .network-channel-list { width:100%;display:flex!important;gap:7px!important;padding:7px 9px 11px!important;overflow-x:auto!important;overflow-y:hidden!important;scroll-snap-type:x proximity;-webkit-overflow-scrolling:touch; }
        .network-channel-list > button { width:176px!important;min-width:176px!important;max-width:176px!important;grid-template-columns:34px minmax(0,1fr)!important;gap:8px!important;padding:9px!important;scroll-snap-align:start; }
        .network-channel-list .network-channel-mark { width:34px!important;height:34px!important; }
        .network-channel-list strong { display:block;overflow:hidden;font-size:12px!important;text-overflow:ellipsis;white-space:nowrap; }
        .network-channel-list small { display:block;overflow:hidden;font-size:9px!important;text-overflow:ellipsis;white-space:nowrap; }
        .network-channel-list em { display:none; }
        .network-online-roster { display:none!important; }
        .network-conversation-list { grid-template-columns:1fr!important;max-height:190px;overflow-y:auto; }
        .network-stage { width:100%;min-width:0;min-height:0!important;display:grid;grid-template-rows:auto minmax(300px,auto) auto; }
        .network-stage-header { width:100%;display:grid!important;grid-template-columns:minmax(0,1fr) auto;align-items:center!important;gap:10px!important;padding:14px!important; }
        .network-stage-channel { min-width:0;gap:10px!important; }
        .network-stage-channel > div { min-width:0; }
        .network-stage-header h2 { max-width:100%;overflow:hidden;font-size:clamp(25px,8vw,34px)!important;text-overflow:ellipsis;white-space:nowrap; }
        .network-stage-header p { display:-webkit-box;max-width:100%;overflow:hidden;font-size:12px!important;line-height:1.4;-webkit-box-orient:vertical;-webkit-line-clamp:2; }
        .network-stage-actions { width:auto!important; }
        .network-stage-actions button { width:auto;min-height:38px;padding:7px 9px;font-size:9px; }
        .network-message-feed { width:100%;min-height:300px!important;max-height:56vh;padding:10px!important;overflow-y:auto; }
        .network-empty { min-height:300px!important;padding:24px 12px; }
        .network-empty b { font-size:20px; }
        .network-message-feed article { min-width:0;padding:11px!important; }
        .network-message-head { min-width:0;align-items:flex-start; }
        .network-message-feed article p { overflow-wrap:anywhere;word-break:break-word;font-size:14px!important; }
        .network-composer { width:100%;grid-template-columns:1fr!important;gap:8px!important;padding:10px!important; }
        .network-compose-box,.network-compose-box textarea { min-width:0;width:100%; }
        .network-composer textarea { min-height:78px; }
        .network-send { width:100%;min-height:46px!important;font-size:14px!important; }
        .network-role-manager,.network-emoji-library { width:100%;padding:14px!important;overflow-x:hidden; }
        .network-role-manager > header,.network-emoji-library > header { align-items:flex-start;flex-direction:column;gap:8px; }
        .network-role-layout > aside { grid-template-columns:1fr!important; }
        .network-role-form { grid-template-columns:1fr!important; }
        .network-role-form > * { grid-column:1!important; }
      }
      @media (max-width:520px) {
        .cfb-coach-hero-identity-v68 { grid-template-columns:68px minmax(0,1fr)!important;gap:10px!important; }
        .cfb-coach-hero-identity-v68 > span:first-child { width:68px!important;height:68px!important;min-width:68px!important;min-height:68px!important; }
        .cfb-coach-hero-identity-v68 > span:first-child img { width:66px!important;height:66px!important;max-width:66px!important;max-height:66px!important; }
        .cfb-coach-name-v68 { font-size:clamp(29px,9.5vw,38px)!important;line-height:.95!important; }
        .network-hero { grid-template-columns:1fr!important; }
        .network-live-presence { width:max-content; }
      }

      /* v31: Discord-style mobile Hub, cleaner menu and compact coach identity. */
      .network-mobile-hub-header,.network-mobile-directory-title { display:none; }
      .network-channel-category { padding:12px 8px 5px;color:#64748b;font-size:9px;font-weight:1000;letter-spacing:.13em; }
      .drawer-nav-arrow { margin-left:auto;flex:none;font-size:24px;font-weight:800;line-height:1;transition:transform .18s ease; }
      .drawer-nav-tile:hover .drawer-nav-arrow { transform:translateX(3px); }
      .drawer-nav-arrow.active { font-size:10px;filter:drop-shadow(0 0 8px currentColor); }
      .drawer-nav-copy { padding-left:3px; }

      @media (max-width:760px) {
        .cfb-coach-hero-v68 { gap:14px!important;padding:16px!important;border-radius:18px!important; }
        .cfb-coach-hero-identity-v68 { grid-template-columns:62px minmax(0,1fr)!important;gap:12px!important; }
        .cfb-coach-hero-identity-v68 > span:first-child { width:62px!important;height:62px!important;min-width:62px!important;min-height:62px!important; }
        .cfb-coach-hero-identity-v68 > span:first-child img { width:60px!important;height:60px!important;max-width:60px!important;max-height:60px!important; }
        .cfb-coach-hero-identity-v68 > div:last-child { width:100%;max-width:100%;overflow:hidden; }
        .cfb-coach-hero-identity-v68 > div:last-child > div:first-child { overflow:hidden;font-size:10px!important;line-height:1.3;text-overflow:ellipsis;white-space:nowrap; }
        .cfb-coach-name-v68 { width:100%;max-width:100%;margin:5px 0!important;overflow:visible!important;font-size:clamp(23px,7.2vw,31px)!important;line-height:1!important;letter-spacing:-.035em!important;overflow-wrap:anywhere!important;word-break:break-word!important;white-space:normal!important; }
        ...
        .network-page { padding-bottom:92px; }
        .network-page > .network-hero { display:none!important; }
        .network-mobile-hub-header { position:sticky;top:0;z-index:45;width:100%;min-height:68px;display:grid;grid-template-columns:44px minmax(0,1fr) 44px;align-items:center;gap:9px;padding:10px 12px;border:1px solid rgba(148,163,184,.13);border-radius:16px 16px 0 0;background:rgba(17,18,24,.97);backdrop-filter:blur(18px);box-shadow:0 8px 24px rgba(0,0,0,.25); }
        ...
        .network-channel-list strong { overflow:hidden!important;font-size:14px!important;text-overflow:ellipsis!important;white-space:nowrap!important; }
        .network-channel-list small { overflow:hidden!important;color:#737989!important;font-size:10px!important;text-overflow:ellipsis!important;white-space:nowrap!important; }
        .network-channel-list em { position:static!important;display:block!important;color:#c9d0d9;font-size:7px; }
        .network-member-search { padding:13px!important; }
        .network-member-search input { border:0!important;border-radius:999px!important;background:#24262e!important; }
        .network-conversation-list { max-height:none!important;padding:4px 9px 22px!important; }
        .network-conversation-list button { min-height:54px;padding:9px 11px!important;border-radius:8px!important; }
        ...
        .network-message-toolbar,.network-reaction-row { margin-left:47px; }
        ...
        .network-role-manager,.network-emoji-library,.network-notifications,.network-settings,.network-channel-manager,.network-category-manager { height:100%;overflow-y:auto!important;background:#0b1220!important; }
      }
      ...
      .data-status { align-self:start;padding:4px 6px;border-radius:999px;color:#9ca3af;background:#1f2937;font-size:7px;font-weight:1000;letter-spacing:.1em;text-transform:uppercase; }.data-status-review { color:#dde3e9;background:rgba(245,158,11,.13); }.data-status-approved { color:#99f6e4;background:rgba(20,184,166,.13); }.data-status-failed,.data-status-rejected { color:#fecaca;background:rgba(220,38,38,.13); }.data-status-processing,.data-status-uploaded { color:#bfdbfe;background:rgba(37,99,235,.14); }
      ...
      .archive-hero { border-color:rgba(154,143,209,.25);border-top-color:var(--cfb-red);background:radial-gradient(circle at 82% 0%,rgba(62,127,193,.16),transparent 32%),linear-gradient(120deg,#0d0a16,#101827 65%,#170f24); }.archive-hero span { color:var(--cfb-red); }.archive-hero > div:last-child { border-color:rgba(154,143,209,.28);background:#100b1b; }
      .archive-hero-links { grid-template-columns:1fr 1fr!important;gap:7px!important; }.archive-hero-links > b,.archive-hero-links > span { grid-column:1/-1; }.archive-hero-links button { padding:8px;border:1px solid rgba(154,143,209,.28);border-radius:7px;color:#ddd6fe;background:rgba(62,127,193,.08);font-size:8px;font-weight:1000;cursor:pointer; }
      .archive-filter { display:flex;align-items:end;flex-wrap:wrap;gap:12px;padding:16px; }.archive-filter label { min-width:200px; }.archive-filter button { margin-left:auto;padding:11px 14px;border:1px solid rgba(154,143,209,.35);border-radius:8px;color:#ddd6fe;background:rgba(62,127,193,.09);font-weight:900;cursor:pointer; }
      .archive-grid { display:grid;gap:10px; }.archive-grid > article.open { border-color:rgba(154,143,209,.38); }.archive-card-head { width:100%;display:grid;grid-template-columns:minmax(0,1fr) auto 24px;align-items:center;gap:12px;padding:17px;border:0;color:#fff;text-align:left;background:transparent;cursor:pointer; }.archive-card-head span { display:grid;gap:3px; }.archive-card-head small { color:var(--cfb-red);font-size:8px;font-weight:1000;letter-spacing:.13em;text-transform:uppercase; }.archive-card-head strong { font-size:19px; }.archive-card-head b { color:var(--cfb-muted);font-size:9px;letter-spacing:.1em; }.archive-card-head em { color:var(--cfb-red);font-size:23px;font-style:normal; }.archive-card-body { padding:0 17px 17px; }.archive-record-list { display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:7px;margin-bottom:13px; }.archive-record-list > div { display:grid;grid-template-columns:36px minmax(0,1fr);gap:2px 8px;padding:10px;border:1px solid rgba(148,163,184,.12);border-radius:8px;background:#070c14; }.archive-record-list > div > span { grid-row:1/4;align-self:center;color:var(--cfb-red);font-weight:1000;text-align:center; }.archive-record-list strong { overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }.archive-record-list small { color:#7f8da2; }.archive-record-list em { overflow:hidden;color:#aab6c6;font-size:9px;font-style:normal;text-overflow:ellipsis;white-space:nowrap; }.archive-card-body details summary { color:var(--cfb-red);font-size:10px;font-weight:900;cursor:pointer; }
      ...
      .home-capture-panel { overflow:hidden;border:1px solid rgba(79,143,168,.23);border-top:4px solid #c9d0d9;border-radius:15px;background:radial-gradient(circle at 88% 0%,rgba(201,208,217,.11),transparent 32%),linear-gradient(125deg,#0a111c,#0d1726 68%,#11130d);box-shadow:0 18px 48px rgba(0,0,0,.28); }
      .home-capture-panel > header { display:flex;align-items:center;justify-content:space-between;gap:20px;padding:20px 22px;border-bottom:1px solid rgba(201,208,217,.14); }.home-capture-panel > header span,.home-capture-feature > span,.captured-data-hero span,.captured-top25-board > header span,.captured-schedule-card > header span,.captured-program-schedule > header span { color:#c9d0d9;font-size:8px;font-weight:1000;letter-spacing:.16em; }.home-capture-panel h2 { margin:4px 0 3px;font-size:25px; }.home-capture-panel p { margin:0;color:var(--cfb-muted);font-size:11px;line-height:1.5; }.home-capture-panel > header button { padding:10px 12px;border:1px solid rgba(201,208,217,.3);border-radius:8px;color:#dde3e9;background:rgba(201,208,217,.07);font-size:9px;font-weight:1000;cursor:pointer; }
      .home-capture-grid { display:grid;grid-template-columns:1.1fr .9fr 1fr; }.home-capture-feature { min-width:0;min-height:190px;display:grid;align-content:start;gap:8px;padding:19px;border:0;border-right:1px solid rgba(148,163,184,.13);color:var(--cfb-text);text-align:left;background:transparent;cursor:pointer; }.home-capture-feature:last-child { border-right:0; }.home-capture-feature:hover { background:rgba(79,143,168,.055); }.home-capture-feature > strong { font-size:20px;letter-spacing:-.025em; }.home-capture-feature > div { display:grid;gap:5px;margin-top:3px; }.home-capture-feature i { display:grid;grid-template-columns:30px 30px minmax(0,1fr);align-items:center;gap:6px;padding:5px 7px;border-radius:7px;background:rgba(255,255,255,.035);font-style:normal; }.home-capture-feature i > b { color:#c9d0d9;font-size:10px; }.home-capture-feature i > em { overflow:hidden;color:#dce5f1;font-size:10px;font-style:normal;text-overflow:ellipsis;white-space:nowrap; }.home-capture-feature.archive { align-content:center;background:linear-gradient(145deg,rgba(62,127,193,.08),transparent); }.home-capture-feature.archive p { font-size:12px; }.home-schedule-counts { grid-template-columns:repeat(3,1fr)!important;gap:7px!important;margin-top:15px!important; }.home-schedule-counts > b { display:grid;gap:3px;padding:12px;border:1px solid rgba(79,143,168,.16);border-radius:9px;color:#fff;text-align:center;background:#09121e;font-size:23px; }.home-schedule-counts small { color:#7f91a8;font-size:7px;text-transform:uppercase;letter-spacing:.1em; }
      .captured-data-page { display:grid;gap:15px;padding-bottom:30px; }.captured-data-hero { display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:24px;padding:32px;border:1px solid rgba(201,208,217,.25);border-top:5px solid #c9d0d9;border-radius:18px;background:radial-gradient(circle at 85% 0%,rgba(201,208,217,.16),transparent 32%),linear-gradient(120deg,#100d06,#101827 65%,#161107);box-shadow:0 24px 65px rgba(0,0,0,.34); }.captured-data-hero.schedules { border-color:rgba(79,143,168,.26);border-top-color:var(--cfb-red);background:radial-gradient(circle at 85% 0%,rgba(59,130,246,.16),transparent 32%),linear-gradient(120deg,#07101e,#101827 65%,#08152a); }.captured-data-hero.schedules span { color:#bcd6f0; }.captured-data-hero h1 { margin:6px 0;font-size:clamp(45px,6vw,82px);line-height:.92;letter-spacing:-.065em; }.captured-data-hero p { max-width:820px;margin:0;color:#a6b2c3;line-height:1.55; }.captured-data-hero > div:last-child { display:grid;min-width:170px;padding:17px;border:1px solid rgba(201,208,217,.28);border-radius:13px;background:#110e07; }.captured-data-hero.schedules > div:last-child { border-color:rgba(79,143,168,.28);background:#071222; }.captured-data-hero > div:last-child b { font-size:38px; }.captured-data-hero > div:last-child span { font-size:8px; }
      ...
      @media (max-width:430px) {
        .cfb-coach-name-v68 { font-size:clamp(21px,6.8vw,28px)!important; }
        .drawer-menu-group { padding:8px!important; }
        .drawer-nav-tile { min-height:60px!important;padding:11px 13px!important;border-radius:13px!important; }
        .drawer-nav-copy strong { font-size:13px; }
        .drawer-nav-copy small { font-size:9px; }
      }

      /* League Hub v34 — national broadcast control room. */
      .network-page { --hub-canvas:#05070b;--hub-sidebar:#0d121b;--hub-panel:#101620;--hub-raised:#171e2a;--hub-blue:#2797ff;--hub-gold:#c9d0d9;--hub-red:#e5484d;--hub-text:#f7f8fb;--hub-muted:#9aa6b8;background:radial-gradient(circle at 50% -20%,rgba(39,151,255,.12),transparent 38%),#05070b!important; }
      .network-page .network-hero { border:1px solid rgba(201,208,217,.24);border-top:4px solid var(--hub-red);background:linear-gradient(105deg,#080b11 0%,#111823 58%,#241217 100%);box-shadow:0 18px 60px rgba(0,0,0,.35),inset 0 -1px rgba(39,151,255,.18); }
      .network-page .network-hero::after { content:"";position:absolute;right:18%;bottom:0;width:260px;height:3px;background:linear-gradient(90deg,transparent,var(--hub-gold),transparent); }
      .network-page .network-hero > div > span,.network-page .network-stage-header span { color:var(--hub-gold); }
      .network-page .network-layout { border:1px solid rgba(201,208,217,.18);border-top:3px solid var(--hub-red);background:#05070b;box-shadow:0 28px 80px rgba(0,0,0,.42); }
      ...
      .network-page .network-member-rail { border-left:1px solid rgba(229,72,77,.28);background:linear-gradient(180deg,#101620,#090e15); }
      .network-page .network-member-rail > header { border-bottom:2px solid rgba(201,208,217,.42);background:#090d14; }
      ...
      .network-page .network-sidebar > nav { border-bottom:2px solid rgba(229,72,77,.46);background:#080c12; }
      ...
      .network-page .network-sidebar > nav button.active { border-color:rgba(201,208,217,.5)!important;color:#171208!important;background:linear-gradient(135deg,#ffd95e,#e9b72f)!important;box-shadow:0 0 24px rgba(201,208,217,.2),inset 0 1px rgba(255,255,255,.45)!important; }
      .network-page .network-channel-category { color:#d7dee8;background:linear-gradient(90deg,rgba(201,208,217,.06),transparent); }
      .network-page .network-channel-list > button:hover { color:#fff;background:rgba(39,151,255,.1); }
      .network-page .network-channel-list > button.active { border-color:rgba(229,72,77,.32);color:#fff;background:linear-gradient(90deg,rgba(229,72,77,.22),rgba(39,151,255,.09));box-shadow:inset 4px 0 var(--hub-red),0 8px 20px rgba(0,0,0,.2); }
      ...
      .newsroom-story-meta { display:flex;align-items:center;gap:9px;color:#8f9caf;font-size:9px;text-transform:uppercase;letter-spacing:.1em; }.newsroom-story-meta b { color:#bcd6f0; }.newsroom-story-meta em { margin-left:auto;padding:4px 6px;border-radius:999px;color:#c9d0d9;background:rgba(201,208,217,.08);font-style:normal;font-weight:1000; }
      ...
      .home-newsroom-rail { overflow:hidden;border:1px solid rgba(201,208,217,.22);border-top:4px solid #3e7fc1;border-radius:17px;color:var(--cfb-text);background:linear-gradient(125deg,#090d14,#101722 62%,#211014);box-shadow:0 18px 50px rgba(0,0,0,.25); }.home-newsroom-rail > header { display:flex;align-items:center;justify-content:space-between;gap:18px;padding:20px 22px;border-bottom:1px solid rgba(201,208,217,.14); }.home-newsroom-rail > header span,.newsroom-platform-hero span,.newsroom-platform-feed > header span,.newsroom-platform-desk span { color:#c9d0d9;font-size:9px;font-weight:1000;letter-spacing:.17em; }.home-newsroom-rail > header h2 { margin:4px 0;font-size:25px; }.home-newsroom-rail > header p { margin:0;color:var(--cfb-muted);font-size:12px; }.home-newsroom-rail > header button,.newsroom-platform-feed > header button { border:1px solid rgba(201,208,217,.3);border-radius:9px;padding:10px 13px;color:#c9d0d9;background:rgba(201,208,217,.08);font-weight:900;cursor:pointer; }.home-newsroom-rail > div { display:grid;grid-template-columns:repeat(4,minmax(0,1fr)); }.home-newsroom-rail article { min-width:0;border-right:1px solid rgba(148,163,184,.13); }.home-newsroom-rail article:last-child { border-right:0; }.home-newsroom-rail article > button { position:relative;width:100%;min-height:142px;padding:18px;border:0;color:#fff;text-align:left;background:transparent;cursor:pointer; }.home-newsroom-rail article > button span { color:#bcd6f0;font-size:8px;font-weight:1000;letter-spacing:.13em; }.home-newsroom-rail article > button h3 { margin:8px 25px 13px 0;font-size:18px;line-height:1.15; }.home-newsroom-rail article > button small { color:#7f8b9d; }.home-newsroom-rail article > button i { position:absolute;right:15px;top:16px;color:#c9d0d9;font-size:20px;font-style:normal; }.home-newsroom-rail article.open { grid-column:span 2;background:rgba(201,208,217,.04); }.home-newsroom-rail article.open > div { padding:0 18px 18px;color:#b8c2d0; }.home-newsroom-rail article.open > div strong { display:block;color:#e2e8f0; }.home-newsroom-rail article.open > div p { line-height:1.65;white-space:pre-wrap; }.home-newsroom-rail article.open > div button { border:0;color:#c9d0d9;background:transparent;font-weight:900;cursor:pointer; }
      .newsroom-platform-page { min-height:80vh;color:var(--cfb-text);background:radial-gradient(circle at 80% 0%,rgba(229,72,77,.12),transparent 30%),#060910!important; }.newsroom-platform-hero { display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:25px;padding:36px 32px;border:1px solid rgba(201,208,217,.22);border-top:5px solid #3e7fc1;border-radius:18px;background:linear-gradient(110deg,#080c13,#111827 65%,#251015);box-shadow:0 26px 70px rgba(0,0,0,.35); }.newsroom-platform-hero h1 { margin:5px 0 8px;font-size:clamp(48px,7vw,96px);line-height:.9;letter-spacing:-.065em; }.newsroom-platform-hero p { max-width:720px;margin:0;color:#aeb8c7;font-size:15px; }.newsroom-platform-hero > button { padding:14px 18px;border:1px solid rgba(201,208,217,.5);border-radius:10px;color:#161006;background:linear-gradient(135deg,#ffdd69,#e8b72f);font-weight:1000;cursor:pointer; }.newsroom-platform-score { display:grid;min-width:170px;padding:17px;border:1px solid rgba(39,151,255,.25);border-radius:13px;background:#07101d; }.newsroom-platform-score b { font-size:36px; }.newsroom-platform-score span { color:#bcd6f0;font-size:8px;font-weight:1000;letter-spacing:.12em; }.newsroom-platform-score small { margin-top:6px;color:#7f8b9d; }.newsroom-platform-desk { display:flex;align-items:center;justify-content:space-between;gap:20px;margin-top:13px;padding:15px 20px;border:1px solid rgba(201,208,217,.16);border-left:4px solid #c9d0d9;border-radius:10px;background:#101620; }.newsroom-platform-desk div { display:grid;gap:3px; }.newsroom-platform-desk small { color:#93a0b2; }.newsroom-review { margin-top:14px;border:1px solid rgba(201,208,217,.14);border-radius:14px;background:#080c13; }.newsroom-platform-feed { margin-top:16px;overflow:hidden;border:1px solid rgba(39,151,255,.14);border-radius:17px;background:#080c13; }.newsroom-platform-feed > header { display:flex;align-items:center;justify-content:space-between;padding:22px 25px;border-bottom:2px solid rgba(229,72,77,.36);background:#0d131d; }.newsroom-platform-feed > header h2 { margin:4px 0 0;font-size:30px; }.newsroom-platform-list { display:grid; }.newsroom-platform-story { border-bottom:1px solid rgba(148,163,184,.12); }.newsroom-platform-story:last-child { border-bottom:0; }.newsroom-platform-story-head { position:relative;width:100%;padding:23px 62px 23px 25px;border:0;color:#fff;text-align:left;background:linear-gradient(90deg,rgba(255,255,255,.02),transparent);cursor:pointer; }.newsroom-platform-story-head:hover { background:linear-gradient(90deg,rgba(39,151,255,.08),transparent); }.newsroom-platform-story-head h3 { margin:10px 0 7px;font-size:clamp(25px,3vw,42px);line-height:1.05;letter-spacing:-.035em; }.newsroom-platform-story-head > strong { display:block;color:#c5cfdb;font-size:14px; }.newsroom-platform-story-head > small { display:block;margin-top:10px;color:#748197; }.newsroom-platform-story-head > i { position:absolute;right:26px;top:50%;transform:translateY(-50%);color:#c9d0d9;font-size:30px;font-style:normal; }.newsroom-platform-story.open { background:radial-gradient(circle at 90% 0%,rgba(229,72,77,.08),transparent 30%); }.newsroom-platform-story-body { padding:0 25px 27px; }.newsroom-platform-story-body > p { max-width:980px;color:#b6c1cf;font-size:15px;line-height:1.8;white-space:pre-wrap; }.newsroom-article-reactions { display:flex;align-items:center;flex-wrap:wrap;gap:7px;padding:13px 0;border-top:1px solid rgba(148,163,184,.12);border-bottom:1px solid rgba(148,163,184,.12); }.newsroom-article-reactions > button,.newsroom-article-reactions > span button { min-width:36px;height:34px;border:1px solid rgba(148,163,184,.16);border-radius:999px;color:#fff;background:#111925;cursor:pointer; }.newsroom-article-reactions > button.mine { border-color:#c9d0d9;background:rgba(201,208,217,.1); }.newsroom-article-reactions > span { display:flex;gap:4px;margin-left:4px; }.newsroom-article-reactions img { width:20px;height:20px;object-fit:contain; }.newsroom-comments { max-width:980px;margin-top:20px; }.newsroom-comments > header { display:flex;justify-content:space-between;padding-bottom:10px;color:#e2e8f0; }.newsroom-comments > header span { color:#7f8b9d; }.newsroom-comment { padding:14px 0;border-top:1px solid rgba(148,163,184,.12); }.newsroom-comment > small { display:block;margin-bottom:7px;color:#bcd6f0; }.newsroom-comment > div { display:flex;align-items:center;justify-content:space-between;gap:10px; }.newsroom-comment time { color:#718096;font-size:9px; }.newsroom-comment > p { margin:8px 0;color:#cbd5e1;line-height:1.55; }.newsroom-comment footer { display:flex;gap:8px; }.newsroom-comment footer button { padding:0;border:0;color:#bcd6f0;background:transparent;font-size:9px;font-weight:900;cursor:pointer; }.newsroom-comment-composer { display:grid;gap:9px;margin-top:16px;padding:14px;border:1px solid rgba(39,151,255,.15);border-radius:12px;background:#0c131e; }.newsroom-comment-composer > div { display:flex;justify-content:space-between;color:#bcd6f0; }.newsroom-comment-composer textarea { min-height:80px;padding:12px;border:1px solid rgba(148,163,184,.18);border-radius:9px;color:#fff;background:#060a10;resize:vertical; }.newsroom-comment-composer > button { justify-self:end;padding:10px 14px;border:0;border-radius:8px;color:#161006;background:#c9d0d9;font-weight:1000;cursor:pointer; }
      @media (max-width:720px) {
      ...
      /* v37.1 — League Hub mobile containment hotfix. */
      @media (max-width:720px) {
        .network-page,
        .network-page .network-layout,
        .network-page .network-sidebar,
        .network-page .network-stage { min-width:0!important;max-width:100%!important;overflow-x:hidden!important; }
        .network-page .network-channel-list { padding:5px 8px 20px!important; }
        .network-page .network-channel-list > button.matchup {
          width:100%!important;min-height:64px!important;display:grid!important;
          grid-template-columns:68px minmax(0,1fr) auto!important;gap:8px!important;
          align-items:center!important;margin:2px 0!important;padding:8px 9px!important;overflow:hidden!important;
        }
        .network-page .network-matchup-logos {
          width:68px!important;min-width:68px!important;height:40px!important;display:flex!important;
          flex-direction:row!important;flex-wrap:nowrap!important;align-items:center!important;
          justify-content:center!important;gap:3px!important;overflow:hidden!important;
        }
        .network-page .network-matchup-logos > span {
          width:27px!important;min-width:27px!important;height:27px!important;display:grid!important;place-items:center!important;
        }
        .network-page .network-matchup-logos img {
          width:27px!important;max-width:27px!important;height:27px!important;max-height:27px!important;object-fit:contain!important;
        }
        .network-page .network-matchup-logos > b { flex:0 0 auto;font-size:7px!important; }
        .network-page .network-matchup-copy { min-width:0!important;display:grid!important;gap:2px!important; }
        .network-page .network-matchup-copy strong,
        .network-page .network-matchup-copy small {
          max-width:100%!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;
        }
        .network-page .network-matchup-copy strong { font-size:14px!important; }
        .network-page .network-matchup-copy small { font-size:9px!important; }
        .network-page .network-channel-list > button.matchup > em {
          position:static!important;align-self:center!important;justify-self:end!important;margin:0!important;
          padding:4px 5px!important;border:1px solid rgba(201,208,217,.22);border-radius:999px;
          font-size:6px!important;line-height:1!important;white-space:nowrap!important;
        }
        .network-page .network-composer {
          box-sizing:border-box!important;width:100%!important;min-width:0!important;display:grid!important;
          grid-template-columns:minmax(0,1fr) 44px!important;align-items:end!important;gap:6px!important;
          padding:8px 9px calc(8px + env(safe-area-inset-bottom))!important;overflow:visible!important;
        }
        .network-page .network-compose-box { min-width:0!important;width:100%!important;height:44px!important; }
        .network-page .network-compose-box textarea {
          box-sizing:border-box!important;width:100%!important;min-width:0!important;height:44px!important;
          min-height:44px!important;max-height:88px!important;padding:11px 42px 9px 13px!important;
          overflow-y:auto!important;border-radius:22px!important;font-size:13px!important;line-height:20px!important;
        }
        .network-page .network-emoji-toggle {
          right:6px!important;bottom:5px!important;width:34px!important;min-width:34px!important;
          height:34px!important;min-height:34px!important;padding:0!important;border-radius:50%!important;font-size:18px!important;
        }
        .network-page .network-send {
        ...
        .network-page .network-send::after { font-size:16px!important; }
        .network-page .network-replying { grid-column:1/-1!important;min-width:0!important; }
        .network-page .network-composer > small { display:none!important; }
      }
      @media (max-width:430px) {
        .network-page .network-channel-list > button.matchup {
          grid-template-columns:62px minmax(0,1fr)!important;
          grid-template-areas:"logos copy" "logos tag"!important;
        }
        .network-page .network-matchup-logos { grid-area:logos;width:62px!important;min-width:62px!important; }
        .network-page .network-matchup-copy { grid-area:copy; }
        .network-page .network-channel-list > button.matchup > em {
          grid-area:tag;justify-self:start!important;margin-top:1px!important;
        }
      }
      /* v38 — calmer navigation hierarchy and focused Home experience. */
      .drawer-menu-group { padding:10px!important;border-color:rgba(148,163,184,.12)!important;border-radius:13px!important;background:rgba(7,12,20,.72)!important; }
      .drawer-menu-grid { grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:7px!important; }
      .drawer-nav-tile { min-height:60px!important;padding:10px 12px!important;border-color:rgba(148,163,184,.2)!important;border-radius:11px!important;background:linear-gradient(135deg,#0c1420,#09101a)!important;box-shadow:none!important;animation:none!important; }
      .drawer-nav-tile:hover { transform:translateY(-1px)!important;filter:brightness(1.08)!important;box-shadow:0 10px 22px rgba(0,0,0,.24)!important; }
      .drawer-nav-tile.active { border-color:rgba(201,208,217,.52)!important;background:linear-gradient(135deg,#171a22,#111722)!important;box-shadow:inset 0 0 0 1px rgba(201,208,217,.12)!important; }
      ...
      .home-pulse-v38 { overflow:hidden; }.home-pulse-v38 > header,.home-rankings-v38 > header { display:flex;align-items:center;justify-content:space-between;gap:16px;padding:17px 20px;border-bottom:1px solid rgba(148,163,184,.12); }.home-pulse-v38 > header span,.home-rankings-v38 > header span,.home-matchup-v38 > div:first-child > span,.cfb-center-hero-v38 span,.cfb-center-group-v38 header > span { color:#c9d0d9;font-size:8px;font-weight:1000;letter-spacing:.16em; }.home-pulse-v38 h2,.home-rankings-v38 h2 { margin:4px 0 0;font-size:23px;font-family:var(--cfb-display);font-weight:600; }.home-pulse-v38 > header small { padding:5px 8px;border-radius:999px;color:#cbd5e1;background:#121c29;font-weight:900; }.home-pulse-v38 > div { display:grid;grid-template-columns:repeat(3,minmax(0,1fr)); }.home-pulse-v38 > div > button { min-width:0;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:4px 12px;padding:18px;border:0;border-right:1px solid rgba(148,163,184,.12);color:var(--cfb-text);text-align:left;background:transparent;cursor:pointer;transition:background .15s ease; }.home-pulse-v38 > div > button:last-child { border-right:0; }.home-pulse-v38 > div > button:hover { background:rgba(62,127,193,.06); }.home-pulse-v38 button > span { color:#bcd6f0;font-size:8px;font-weight:1000;letter-spacing:.13em; }.home-pulse-v38 button > strong { grid-row:1/3;grid-column:2;align-self:center;font-size:26px;font-family:var(--cfb-display);font-weight:600;font-variant-numeric:tabular-nums; }.home-pulse-v38 button > small { color:#8fa0b5;font-size:9px; }.home-pulse-v38 button > p { grid-column:1/-1;margin:7px 0 0;overflow:hidden;color:#b1bdcc;font-size:11px;line-height:1.4;text-overflow:ellipsis;white-space:nowrap; }.home-pulse-v38 button.live { background:radial-gradient(circle at 100% 0%,rgba(220,38,38,.18),transparent 55%); }.home-pulse-v38 button.live > span { color:#f87171; }
      .home-matchup-v38 { display:grid;grid-template-columns:minmax(160px,.45fr) minmax(0,1.3fr) auto;align-items:center;gap:18px;padding:18px 20px;border-left:4px solid var(--cfb-red); }.home-matchup-v38 h2 { margin:4px 0;font-size:25px;font-family:var(--cfb-display);font-weight:600; }.home-matchup-v38 small { color:#8fa0b5; }.home-matchup-teams-v38 { min-width:0;display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;gap:14px; }.home-matchup-teams-v38 > span { min-width:0;display:flex;align-items:center;gap:10px; }.home-matchup-teams-v38 > span:last-child { justify-content:flex-end;text-align:right; }.home-matchup-teams-v38 strong { overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-family:var(--cfb-display);font-weight:600; }.home-matchup-teams-v38 > b { color:var(--cfb-muted);font-size:11px;font-family:var(--cfb-display);font-weight:700; }.home-matchup-v38 > button,.home-rankings-v38 > header > button,.cfb-center-hero-v38 > button { padding:10px 13px;border:1px solid rgba(62,127,193,.35);border-radius:9px;color:var(--cfb-text);background:rgba(62,127,193,.1);font-size:10px;font-family:var(--cfb-display);font-weight:600;letter-spacing:.03em;cursor:pointer; }
      ...
      .home-rankings-v38 { overflow:hidden; }.home-rankings-v38 > header p { margin:3px 0 0;color:#8fa0b5;font-size:11px; }.home-rankings-v38 > div { display:grid; }.home-rankings-v38 > div > button { display:grid;grid-template-columns:45px 42px minmax(0,1fr) 70px 70px;align-items:center;gap:10px;padding:11px 18px;border:0;border-bottom:1px solid rgba(148,163,184,.1);color:var(--cfb-text);text-align:left;background:transparent;cursor:pointer;transition:background .15s ease; }.home-rankings-v38 > div > button:last-child { border-bottom:0; }.home-rankings-v38 > div > button:hover { background:rgba(62,127,193,.06); }.home-rankings-v38 > div > button > b { display:flex;align-items:center;justify-content:center;width:32px;height:26px;color:var(--cfb-panel);background:var(--cfb-gold);clip-path:polygon(0 0,100% 0,100% 70%,80% 100%,0 100%);font-family:var(--cfb-display);font-weight:700;font-size:14px;font-variant-numeric:tabular-nums; }.home-rankings-v38 > div > button:first-child > b { background:linear-gradient(135deg,var(--cfb-red),var(--cfb-red-dark));color:#fff; }.home-rankings-v38 > div > button > span { min-width:0;display:grid;gap:2px; }.home-rankings-v38 > div > button strong,.home-rankings-v38 > div > button small { overflow:hidden;text-overflow:ellipsis;white-space:nowrap; }.home-rankings-v38 > div > button strong { font-family:var(--cfb-display);font-weight:600; }.home-rankings-v38 > div > button small { color:#8090a4;font-size:9px; }.home-rankings-v38 > div > button > em { color:#cbd5e1;font-style:normal;text-align:center;font-family:var(--cfb-display);font-variant-numeric:tabular-nums; }.home-rankings-v38 > div > button > i { color:#bcd6f0;font-size:15px;font-style:normal;font-weight:700;text-align:right;font-family:var(--cfb-display);font-variant-numeric:tabular-nums; }
      .cfb-center-v38 { display:grid;gap:15px; }.cfb-center-hero-v38 { display:flex;align-items:center;justify-content:space-between;gap:24px;padding:28px;border-top:4px solid var(--cfb-red);background:radial-gradient(circle at 82% 0%,rgba(62,127,193,.12),transparent 34%),linear-gradient(125deg,#0a1019,#111827); }.cfb-center-hero-v38 h1 { margin:6px 0;font-size:clamp(40px,5vw,68px);line-height:.95;letter-spacing:-.03em; }.cfb-center-hero-v38 p { max-width:780px;margin:0;color:#9eabba;font-size:14px;line-height:1.5; }.cfb-center-groups-v38 { display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;align-items:start; }.cfb-center-group-v38 { overflow:hidden; }.cfb-center-group-v38 > header { padding:18px 19px;border-bottom:1px solid rgba(148,163,184,.12); }.cfb-center-group-v38 h2 { margin:5px 0 3px;font-size:22px; }.cfb-center-group-v38 header p { margin:0;color:#8493a6;font-size:11px;line-height:1.45; }.cfb-center-group-v38 > div { display:grid; }.cfb-center-group-v38 > div > button { display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px 18px;border:0;border-bottom:1px solid rgba(148,163,184,.1);color:var(--cfb-text);text-align:left;background:transparent;cursor:pointer;transition:background .15s ease; }.cfb-center-group-v38 > div > button:last-child { border-bottom:0; }.cfb-center-group-v38 > div > button:hover { background:rgba(62,127,193,.06); }.cfb-center-group-v38 > div > button span { min-width:0;display:grid;gap:3px; }.cfb-center-group-v38 > div > button strong { font-size:14px;font-family:var(--cfb-display);font-weight:600; }.cfb-center-group-v38 > div > button small { color:#8493a6;font-size:10px; }.cfb-center-group-v38 > div > button b { color:#bcd6f0;font-size:24px;font-family:var(--cfb-display);font-weight:600; }
      ...
      @media (max-width:720px) {
        ...
        .drawer-nav-tile { min-height:58px!important; }
        .home-pulse-v38 > div { grid-template-columns:1fr; }.home-pulse-v38 > div > button { border-right:0;border-bottom:1px solid rgba(148,163,184,.12); }.home-pulse-v38 > div > button:last-child { border-bottom:0; }
        .home-matchup-v38 { grid-template-columns:1fr;gap:13px;padding:16px; }.home-matchup-teams-v38 { gap:8px; }.home-matchup-teams-v38 > span { align-items:center;flex-direction:column;text-align:center; }.home-matchup-teams-v38 > span:last-child { justify-content:center;text-align:center; }.home-matchup-teams-v38 strong { max-width:130px;white-space:normal;text-align:center;line-height:1.2; }.home-matchup-v38 > button { width:100%; }
        .home-newsroom-rail > div { grid-template-columns:1fr!important; }.home-newsroom-rail article.open { grid-column:1!important; }
        .home-rankings-v38 > header { align-items:flex-start;flex-direction:column; }.home-rankings-v38 > header > button { width:100%; }.home-rankings-v38 > div > button { grid-template-columns:36px 38px minmax(0,1fr) 48px;padding:10px 12px; }.home-rankings-v38 > div > button > em { display:none; }
        .cfb-center-hero-v38 { align-items:flex-start;flex-direction:column;padding:22px 18px; }.cfb-center-hero-v38 h1 { font-size:45px; }.cfb-center-hero-v38 > button { width:100%; }.cfb-center-groups-v38 { grid-template-columns:1fr; }.cfb-center-group-v38 > div > button { padding:14px 15px; }.cfb-center-group-v38 > div > button strong { font-size:15px; }.cfb-center-group-v38 > div > button small { white-space:normal;line-height:1.35; }
      }
      ...
      .cfb-identity-brand { min-width: 174px !important; }
      .cfb-identity-progress { min-width: 176px !important; }
      .cfb-identity-featured { min-width: 184px !important; }
      .cfb-identity-brand > span { width:34px !important; height:34px !important; }
      .cfb-identity-item b, .cfb-identity-featured b { font-size:12px !important; }

      /* Header search belongs to the product header, not as a tiny floating field. */
      ...
      /* True Discord desktop proportions. */
      .discord-clone-page {
        max-width: none !important;
        gap: 0 !important;
        padding: 0 !important;
      ...
      .discord-clone-page .network-hero {
        display: none !important;
      }
      .discord-clone-page .network-mobile-hub-header {
        display: none !important;
      }
      .discord-clone-page .network-layout {
        display: grid !important;
        grid-template-columns: 68px 260px minmax(0,1fr) 248px !important;
        min-height: min(78vh, 900px) !important;
        height: calc(100vh - 190px) !important;
        max-height: 920px !important;
        background: var(--discord-chat) !important;
      }
      ...
      .network-server-rail > i {
        width: 32px;
        height: 2px;
        border-radius: 999px;
        background: rgba(255,255,255,.08);
      }
      .network-server-rail button {
        position: relative;
        width: 44px;
        min-width: 44px;
        height: 44px;
      ...
      .network-server-rail button:hover,
      .network-server-rail button.active {
        border-radius: 14px;
        background: var(--discord-blurple);
        color: #fff;
      }
      .network-server-rail button.active::before {
        content:"";
        position:absolute;
        left:-8px;
        width:4px;
      ...
      .network-server-rail button:first-child span {
        display:grid;
        place-items:center;
        width:100%;
        height:100%;
      ...
      .discord-clone-page .network-sidebar {
        ...
        max-width: none !important;
        background: var(--discord-panel) !important;
        border: 0 !important;
        border-right: 1px solid rgba(0,0,0,.22) !important;
        border-radius: 0 !important;
        overflow: hidden auto !important;
      }
      .discord-clone-page .network-mobile-directory-title {
      ...
      .discord-clone-page .network-mobile-directory-title span {
        display:none;
      }
      ...
      .discord-clone-page .network-sidebar > nav button.active,
      .discord-clone-page .network-sidebar > nav button:hover {
        background:var(--discord-selected) !important;
      ...
      .network-sidebar-tools { opacity:.75; }
      .network-channel-list { padding:0 6px 12px !important; }
      .network-channel-category {
      ...
      .network-channel-list > button:hover {
        background:var(--discord-hover) !important;
        color:#dbdee1 !important;
      }
      .network-channel-list > button.active {
        background:var(--discord-selected) !important;
        color:var(--discord-text) !important;
      }
      ...
      .network-channel-list > button small {
        display:none !important;
      }
      ...
      .network-stage-header .network-stage-channel {
        gap:9px !important;
      }
      ...
      .network-stage-header p,
      .network-stage-header span {
        display:none !important;
      }
      .network-stage-actions button {
      ...
      .network-channel-utilities label {
      ...
      .network-channel-utilities > button,
      .network-search-filter-toggle {
        min-height:32px !important;
      ...
      .network-channel-utilities > button:hover,
      .network-search-filter-toggle:hover {
        color:#fff !important;
        background:var(--discord-hover) !important;
      }
      ...
      .network-search-popover footer {
        display:flex;
        align-items:center;
        justify-content:space-between;
        gap:8px;
      }
      .network-search-popover header button {
        min-height:28px;
        width:28px;
        border:0;
        background:transparent;
      ...
      .network-search-popover label {
        display:grid !important;
        gap:5px;
        width:100% !important;
        background:transparent !important;
      }
      .network-search-popover label > span {
        color:var(--discord-text);
        font-size:12px;
        font-weight:800;
      }
      .network-search-popover input,
      .network-search-popover select {
        width:100%;
      ...
      .network-search-popover footer button {
        flex:1;
        min-height:36px;
        border:0;
        border-radius:4px;
      ...
      .network-search-popover footer button:first-child {
        background:transparent;
        color:var(--discord-muted);
      }
      ...
      .network-message-feed article:hover {
        background:rgba(2,2,3,.08) !important;
      }
      .network-message-feed article time {
        color:var(--discord-muted) !important;
        font-size:10px !important;
      }
      .network-message-head strong {
        font-size:14px !important;
      }
      .network-message-feed article p {
        margin:2px 0 0 !important;
        color:#dbdee1 !important;
        font-size:14px !important;
        line-height:1.38 !important;
      }
      .network-message-media {
        max-width:min(520px,100%) !important;
        max-height:430px !important;
        border-radius:8px !important;
      }
      .network-reaction-row button {
        min-height:28px !important;
      ...
      .network-reply-context {
        ...
        border-left:2px solid #4e5058 !important;
        background:transparent !important;
      }
      .network-message-toolbar {
        top:-18px !important;
      ...
      .network-gif-toggle:hover,
      .network-emoji-toggle:hover,
      .network-send:hover {
        background:rgba(255,255,255,.06) !important;
        color:#fff !important;
      }
      .network-composer small { display:none !important; }
      ...
      .network-member-rail > header {
        min-height:30px !important;
        color:var(--discord-muted) !important;
        font-size:10px !important;
      }
      .network-member-rail > div > button {
      ...
      .network-member-rail > div > button:hover {
        background:var(--discord-hover) !important;
      }
      .network-member-rail em {
        font-size:9px !important;
      }
      .network-members-toggle { display:none !important; }

      /* Coach profiles: one clean horizontal row per team, no broken wrapping. */
      ...
      .coach-menu-logo-plate-v86 img {
        max-width:46px !important;
        max-height:46px !important;
      }
      .coach-drawer-tile strong {
        display:block !important;
        overflow:hidden !important;
      ...
      /* Mobile behaves like Discord: channel directory, then full chat, then members drawer. */
      @media (max-width: 900px) {
        .discord-clone-page {
        ...
        .network-mobile-directory .network-sidebar {
          display:block !important;
          z-index:3;
        }
        .network-mobile-directory .network-stage,
        .network-mobile-directory .network-member-rail {
          display:none !important;
        }
        .network-mobile-chat .network-sidebar {
          display:none !important;
        }
        .network-mobile-chat .network-stage {
          display:grid !important;
          z-index:3;
        }
        .network-mobile-chat .network-member-rail {
          display:none !important;
        }
        .network-mobile-members .network-sidebar,
        .network-mobile-members .network-stage {
          display:none !important;
        }
        .network-mobile-members .network-member-rail {
          display:block !important;
          z-index:4;
          padding:16px 14px !important;
        }
        .network-members-toggle {
      ...
      @media (max-width: 560px) {
        ...
        .cfb-identity-progress,
        .cfb-identity-featured {
          display:none !important;
        }
        .network-channel-utilities {
          display:grid !important;
      ...
      .cfb-app h1,.cfb-app h2,.cfb-app h3{font-family:var(--cfb-display)!important;font-weight:600!important;letter-spacing:.005em!important}
      .cfb-app{font-variant-numeric:tabular-nums}
      .cfb-identity-bar{display:grid!important;grid-template-columns:minmax(190px,1.2fr) repeat(3,minmax(120px,.75fr)) minmax(180px,1fr) minmax(190px,1fr)!important;gap:1px!important;overflow:hidden!important;min-height:72px!important;border:1px solid rgba(201,208,217,.2)!important;border-radius:12px!important;background:var(--cfb-panel)!important;box-shadow:var(--ui-shadow)!important}
      ...
      .cfb-identity-brand>span{width:44px!important;height:44px!important;color:#fff!important;background:linear-gradient(145deg,var(--cfb-red-dark),var(--cfb-red))!important;box-shadow:0 0 0 1px rgba(201,208,217,.45),0 10px 24px rgba(62,127,193,.28)!important;clip-path:polygon(18% 0,100% 0,100% 82%,82% 100%,0 100%,0 18%)!important;font-family:var(--cfb-display)!important;font-weight:700!important}
      .cfb-identity-item small,.cfb-identity-featured small,.cfb-identity-brand small{color:rgba(226,232,240,.5)!important;font-size:9.5px!important;font-family:var(--cfb-display)!important;font-weight:600!important;letter-spacing:.12em!important;text-transform:uppercase!important}.cfb-identity-item b,.cfb-identity-featured b{font-family:var(--cfb-display)!important;font-weight:600!important;font-size:14px!important;margin-top:4px!important;font-variant-numeric:tabular-nums!important}.cfb-identity-progress span{height:6px!important;margin-top:9px!important}.cfb-identity-featured{border-left:2px solid var(--cfb-gold)!important}
      ...
      .drawer-menu-group:last-child .drawer-menu-grid{grid-template-columns:1fr!important;gap:10px!important}.coach-drawer-tile{grid-template-columns:72px minmax(0,1fr)!important;align-items:center!important;min-height:94px!important;padding:10px 16px 10px 22px!important;border-radius:14px!important;background:linear-gradient(145deg,var(--cfb-panel),#07101d)!important;box-shadow:0 12px 28px rgba(0,0,0,.20)!important}.coach-menu-logo-plate-v86{width:64px!important;height:64px!important;min-width:64px!important;border-radius:15px!important}.coach-nav-copy{display:block!important;min-width:0!important;text-align:left!important}.coach-nav-copy strong,.coach-nav-copy small,.coach-nav-copy em{display:block!important;min-width:0!important}.coach-nav-copy strong{overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;font-size:18px!important;line-height:1.12!important;color:#fff!important}.coach-nav-copy small{margin-top:5px!important;font-size:13px!important;color:rgba(255,255,255,.78)!important}.coach-nav-copy em{margin-top:7px!important;font-size:10px!important;font-style:normal!important;color:rgba(201,208,217,.72)!important;letter-spacing:.04em!important}.coach-drawer-tile>img{display:none!important}
      ...
      /* v53-logo-schedule-network-organizer */
      ...
      .cfbn-row-v53:hover{background:#252d39!important}
      .cfbn-row-v53.active{background:#303a49!important;color:#fff!important}
      .cfbn-leading-v53{width:42px!important;display:grid!important;place-items:center!important}
      .cfbn-hash-v53{color:#798596!important;font-size:22px!important;line-height:1!important}
      .cfbn-copy-v53{min-width:0!important;overflow:hidden!important}
      .cfbn-copy-v53 strong{
        display:block!important;
        overflow:hidden!important;
        text-overflow:ellipsis!important;
        white-space:nowrap!important;
      ...
      .cfbn-row-v53.logo-only-matchup{
        grid-template-columns:minmax(0,1fr) 70px!important;
        min-height:58px!important;
        padding:7px 8px 7px 12px!important;
      }
      .cfbn-logo-matchup-v53{
        display:flex!important;
        align-items:center!important;
        justify-content:flex-start!important;
        gap:10px!important;
      ...
      .cfbn-logo-matchup-v53>b{color:#788496!important;font-size:8px!important;font-weight:900!important}
      .cfbn-team-logo-v53{
        width:34px!important;
        height:34px!important;
        display:grid!important;
      ...
      .cfbn-team-logo-v53 img{width:28px!important;height:28px!important;object-fit:contain!important;display:block!important}
      .cfbn-team-logo-v53 i{width:14px!important;height:14px!important;border-radius:50%!important;background:#334155!important}
      ...
      .network-organizer-v53>header span{color:#e0bd4b!important;font-size:10px!important;font-weight:950!important;letter-spacing:.12em!important}
      .network-organizer-v53>header h2{margin:5px 0!important;font-size:28px!important}
      .network-organizer-v53>header p{margin:0!important;color:#91a0b5!important;font-size:13px!important}
      .network-organizer-v53>header>button{
        min-height:38px!important;
      ...
      .network-organizer-category-v53{
        overflow:hidden!important;
        border:1px solid rgba(148,163,184,.14)!important;
        border-radius:12px!important;
        background:#101827!important;
      }
      .network-organizer-category-v53.muted{opacity:.9!important}
      .network-organizer-category-v53>header{
        display:grid!important;
        grid-template-columns:28px minmax(0,1fr) auto!important;
        align-items:center!important;
        gap:10px!important;
      ...
      .network-organizer-category-v53>header strong,
      .network-organizer-category-v53>header small{display:block!important}
      .network-organizer-category-v53>header strong{font-size:15px!important}
      .network-organizer-category-v53>header small{margin-top:3px!important;color:#8190a6!important;font-size:10px!important}
      .network-organizer-category-v53>div{display:grid!important}

      .network-organizer-channel-v53{
        display:grid!important;
        grid-template-columns:28px minmax(0,1fr) minmax(130px,180px) auto!important;
        align-items:center!important;
        gap:10px!important;
        min-height:58px!important;
        padding:8px 12px!important;
      ...
      .network-organizer-channel-v53:last-child{border-bottom:0!important}
      .network-organizer-grip-v53{color:#526075!important;font-size:16px!important;letter-spacing:-.18em!important}
      .network-organizer-channel-copy-v53{min-width:0!important}
      .network-organizer-channel-copy-v53 strong,
      .network-organizer-channel-copy-v53 small{
        display:block!important;
        overflow:hidden!important;
        text-overflow:ellipsis!important;
        white-space:nowrap!important;
      }
      .network-organizer-channel-copy-v53 strong{font-size:13px!important}
      .network-organizer-channel-copy-v53 small{margin-top:3px!important;color:#7f8da2!important;font-size:10px!important}
      .network-organizer-channel-v53 select{
        width:100%!important;
        min-width:0!important;
        min-height:36px!important;
        border:1px solid rgba(148,163,184,.16)!important;
      ...
      .network-organizer-actions-v53{display:flex!important;gap:5px!important}
      .network-organizer-actions-v53 button{
        width:34px!important;
      ...
      .network-organizer-actions-v53 button:hover:not(:disabled){border-color:rgba(224,189,75,.55)!important;background:#242d3a!important}
      .network-organizer-actions-v53 button:disabled{opacity:.28!important;cursor:not-allowed!important}

      @media(max-width:900px){
        .network-organizer-v53{padding:16px 12px 100px!important}
        .network-organizer-v53>header h2{font-size:23px!important}
        .network-organizer-channel-v53{
          grid-template-columns:24px minmax(0,1fr) auto!important;
          grid-template-areas:"grip copy actions" ". select select"!important;
          gap:7px 9px!important;
          padding:10px!important;
        }
        .network-organizer-channel-v53>.network-organizer-grip-v53{grid-area:grip!important}
        .network-organizer-channel-copy-v53{grid-area:copy!important}
        .network-organizer-channel-v53>select{grid-area:select!important}
        .network-organizer-channel-v53>.network-organizer-actions-v53{grid-area:actions!important}
        .cfbn-row-v53.logo-only-matchup{min-height:62px!important}
        .cfbn-team-logo-v53{width:38px!important;height:38px!important}
        .cfbn-team-logo-v53 img{width:32px!important;height:32px!important}
      }
      ...
      .cfb-universal-search-results>button:focus-visible{
        border-color:rgba(226,190,75,.28)!important;
        background:#142137!important;
      }
      .cfb-universal-search-results>button>span:nth-child(2){
        min-width:0!important;
        overflow:hidden!important;
      }
      ...
      .discord-clone-page{
        ...
        --discord-hover:#17243a!important;
        --discord-text:#f4f7fb!important;
      ...
      .network-search-wrap label:focus-within,
      .network-channel-utilities label:focus-within{
        border-color:rgba(226,190,75,.48)!important;
        box-shadow:0 0 0 3px rgba(226,190,75,.08)!important;
      }
      ...
      .network-search-wrap input::placeholder,
      .network-channel-utilities input::placeholder{color:#718096!important}
      .network-search-filter-toggle,
      .network-channel-utilities>button{
        border:1px solid rgba(255,255,255,.10)!important;
        border-radius:9px!important;
      ...
      .network-empty-state h2{color:#f4f7fb!important}
      .network-composer{
        border:1px solid rgba(255,255,255,.08)!important;
        background:#162237!important;
        box-shadow:0 10px 28px rgba(0,0,0,.20)!important;
      }
      .network-compose-box textarea{
        color:#fff!important;
        background:transparent!important;
        font-size:16px!important;
        line-height:1.45!important;
      }
      .network-compose-box textarea::placeholder{color:#8996aa!important}
      .network-composer-tools button{
      ...
      .cfb-nav-drawer-v54 .drawer-menu-grid{
        display:grid!important;
      }
      .cfb-nav-drawer-v54 .drawer-primary-group{
        display:block!important;
      }
      .cfb-nav-drawer-v54{
        background:linear-gradient(180deg,#07101d,#050914)!important;
        border-right:1px solid rgba(226,190,75,.22)!important;
      }
      .drawer-menu-search input{
        font-size:16px!important;
      }

      /* Cleaner mobile league status header, no harsh square outlines. */
      .cfb-identity-bar{
        border:1px solid rgba(226,190,75,.22)!important;
        border-radius:18px!important;
        background:linear-gradient(150deg,#091221,#050914)!important;
      ...
      .cfb-identity-brand>span{
        ...
        border:1px solid rgba(226,190,75,.34)!important;
        color:#f3d36b!important;
        box-shadow:none!important;
      }
      .cfb-identity-item small,
      .cfb-identity-featured small,
      .cfb-identity-brand small{color:#8390a4!important}
      .cfb-identity-item b,
      .cfb-identity-featured b,
      .cfb-identity-brand b{color:#fff!important}

      /* Coach rows: preserve full hierarchy without giant tiles. */
      .coach-row-v52{
        min-height:76px!important;
        grid-template-columns:4px 54px minmax(0,1fr) 22px!important;
        gap:12px!important;
      ...
      .coach-copy-v52 em{font-size:9px!important}
      ...
      @media(max-width:900px){
        ...
        .cfb-identity-bar>*+*{border-left:0!important}
        .cfb-identity-bar>.cfb-identity-item:nth-child(even),
        .cfb-identity-bar>.cfb-identity-featured{
        ...
        .network-mobile-hub-header>button{
          width:44px!important;
          height:44px!important;
        ...
        .network-mobile-hub-header strong{font-size:18px!important}
        .network-mobile-hub-header small{font-size:12px!important;color:#91a0b6!important}
        .network-channel-utilities{
          padding:9px 12px!important;
          gap:8px!important;
        }
        .network-search-wrap,
        .network-search-wrap label{width:100%!important}
        .network-search-filter-toggle{display:none!important}
        ...
        .coach-row-v52{
          min-height:74px!important;
          grid-template-columns:4px 52px minmax(0,1fr) 18px!important;
        }
      }
      ...
      @media(max-width:560px){
        ...
        .cfb-universal-search-results em{display:none!important}
        .cfb-search-result-logo{
...
function LeagueIdentityBar({ currentYear, currentWeek, advanceAt, weeklyMatchups = [], results = [], teams = [], setActiveTab }) {
  const weekGames = (weeklyMatchups || []).filter((row)=>
    String(row.season_year)===String(currentYear) &&
    String(row.week)===String(currentWeek)
  );
  const weekResults = (results || []).filter((row)=>
    String(row.season_year)===String(currentYear) &&
    String(row.week)===String(currentWeek)
  );
  const completed = weekGames.filter((game)=>weekResults.some((result)=>
    (String(result.team_1_id)===String(game.team_1_id) && String(result.team_2_id)===String(game.team_2_id)) ||
    (String(result.team_1_id)===String(game.team_2_id) && String(result.team_2_id)===String(game.team_1_id))
  )).length;
  const featured = weekGames.find((row)=>row.is_game_of_week);
  const team1 = teams.find((team)=>String(team.id)===String(featured?.team_1_id));
  const team2 = teams.find((team)=>String(team.id)===String(featured?.team_2_id));
  const advanceLabel = advanceAt ? new Date(advanceAt).toLocaleString([], { month:"short", day:"numeric", hour:"numeric", minute:"2-digit" }) : "Not scheduled";

  return (
    <section className="cfb-identity-bar" aria-label="League status">
      <button type="button" className="cfb-identity-brand" onClick={()=>setActiveTab("dashboard")}>
        <span>CFB</span>
        <div><b>CFBElite27</b><small>League status</small></div>
      </button>
      <div className="cfb-identity-item">
        <small>Season</small>
        <b>{currentYear}</b>
      </div>
      <div className="cfb-identity-item">
      ...
      <button type="button" className="cfb-identity-item cfb-identity-progress" onClick={()=>setActiveTab("schedule")}>
        <small>User games complete</small>
        <b>{completed}/{weekGames.length}</b>
        <span><i style={{width:`${weekGames.length ? Math.min(100,(completed/weekGames.length)*100) : 0}%`}}/></span>
      </button>
      <button type="button" className="cfb-identity-featured" onClick={()=>setActiveTab("schedule")}>
        <small>Game of the Week</small>
        <b>{featured ? `${getTeamAbbreviation(team1)} vs ${getTeamAbbreviation(team2)}` : "Not selected"}</b>
      </button>
    </section>
  );
}

function AppSkeleton({ visible }) {
  if (!visible) return null;
  return (
    <section className="cfb-app-skeleton" aria-label="Loading dashboard">
      <div className="cfb-skeleton-line cfb-skeleton-title"/>
      <div className="cfb-skeleton-line cfb-skeleton-subtitle"/>
      <div className="cfb-skeleton-grid">
...
function UniversalSearchBar({ tabs = [], teams = [], users = [], assignments = [], weeklyMatchups = [], results = [], currentYear, setActiveTab }) {
  const [query, setQuery] = useState("");
  ...
  const resultsList = useMemo(() => {
    ...
    tabs.forEach(([key, label]) => {
      if (String(label || "").toLowerCase().includes(normalized)) {
        items.push({ key:`page-${key}`, type:"Page", title:label, subtitle:"Open dashboard section", action:()=>setActiveTab(key) });
      }
    });

    teams.forEach((team) => {
      const assignment =
        assignments.find((row)=>String(row.team_id)===String(team.id) && row.status==="Active") ||
        assignments.find((row)=>String(row.team_id)===String(team.id));
      const coach = users.find((user)=>String(user.id)===String(assignment?.discord_user_id));
      const haystack = `${team.name || ""} ${team.abbreviation || ""} ${team.conference || ""} ${coach?.discord_username || ""}`.toLowerCase();
      if (haystack.includes(normalized)) {
        items.push({
          key:`team-${team.id}`,
          type:"Team",
          title:team.name,
          subtitle:coach?.discord_username ? `${coach.discord_username} • ${team.conference || "Conference TBD"}` : team.conference || "Team page",
          logo:team.logo_url,
          action:()=>setActiveTab(assignment?.discord_user_id ? `coach-${assignment.discord_user_id}` : `team-${team.id}`),
        });
      }
    });

    users.forEach((user) => {
      if (!String(user.discord_username || "").toLowerCase().includes(normalized)) return;
      const assignment = assignments.find((row)=>String(row.discord_user_id)===String(user.id) && row.status==="Active");
      const team = teams.find((item)=>String(item.id)===String(assignment?.team_id));
      items.push({
        key:`coach-${user.id}`,
        type:"Coach",
        title:user.discord_username,
        subtitle:team?.name || "Coach profile",
        logo:team?.logo_url,
        action:()=>setActiveTab(`coach-${user.id}`),
      });
    });

    weeklyMatchups.forEach((row) => {
      if (String(row.season_year) !== String(currentYear)) return;
      const team1 = row.team_1 || teams.find((team)=>String(team.id)===String(row.team_1_id));
      const team2 = row.team_2 || teams.find((team)=>String(team.id)===String(row.team_2_id));
      const label = `${row.week || ""} ${team1?.name || ""} ${team2?.name || ""}`.toLowerCase();
      if (label.includes(normalized)) {
        items.push({
          key:`game-${row.id || `${row.week}-${row.team_1_id}-${row.team_2_id}`}`,
          type:"Game",
          title:`${team1?.name || "Team TBD"} vs ${team2?.name || "Team TBD"}`,
          subtitle:`${row.week || "Week TBD"} • GameCenter`,
  ...
  }, [normalized, tabs, teams, users, assignments, weeklyMatchups, currentYear, setActiveTab]);
  ...
  return (
    <div className="cfb-universal-search">
      <div className="cfb-universal-search-input">
        <span>⌕</span>
        <input value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Search teams, coaches, games, and pages"/>
        {query && <button type="button" onClick={()=>setQuery("")}>×</button>}
      </div>
      {normalized && (
        <div className="cfb-universal-search-results">
          {resultsList.length ? resultsList.map((item)=>(
            <button type="button" key={item.key} onClick={()=>choose(item)}>
              <span className="cfb-search-result-logo">
                {item.logo ? <img src={item.logo} alt=""/> : item.type.slice(0,1)}
              </span>
              <span><b>{item.title}</b><small>{item.subtitle}</small></span>
              <em>{item.type}</em>
            </button>
          )) : <div className="cfb-search-empty">No matching teams, coaches, games, or pages.</div>}
        </div>
      )}
    </div>
  );
...
function TabBar({ tabs, activeTab, setActiveTab, draggedTab, setDraggedTab, reorderTabs, adminUnlocked, adminCodeInput, setAdminCodeInput, unlockAdmin, teams = [], assignments = [], currentYear, users: navUsers = [], discordSession, linkedDiscordUser, signInWithDiscord, signOutDiscord,soundPreferences={},draftIsLive=false}) {
  ...
  const coachKeys = tabs
    .filter(([key]) => key.startsWith("coach-"))
    .sort((a,b)=>String(a[1]||"").localeCompare(String(b[1]||""), undefined, { sensitivity:"base", numeric:true }))
    .map(([key])=>key);
  ...
  const groups = [
    ...
    { title: "League Tools", keys: additionalKeys },
    { title: "Commissioner", keys: commissionerKeys },
    { title: "Coach Profiles", coach:true, keys: coachKeys },
  ];
  const navMeta={dashboard:["⌂","League overview"],leagueHub:["#","Chat, roles and channels"],newsroom:["N","Verified league coverage"],dataIntake:["⇧","Screenshot intake and checklist"],schedule:["G","Games and final scores"],redZone:["●","Live league streams"],myTeam:["M","Your personal headquarters"],eliteBooks:["$","Weekly picks and futures"],rankingsCenter:["R","All league ranking systems"],teamsCoaches:["T","Teams, schedules and coach profiles"],leagueArchive:["A","Records, trophies and league history"],commissionerCenter:["⚙","Private league operations"],draftRoom:["D","Live league draft"]};
  ...
  function navThemeFor(tabKey) {
    ...
    if (tabKey==="newsroom") return ["#161006", "#c9d0d9", "#ffffff"];
    ...
    if (tabKey==="gameTop25") return ["#211705", "#c9d0d9", "#ffffff"];
  ...
  function NavButton({ tabKey, label }) {
    ...
    return (
      <button
        type="button"
        className={`drawer-nav-tile ${active?"active":""}`}
      ...
      >
        ...
        <span className="drawer-nav-copy"><strong>{label}</strong><small>{navMeta[tabKey]?.[1]||"Open league section"}</small></span>
        <span className={`drawer-nav-arrow ${active?"active":""}`} style={{color:accent}}>{active?"●":"›"}</span>
      </button>
    );
  }
    ...
    <>
      ...
      {menuOpen && (
        <div style={drawerOverlay} onClick={() => setMenuOpen(false)}>
          <aside className="cfb-nav-drawer-v54" style={drawerPanel} onClick={(event) => event.stopPropagation()}>
            ...
            <label className="drawer-menu-search"><span>FIND A PAGE</span><input value={menuSearch} onChange={(event)=>setMenuSearch(event.target.value)} placeholder="Search league navigation…"/></label>

            <div style={drawerContent}>
              {visibleGroups.filter((group)=>group.keys.length > 0).map((group) => (
                <div key={group.title} style={drawerGroup} className={`drawer-menu-group ${group.primary?"drawer-primary-group":""}`}>
                  <div style={drawerGroupTitle}>{group.title}</div>
                  <div className="drawer-menu-grid">{group.keys
                    .filter((key) => tabMap.has(key)&&(!menuSearch||`${tabMap.get(key)} ${group.title} ${navMeta[key]?.[1]||""}`.toLowerCase().includes(menuSearch.toLowerCase())))
                    .map((key) => group.coach
                      ? <CoachNavButton key={key} tabKey={key} label={tabMap.get(key)} />
                      : <NavButton key={key} tabKey={key} label={tabMap.get(key)} />)}</div>
                </div>
              ))}
            </div>
          </aside>
...
const statusBox={background:"linear-gradient(135deg,var(--cfb-red),var(--cfb-red-dark))",border:"1px solid rgba(248,113,113,.65)",padding:"11px 17px",borderRadius:7,fontWeight:1000,color:"#fff",cursor:"pointer",boxShadow:"0 12px 28px rgba(62,127,193,.20)",textTransform:"uppercase",letterSpacing:".04em"};
...
const button = {
  background: "linear-gradient(135deg,var(--cfb-red),var(--cfb-red-dark))",
  color: "#fff",
  ...
  fontWeight: 950,
  cursor: "pointer",
  boxShadow: "0 12px 28px rgba(62,127,193,.18)",
...
const recognitionTeamBadge={fontSize:13,color:"#dde3e9",maxWidth:220};
...
const hamburgerButton = {
  ...
  fontWeight: 1000,
  cursor: "pointer",
  boxShadow: "0 12px 28px rgba(62,127,193,.22)",
  textTransform:"uppercase",
  letterSpacing:".04em",
};
...
const drawerPanel = {
  ...
  height: "100%",
  background: "radial-gradient(circle at 0 0,rgba(62,127,193,.17),transparent 27%),linear-gradient(180deg,#080b12,#030509)",
...
const v2PrimaryButton={border:"1px solid var(--cfb-danger)",borderRadius:5,padding:"11px 15px",background:"linear-gradient(135deg,var(--cfb-red),var(--cfb-red-dark))",color:"#fff",fontWeight:1000,cursor:"pointer",boxShadow:"0 10px 24px rgba(62,127,193,.16)",textTransform:"uppercase",letterSpacing:".04em"};
...
const v2DangerButton={...v2DangerSoft,background:"#e5484d",borderColor:"#fca5a5",color:"#fff"};
...
const v2WeekTabActive={...v2WeekTab,background:"var(--cfb-gold)",borderColor:"#dde3e9",color:"#111827"};
