...
function LeagueHub({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],weeklyMatchups=[],currentYear,setActiveTab,setError}) {
  ...
  useEffect(()=>{loadChannelMessages();loadSocialExtras();setReplyingTo(null);setMessageSearch("");setPinnedOnly(false);},[selectedChannel]);
  ...
  const displayedMessages=rawDisplayedMessages.filter((message)=>{const searchMatch=!messageSearch||String(message.body||"").toLowerCase().includes(messageSearch.toLowerCase())||String(users.find((user)=>String(user.id)===String(message.author_discord_user_id))?.discord_username||"").toLowerCase().includes(messageSearch.toLowerCase());const pinMatch=!pinnedOnly||Boolean(message.is_pinned);return searchMatch&&pinMatch;});
