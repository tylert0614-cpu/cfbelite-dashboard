...
function LeagueHub({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],currentYear,setActiveTab,setError}) {
  ...
  return <main className="cfb-v2-page network-page">
    ...
    <div className="network-layout"><aside className="network-sidebar">
      <nav><button className={mode==="channels"?"active":""} onClick={()=>{setMode("channels");setManageOpen(false);}}>Channels</button><button className={mode==="direct"?"active":""} onClick={()=>{setMode("direct");setManageOpen(false);}}>Messages</button><button className={mode==="notifications"?"active":""} onClick={()=>{setMode("notifications");setManageOpen(false);}}>Alerts {unread>0&&<b>{unread}</b>}</button><button className={mode==="roles"?"active":""} onClick={()=>{setMode("roles");setManageOpen(false);}}>Roles</button><button className={mode==="emojis"?"active":""} onClick={()=>{setMode("emojis");setManageOpen(false);}}>Emojis</button><button className={mode==="settings"?"active":""} onClick={()=>{setMode("settings");setManageOpen(false);}}>Settings</button></nav>
      {mode==="channels"&&<><div className="network-sidebar-label"><span>CHANNELS</span>{canCreateChannels&&<button onClick={()=>openChannelManager()}>＋ New</button>}</div><div className="network-channel-list">{channels.map((channel)=><button key={channel.id} className={String(selectedChannel)===String(channel.id)?"active":""} onClick={()=>{setSelectedChannel(channel.id);setManageOpen(false);}}><NetworkChannelMark channel={channel} size={38}/><span><strong>{channel.name}</strong><small>{channel.description}</small></span>{channel.is_auto_matchup?<em>YOUR GAME</em>:channel.is_locked&&<em>LOCKED</em>}</button>)}</div><div className="network-online-roster"><header><i/><span>ONLINE NOW</span><b>{onlineUsers.length}</b></header>{onlineUsers.slice(0,12).map((user)=><div key={user.id}><span className="network-presence-dot"/><NetworkIdentity userId={user.id} users={users} teams={teams} assignments={assignments} currentYear={currentYear} compact/></div>)}{!onlineUsers.length&&<small>No other members are active right now.</small>}</div></>}
  ...
  function NavButton({ tabKey, label }) {
    ...
    return (
      ...
      >
        ...
        <b className="drawer-nav-icon" style={{color:accent,borderColor:`${accent}55`}}>{navMeta[tabKey]?.[0]||"•"}</b>
