...
function NetworkEmojiLibrary({customEmojis=[],isCommissioner,emojiUpload,setEmojiUpload,emojiBusy,uploadCustomEmoji,deleteCustomEmoji}){
  return <div className="network-emoji-library"><header><div><span>LEAGUE EMOJI VAULT</span><h2>Custom Reactions</h2><p>Use a custom emoji in chat by choosing it from the picker or typing its code.</p></div><b>{customEmojis.length} EMOJIS</b></header>{isCommissioner&&<div className="network-emoji-uploader"><label><span>Emoji Name</span><input value={emojiUpload.name} onChange={(event)=>setEmojiUpload({...emojiUpload,name:event.target.value})} placeholder="panther_party"/></label><label><span>Image • 512 KB max</span><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={(event)=>setEmojiUpload({...emojiUpload,file:event.target.files?.[0]||null})}/></label><button disabled={emojiBusy||!emojiUpload.name||!emojiUpload.file} onClick={uploadCustomEmoji}>{emojiBusy?"Uploading…":"Upload Emoji"}</button></div>}<div className="network-emoji-gallery">{customEmojis.map((emoji)=><article key={emoji.id}><img src={emoji.image_url} alt={emoji.name}/><strong>:{emoji.name}:</strong>{isCommissioner&&<button onClick={()=>deleteCustomEmoji(emoji)}>Delete</button>}</article>)}{!customEmojis.length&&<div className="network-empty"><b>No custom emojis yet.</b><span>Commissioners can upload the league's first reaction.</span></div>}</div></div>;
}
...
function LeagueHub({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],currentYear,setActiveTab,setError}) {
  ...
  async function loadNetworkShell() {
    if(!discordSession?.user)return;
    await supabase.rpc("ensure_league_network_profile");
    const [channelRes,conversationRes,memberRes,notificationRes,prefRes,presenceRes,permissionRes,emojiRes]=await Promise.all([
      supabase.from("league_channels").select("*").eq("is_archived",false).order("sort_order"),
      supabase.from("direct_conversations").select("*").order("last_message_at",{ascending:false}),
      supabase.from("direct_conversation_members").select("*"),
      supabase.from("app_notifications").select("*").order("created_at",{ascending:false}).limit(80),
    ...
    setConversations(conversationRes.data||[]);setConversationMembers(memberRes.data||[]);setPresence(presenceRes.data||[]);setPermissions(permissionRes.data||[]);setCustomEmojis(emojiRes.data||[]);
    const nextNotifications=notificationRes.data||[];
    if(lastNotificationRef.current&&nextNotifications[0]?.id&&nextNotifications[0].id!==lastNotificationRef.current&&prefRes.data?.sound_enabled!==false)playEliteSound("notification",true);
    lastNotificationRef.current=nextNotifications[0]?.id||lastNotificationRef.current;
    const nextPreferences=prefRes.data||null;setNotifications(nextNotifications);setPreferences(nextPreferences);
  ...
  const canModerate=isCommissioner||myPermission?.can_manage===true;
  ...
  return (
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      .network-sidebar > nav { grid-template-columns:1fr 1fr; }.network-sidebar > nav button:nth-child(5) { grid-column:1/-1; }
