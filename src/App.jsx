...
function LeagueHub({discordSession,linkedDiscordUser,users=[],teams=[],assignments=[],currentYear,setActiveTab,setError}) {
  ...
  async function loadNetworkShell() {
    if(!discordSession?.user)return;
    await supabase.rpc("ensure_league_network_profile");
    const [channelRes,conversationRes,memberRes,notificationRes,prefRes,presenceRes,permissionRes]=await Promise.all([
      supabase.from("league_channels").select("*").eq("is_archived",false).order("sort_order"),
      supabase.from("direct_conversations").select("*").order("last_message_at",{ascending:false}),
      supabase.from("direct_conversation_members").select("*"),
      supabase.from("app_notifications").select("*").order("created_at",{ascending:false}).limit(80),
    ...
    setConversations(conversationRes.data||[]);setConversationMembers(memberRes.data||[]);setPresence(presenceRes.data||[]);setPermissions(permissionRes.data||[]);
    const nextNotifications=notificationRes.data||[];
    if(lastNotificationRef.current&&nextNotifications[0]?.id&&nextNotifications[0].id!==lastNotificationRef.current&&prefRes.data?.sound_enabled!==false)playEliteSound("notification",true);
    lastNotificationRef.current=nextNotifications[0]?.id||lastNotificationRef.current;
    const nextPreferences=prefRes.data||null;setNotifications(nextNotifications);setPreferences(nextPreferences);
  ...
  return <main className="cfb-v2-page network-page">
    <section className="network-hero"><div><span>CFB ELITE NETWORK</span><h1>League Hub</h1><p>Every announcement, conversation and rivalry—inside the dynasty.</p></div><div className="network-live-presence"><i/><strong>{onlineUsers.length}</strong><span>ONLINE NOW</span></div></section>
    <div className="network-layout"><aside className="network-sidebar">
      <nav><button className={mode==="channels"?"active":""} onClick={()=>{setMode("channels");setManageOpen(false);}}>Channels</button><button className={mode==="direct"?"active":""} onClick={()=>{setMode("direct");setManageOpen(false);}}>Messages</button><button className={mode==="notifications"?"active":""} onClick={()=>{setMode("notifications");setManageOpen(false);}}>Alerts {unread>0&&<b>{unread}</b>}</button><button className={mode==="settings"?"active":""} onClick={()=>{setMode("settings");setManageOpen(false);}}>Settings</button></nav>
      {mode==="channels"&&<><div className="network-sidebar-label"><span>CHANNELS</span>{isCommissioner&&<button onClick={()=>openChannelManager()}>＋ New</button>}</div><div className="network-channel-list">{channels.map((channel)=><button key={channel.id} className={String(selectedChannel)===String(channel.id)?"active":""} onClick={()=>{setSelectedChannel(channel.id);setManageOpen(false);}}><NetworkChannelMark channel={channel} size={38}/><span><strong>{channel.name}</strong><small>{channel.description}</small></span>{channel.is_locked&&<em>LOCKED</em>}</button>)}</div><div className="network-online-roster"><header><i/><span>ONLINE NOW</span><b>{onlineUsers.length}</b></header>{onlineUsers.slice(0,12).map((user)=><div key={user.id}><span className="network-presence-dot"/><NetworkIdentity userId={user.id} users={users} teams={teams} assignments={assignments} currentYear={currentYear} compact/></div>)}{!onlineUsers.length&&<small>No other members are active right now.</small>}</div></>}
  ...
  return (
    <style>{`
      ...
      * { box-sizing: border-box; }
      ...
      /* League Network v25: readable typography, moderation and richer conversation UI. */
